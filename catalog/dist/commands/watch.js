...
export async function watchCommand(workspaceRefs, options) {
    ...
    const applyPendingChanges = async (realm, remoteMtimes) => {
        if (realm.pendingChanges.size === 0)
            return;
    ...
    const checkRealmForChanges = async (realm) => {
        try {
            ...
            for (const [fullUrl, mtime] of Object.entries(mtimesData)) {
                if (fullUrl.startsWith(realm.workspaceUrl)) {
                    ...
                    if (relativePath && !relativePath.startsWith('_')) {
                        remoteMtimes[relativePath] = mtime;
                    }
                }
            }
