...
const BOLD = '\x1b[1m';
const RESET = '\x1b[0m';
export async function shareCommand(workspace, options) {
    ...
    if (options.milestone) {
        ...
        const milestone = milestones.find(m => m.milestoneName?.toLowerCase().includes(options.milestone.toLowerCase()) ||
...
function getWorkspaceFiles(dir) {
    const files = [];
...
function detectSubfolder(workspaceDir) {
...
function generateBranchName(milestoneName) {
...
function generatePrBody(milestoneName, hash, fileCount) {
    const lines = ['## Summary'];
    if (milestoneName) {
