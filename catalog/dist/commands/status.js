...
export async function statusCommand(workspaceRef, options) {
...
async function statusAll(matrixClient) {
    ...
    for (const ws of workspaces) {
        ...
        if (changes.length === 0) {
            ...
            const parts = [];
            if (newRemote > 0)
                parts.push(`☁️ +${newRemote}`);
            if (modRemote > 0)
                parts.push(`☁️ ~${modRemote}`);
            if (newLocal > 0)
                parts.push(`📝 +${newLocal}`);
            if (modLocal > 0)
                parts.push(`📝 ~${modLocal}`);
            if (conflicts > 0)
                parts.push(`⚠️ !${conflicts}`);
            console.log(`   ${parts.join('  ')}`);
        }
        console.log('');
    }
}
async function analyzeChanges(localDir, workspaceUrl, manifest, mtimesResponse) {
...
async function statusSingle(localDir, workspaceUrl, manifest, matrixClient, options) {
