...
async function fetchSkillsFromRealm(realmUrl, jwt, isReadOnly = false) {
...
function parseSkillsResponse(data, realmUrl) {
...
function extractTitleFromInstructions(instructions) {
    if (!instructions)
        return undefined;
...
function getSkillsManifestPath() {
...
function saveSkillsManifest(manifest) {
    const manifestPath = getSkillsManifestPath();
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
}
function exportSkillsToClaudeCommands(manifest, targetDir) {
    const commandsDir = path.join(targetDir, '.claude', 'commands');
    if (!fs.existsSync(commandsDir)) {
        fs.mkdirSync(commandsDir, { recursive: true });
    }
    const enabledSkills = manifest.skills.filter(s => manifest.enabledSkillIds.includes(s.id));
...
export async function skillsCommand(options) {
    ...
    // Enable a skill
    if (options.enable) {
        const skill = manifest.skills.find(s => s.title.toLowerCase() === options.enable.toLowerCase() ||
            s.id === options.enable);
        if (!skill) {
    ...
    // Disable a skill
    if (options.disable) {
        const skill = manifest.skills.find(s => s.title.toLowerCase() === options.disable.toLowerCase() ||
