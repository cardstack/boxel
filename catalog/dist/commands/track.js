...
export async function trackCommand(workspaceRef, options) {
    ...
    const initializeFileStates = (dir, prefix = '') => {
        ...
        try {
            const watcher = fs.watch(dir, { recursive: !isLinux }, (eventType, filename) => {
                if (!filename)
                    return;
                // Skip internal files
                if (filename.startsWith('.boxel-') || filename.includes('.git'))
                    return;
                if (filename.startsWith('.') && filename !== '.realm.json')
                    return;
