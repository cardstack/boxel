...
| `@fields`                                                                          | Host-owned contextual FieldDef components. Invoking one delegates rendering back through the correct card/field boundary.                                                                                | Normal child-local Base fields; root edits return through the document-update channel.                             | Field lookup is explicit and may include inherited `cardInfo`; it must not expose unrelated parent-card state.    |
