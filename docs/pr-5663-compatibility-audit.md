...
| Rich Markdown embedded-card rendering and canonical references                                                       | After indexing was fixed, existing `RichMarkdownField` tests passed only 15/28. `MarkdownTemplate` calls Base `getComponent(instance)` for linked cards, but user-realm query results are intentionally opaque.                                                              | 1 — explicit boundary API                   | Fixed locally. Opaque records carry a non-enumerable trusted `delegatedCardRenderComponent`; Base `getComponent()` consumes it before constructor introspection. The host adapter routes SES/iframe rendering without exposing the sandbox service to Base or card code. Full existing module now passes 28/28.                                                                                                                                          |
