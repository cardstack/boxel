...
| iframe height ownership                        | Format presentation carries `intrinsic` or `allocated` explicitly                            | Same protocol in Code preview                                                             | Embedded/atom/edit/isolated default to intrinsic measurement; fitted defaults to a Host-allocated viewport. Child resize reports are ignored in allocated mode. Docked/internal-scroll layouts need an explicit allocated render contract. |
