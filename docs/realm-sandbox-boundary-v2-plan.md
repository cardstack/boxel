...
The detailed four-zoom-level execution and rendering design now lives in
[boxel-execution-runtime-architecture.md](boxel-execution-runtime-architecture.md).
This document remains the focused plan for boundary record consolidation.
...
`hasCustomIsolatedTemplate` as compatibility getters from `formats`. Delete
...
```ts
export interface CardBoundaryRecord {
  protocolVersion: number;
...
Introduce `buildCardBoundaryRecord()` as a pure Host assembler over already
captured inert inputs. It must not import or introspect authored CardDef code
in the Host.

```ts
buildCardBoundaryRecord({
  canonicalDocument,
  cardType,
...
3. Add `CardTypeDescription`, `FormatDescription`, `CardBoundaryRecord`, and
   validation without changing consumers.
4. Implement pure `buildCardBoundaryRecord()` from the current inert capture
   outputs.
