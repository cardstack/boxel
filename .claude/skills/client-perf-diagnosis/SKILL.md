...
Rebuilds come from two machines, named by the event's `rebuild_source` (so named because the firelens wrapper's own `source` key would shadow it in the wrapper-unwrap parse):
...
A save of a loaded module in a subscribed realm pays both — one rebuild per source is the expected shape, not a double-count.
