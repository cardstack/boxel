...
**A request's duration is not its network time.** Send→finish in the trace, like `duration` in resource timing, includes the time a response waited for a busy main thread to process it. A burst of requests the HTTP cache answered can each read over a second. To decide cache versus network, read the network stack's own fields on the trace's `ResourceReceiveResponse` / `ResourceFinish` events: `fromCache`, `encodedDataLength`, and `timing.receiveHeadersEnd`. For a request the service worker answered (`fromServiceWorker: true`), the page-side record says nothing about the network, so count requests server-side instead.
