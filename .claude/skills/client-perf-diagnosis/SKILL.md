...
`top_frame_function` is the same frame as a scalar, for grouping, and `source_url` / `line` / `col` locate the throw — for a rejection, which carries no location of its own, from the stack's first frame. How much those locate depends on where the code came from: a frame in a built asset (`/assets/*.js`) source-maps back to real code, but a frame in a card module is a **blob url** minted per tab and per rebuild, which resolves to nothing later. For card-module frames `top_frame_function` is the only durable handle.
