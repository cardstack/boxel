...
- `waits.semaphoreMs` + `waits.tabQueueMs` + `waits.tabStartupMs` ≈ `launchMs`. (`launchMs` is measured around the full `PagePool.getPage` call; the three sub-waits don't overlap.)
