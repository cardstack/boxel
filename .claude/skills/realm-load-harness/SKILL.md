...
the entry bundle or the host build version changed, if a replica answered the
closing probe that had not answered the opening one, or if nothing answered at
all. It also refuses to start when the fleet is already serving two host builds
at once, which happens while a host deploy rolls, because each replica caches
the boot document for the life of its process.
