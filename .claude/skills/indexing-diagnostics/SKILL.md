...
Each affinity with pending work gets a `priorities=` segment (skipped on idle affinities for log scannability). Format: `<source>:<priority>:<count>` pairs, comma-separated within a source, sources separated by `|`. `tab:` is the per-tab queue's waiters; `adm:` is the per-affinity file-admission semaphore's waiters. Priorities listed highest-first, matching dequeue order — so `tab:10:1,0:3` means "1 priority-10 waiter at the head of the queue, 3 priority-0 waiters behind it."
