...
```jsonc
"screenshotErrors": [
  { "name": "hero",                       // slot name; '*' = the whole roster read failed
    "message": "render never painted",
    "captureMs": 15310,                   // wall-clock of the failed attempt (absent if never attempted)
    "consecutiveFailures": 2 }            // this slot's run of consecutive failing renders
]
```

**The retry lane.** The `prerender-html-reconcile` sweep re-renders rows with recorded capture failures the same way it retries `visitRequestFailure` error rows: eligible while any recorded slot's `consecutiveFailures` is below `DECLARED_SCREENSHOT_CAPTURE_RETRY_CAP` (3) and the row's `rendered_at` is at least `DECLARED_SCREENSHOT_CAPTURE_RETRY_MIN_AGE_MS` (45 min) old; an entry without a count is a legacy row, read as a run of one. Once every recorded slot reaches the cap the absence is terminal until the URL's next invalidation. So: a `consecutiveFailures` at the cap plus an old `rendered_at` means "the platform has given up on this slot — fix the card (or the underlying media) and edit/reindex"; a count below the cap means a retry is still coming.
...
- **Older rows predate the bookkeeping.** Rows written before `consecutiveFailures` shipped carry entries without counts; the sweep and the persist path both read those as a run of one, so they get (nearly) the full retry allowance again.
