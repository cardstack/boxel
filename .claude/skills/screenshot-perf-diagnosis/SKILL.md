...
- **Over-bounds is an error, not a slow success.** The PDF leg caps output at **20 pages / 10 MB**, enforced post-render. Past either, the capture fails: `capture.status=error` (its `contentType` may be null if it never finished the document) and `request.outcome=error`, with the cap named in the capture error line — join by `correlationId` to the realm-server's `realm:requests` / error-doc line for the message (which names the offending page count or byte size). Read this as "this card is too big to be one PDF," not as a pipeline regression — the fix is the card's content or print CSS. An over-bounds render still burned the render time, so it can be both slow **and** failed.
- **Document dimensions.** Page count and byte size are properties of the produced document, not perf stages: the byte size is the persisted object's size (the store / ledger object the `?type=pdf` URL serves), and an over-bounds failure names both in its error message. For the render internals behind a slow pagination, hand off to `indexing-diagnostics` by `prerenderRequestId` as with any capture.
