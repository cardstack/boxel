...
- **Register the test** under the manifest's `tests`: the host `--filter` string (a module-name match) or the realm-server `TEST_FILES` entry. The catalog CI runs exactly these when a catalog PR touches the subset.
