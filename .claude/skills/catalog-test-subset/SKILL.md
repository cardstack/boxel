...
5. **Merge the catalog PR first.** Then re-pin the boxel PR to a commit on catalog `main` with `pnpm --dir packages/catalog catalog:test-subset --bump`, run the sync, re-run the consumers, and merge it. On a boxel PR that touches the manifest or the sync script, the `Catalog Test Subset` workflow fails when the pin is not on catalog `main`, or when catalog `main` has changed a subset file since the pin.
...
| `lint:no-catalog-subset-copies`         | CI Lint, as part of `pnpm lint` in `packages/base`                                      | a file in `packages/base` with a subset file's name, or declaring a class a subset file exports                                                                                                                                                                                           |
