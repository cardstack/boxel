...
async function writeStamp(sha, subject, date) {
  ...
  await writeFile(
    ...
    [
      ...
      '',
      'source: https://github.com/christse/deck',
      `deck-commit: ${sha}`,
...
async function pull(deckRepo) {
  ...
  // A stamp taken from a dirty checkout records a commit that does not
  // contain the bytes being copied — the provenance would be a lie.
  if (git(repo, ['status', '--porcelain'])) {
    fail(
      `${repo} has uncommitted changes; commit them so the recorded sha matches the bytes`,
    );
  }
  for (let sub of VENDORED) {
    let r = spawnSync(
