...
| 12  | a test's `sinon.stub(NS, 'fn')` / `(NS as any).fn = …` silently no-ops                               | ES-module namespace objects (`import * as NS`) are sealed; their bindings are read-only                      | inject the dependency, or stub through a mutable indirection — not the namespace                                                                                                                         | **manual** (per-test; e.g. ai-bot Sentry/locking tests)                                                                                            |
...
- **ai-bot**: 5 tests fail on class #12 (ESM namespace mocking) — needs DI/indirection.
- **software-factory**: full node suite needs `boxel-cli` + `software-factory`'s own
  source migrated (their `src/logger.ts` etc. still use bare `require()`); the
  bootstrap wiring is in place.
