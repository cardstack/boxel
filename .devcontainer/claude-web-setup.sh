...
#
# Why this differs from the Codespace (.devcontainer/start-services.sh):
# the cloud VM runs the whole stack on localhost — there is no GitHub edge —
# so it uses the repo's STANDARD dev tooling (`mise run dev`) and needs NONE of
# the Codespace edge-bridging (REALM_SERVER_ASSUME_HTTPS, the permission
# rewrite, the Matrix/icons proxy, port visibility, the TLS shim). The realm is
# at https://localhost:4201, permissions already match the localhost default,
# and the worker/prerender reach it directly. It's just normal local dev.
#
