...
export default class DetailPanel extends Component<Signature> {
  ...
  private get isListingCard() {
    return this.args.cardInstance?.constructor?.name?.endsWith?.('Listing') ?? false;
  }

  private get isSelectedFieldDefinition() {
    return (
      this.args.selectedDeclaration &&
      isCardOrFieldDeclaration(this.args.selectedDeclaration) &&
  ...
  private get isSelectedCardDefinition() {
    return (
      this.args.selectedDeclaration &&
      isCardOrFieldDeclaration(this.args.selectedDeclaration) &&
  ...
  private get isSelectedListingField() {
    if (!this.isSelectedFieldDefinition) {
      return false;
    }
    const fieldName = this.args.selectedDeclaration?.exportName || 
                      this.args.selectedDeclaration?.localName || '';
    return fieldName.endsWith('Listing');
  }

  private get isSelectedListingCard() {
    if (!this.isSelectedCardDefinition) {
      return false;
    }
    const cardName = this.args.selectedDeclaration?.exportName || 
                     this.args.selectedDeclaration?.localName || '';
    return cardName.endsWith('Listing');
  }
  ...
  private get definitionActions() {
    ...
    if (
      this.isSelectedFieldDefinition &&
      !this.isSelectedListingField &&
      this.args.selectedDeclaration?.exportName &&
    ...
    // Add "Create listing" for card definitions if not a listing card and user can edit
    // Note: Users can create multiple listings, so we don't check if a listing already exists
    if (
      this.isSelectedCardDefinition &&
      !this.isSelectedListingCard &&
      this.args.selectedDeclaration?.exportName &&
      this.realm.canWrite(this.args.readyFile.url)
    ) {
      actions.push({
  ...
  private get instanceActions() {
    ...
    if (!this.isListingCard && this.realm.canWrite(this.args.readyFile.url)) {
      actions.push({
        label: 'Create listing',
        icon: Package,
        handler: this.createListingWithAI,
  ...
  @action private async createListingWithAI() {
    const command = new ListingCreateCommand(this.commandService.commandContext);
    
    if (this.isCardInstance && this.args.cardInstance?.id) {
      // Card instance case
      await command.execute({
        openCardId: this.args.cardInstance.id as string,
      });
    } else if (this.isSelectedFieldDefinition) {
      // Field definition case
      const codeRef = this.selectedDeclarationAsCodeRef;
      await command.execute({
        codeRef,
      });
    } else if (this.isSelectedCardDefinition) {
