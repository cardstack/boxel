...
export default class RenderService extends Service {
    ...
    try {
      if (waitForAsync) {
        ...
        for (let i = 0; i < MAX_ASYNC_RENDER_PASSES; i++) {
          this.#renderWithComputeMemo(api, component, element, format);
          await waitForAsync();
        }
      }
  ...
  // Wrap one synchronous Glimmer render flush in a compute-memo pass.
  //
  ...
  //
  // The pass MUST be scoped to a purely synchronous region: `render()` runs
  // `iterator.sync()` with no awaits, and the `finally` closes the pass BEFORE
  // this method returns and before the caller's next `await`. A memo leaking
  // across an await would corrupt a later reactive cycle, so it never spans the
  // inter-pass `waitForAsync()`. Guarded with a `typeof` check like
  // `render.meta`: during a cold boot the host can briefly hold a base/card-api
  // build predating `beginComputePass`, in which case we render without it
  // (still correct).
