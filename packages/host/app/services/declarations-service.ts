...
export default class DeclarationsService extends Service {
  @tracked declarations: ModuleDeclaration[] = [];

  load = restartableTask(async (executableFile: ReadyFile) => {
    //==loading module
    ...
    for (const value of moduleSyntax.declarations) {
      if (isPossibleCardOrFieldClass(value)) {
        ...
        if (cardOrField) {
          let cardType = await getCardType(this, () => cardOrField);
          results.push({
            ...value,
            cardOrField,
            cardType,
          ...
          let cardType = await getCardType(this, () => cardOrField); // this should probably be `await getCardType(...)`
          results.push({
            ...value,
            cardOrField,
            cardType,
