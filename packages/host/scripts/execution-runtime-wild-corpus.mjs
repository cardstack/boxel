/**
 * Fifty persisted CTSE cards used as the URL-first compatibility lane.
 *
 * This is intentionally a matrix of real card URLs, not synthetic GTS test
 * modules. Every instance was selected from the current boxel-workspaces
 * mirror and checked to ensure its adoptsFrom points at a current, non-history
 * module (or trusted Base). The deployed staging Host is the reference and the
