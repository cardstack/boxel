...
module('Integration | schema', function (hooks) {
  ...
  test('it can add a new contains field to a card', async function (assert) {
    ...
    assert
      .shadowDOM(
        `[data-test-demo-embedded]`,
        shadowQuerySelector('[data-test-card-catalog]')
      )
      .exists({ count: 1 }, 'demo card is not displayed for primitive fields');
...
async function getFileResource(
  ...
  let content = (await adapter.openFile(paths.local(new URL(fileURL))))
