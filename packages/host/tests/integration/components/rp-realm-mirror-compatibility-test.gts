...
module('Integration | RP realm mirror compatibility', function (hooks) {
  ...
  test('RP-6.4: an ordinary top-level CardRenderer call routes a mirror card through Capsule and preserves nested FieldDef delegation', async function (assert) {
    ...
    assert.dom('[data-corpus="nested-field"]').containsText('18 Orchard Lane');
  ...
  test('RP-6.3, RP-6.4: the same mirror card resolves its missing authored edit format to the Direct trusted Base surface', async function (assert) {
    ...
    let values = [...document.querySelectorAll<HTMLInputElement>('input')].map(
      (input) => input.value,
    );
    for (let expected of [
  ...
  test('RP-2.6, RP-6.4, RP-8.4: relationship-backed mirror composition loads and delegates one and many linked cards inside Capsule', async function (assert) {
    ...
    await waitUntil(
      () =>
        document.querySelectorAll('[data-corpus="linked-person"]').length === 2,
      { timeout: 10000 },
    );
    ...
    assert.dom('[data-corpus="linked-project"]').containsText('Accessibility');
  ...
  test('RP-4.1, RP-6.4, RP-11.2: a primitive mirror preserves function-form computeVia and presentation metadata in Capsule', async function (assert) {
    ...
    assert.dom('[data-corpus="primitive-profile"] h1').hasText('Avery Rivera');
    assert
  ...
  test('RP-2.6, RP-6.4: authored embedded and fitted formats from one mirror module remain independently selectable in Capsule', async function (assert) {
    let card = (await getService('store').get(
      `${testRealmURL}PrimitiveProfile/sample`,
    )) as BaseDef;
  ...
  test('RP-2.6, RP-6.3, RP-6.4: a trusted RichMarkdown FieldDef composes authored atom, embedded, fitted, and isolated cards inside Capsule', async function (assert) {
    let card = (await getService('store').get(
