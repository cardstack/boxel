...
module('Integration | operator-mode', function (hooks) {
  ...
  module('matrix', function () {
    ...
    test('it will throw error when patch code refers to nonexistent or incorrect field', async function (assert) {
      ...
      assert
        .dom('[data-test-message-idx="2"] [data-test-apply-state="failed"]')
        .exists();
      assert
        .dom(`[data-test-message-idx="2"] [data-test-card-error]`)
        .hasText(`Failed to apply changes. Patch failed.`);
      ...
      await click('[data-test-command-apply]');
      await waitFor('[data-test-message-idx="3"] [data-test-patch-card-idle]');
      assert
        .dom('[data-test-message-idx="3"] [data-test-apply-state="failed"]')
        .exists();
      assert
        .dom(`[data-test-message-idx="3"] [data-test-card-error]`)
        .hasText(`Failed to apply changes. Patch failed.`);
      assert.dom('[data-test-preferredcarrier="DHL"]').exists();
    });
