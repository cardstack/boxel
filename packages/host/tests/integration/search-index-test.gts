    ...
    module(
      ...
      function (hooks) {
        ...
        hooks.beforeEach(function (this: RenderingTestContext) {
          (globalThis as any).__enablePgIndexer = () => isDbIndexingEnabled;
          loader = (
        ...
        test('can recover from indexing a card with a broken link', async function (assert) {
          ...
          );
          {
            ...
            if (mango?.type === 'doc') {
              ...
              assert.ok(
                false,
                `search entry was an error: ${mango?.error.detail}`,
              );
            }
          }
        });
        ...
        test('can index a card that has a cyclic relationship with the field of a card in its fields', async function (assert) {
          ...
          if (card?.type === 'doc') {
            ...
            assert.ok(
              false,
              `search entry was an error: ${card?.error.detail}`,
            );
          }
        });
        ...
        test('can index a card with a containsMany composite containing a linkTo field', async function (assert) {
          ...
          if (vendor?.type === 'doc') {
            ...
            assert.ok(
              false,
              `search entry was an error: ${vendor?.error.detail}`,
            );
          }
        });
        ...
        test('can index a card with linksToMany field', async function (assert) {
          ...
          if (hassan?.type === 'doc') {
            ...
            assert.ok(
              false,
              `search entry was an error: ${hassan?.error.detail}`,
            );
          }
        ...
        test('can index a field with a cycle in the linksToMany field', async function (assert) {
          ...
          if (vanGogh?.type === 'doc') {
            ...
            assert.ok(
              false,
              `search entry was an error: ${vanGogh?.error.detail}`,
            );
          }
