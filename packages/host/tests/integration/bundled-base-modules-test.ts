...
module('Integration | bundled base modules', function (hooks) {
  ...
  // Skipped: fails today, and the junit the aggregate check reads counts a
  // `todo` test's failure as a failure, so `todo` would turn the suite red.
  // A loader credits a class to the first module it
  ...
  // type.
  skip('a bundled module that re-exports a class does not take its identity', async function (assert) {
    let loader = getService('loader-service').loader;
