...
module('Integration | searchable search doc', function (hooks) {
  ...
  // Measurement: this module reached 1.12m (from 8.16m) with
  // `reuseIndexAcrossTests`, which restores one snapshot per module and skips
  // the boot index. Here it uses `withCachedRealmSetup` instead — restore, then
  // still run setup, leaving the boot index a no-op — to see how much of that
  // win the older mechanism delivers on its own.
