...
module('Integration | Modifier | attach-styles', function (hooks) {
  ...
  test('can correctly add and teardown constructable stylesheets in shadow root', async function (assert) {
    ...
    class Isolated extends Component<typeof Person> {
      ...
      toggleStyles2 = () => {
        this.applyStyles2 = !this.applyStyles2;
      };
    ...
    assert.strictEqual(
      root.adoptedStyleSheets.length,
      1,
      'stylesheet 1 attached'
    );
    assert.ok(
      [...root.adoptedStyleSheets][0].cssRules[0].cssText.includes(
        '{ color: magenta; }'
      ),
      'sheet 1 rule matches'
    );
  ...
  test('can correctly add and teardown constructable stylesheets in nested shadow roots', async function (assert) {
    ...
    class Isolated extends Component<typeof Post> {
      ...
      toggleAuthorStyles = () => {
        this.applyAuthorStyles = !this.applyAuthorStyles;
      };
      <template>
    ...
    assert.ok(
      [...root.adoptedStyleSheets][0].cssRules[1].cssText.includes(
        `p { color: blue; font-weight: bold; }`
      ),
    ...
    assert.ok(
      [...root.adoptedStyleSheets][1].cssRules[0].cssText.includes(
        'lightyellow'
      ),
      'author rules are correct'
    );
    let personRoot = shadowQuerySelector('[data-test-person]')
      .parentNode! as ShadowRoot;
    assert.strictEqual(
      personRoot.adoptedStyleSheets.length,
      1,
      'embedded person styles are present only in person root'
    );
    assert.ok(
      [...personRoot.adoptedStyleSheets][0].cssRules[0].cssText.includes(
        'lightblue'
      ),
      'embedded person rules are correct'
    );
    ...
    assert.ok(
      ...
      'author styles remain'
    );
    assert.strictEqual(
      personRoot.adoptedStyleSheets.length,
      1,
      'person styles remain'
    );
    assert.ok(
      [...personRoot.adoptedStyleSheets][0].cssRules[0].cssText.includes(
        'lightblue'
      ),
      'person styles are correct'
    );
