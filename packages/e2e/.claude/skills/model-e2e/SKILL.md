...
Runs are serial (one tab). Budget roughly one to three minutes per good model,
up to `SMOKE_MAX_MINUTES` per bad one.
...
| ------- | --------------------------- | ------ | ------- | ----- | ------------------------------------------------- | ------ | ------ | ------------ | ---- | ------------------ |
| ✅ GOOD | anthropic/claude-sonnet-4.6 | –      | pass    | 5     | readRealmFile ×1, switch-submode ×1, show-card ×1 | 2      | $0.090 | 87% (0 miss) | 63 s | yes, preview panel |
...
2. **Did prompt caching work?** The Cache column gives the headline; confirm it
   ...
   `cached` should climb turn over turn and cover nearly the whole prompt from
   turn two on. A reset to a low number mid-session means the history was
   rewritten or the provider changed; a gap of several minutes between turns
   means the cache expired. Report a miss with the turn it happened on and its
   cost, it is usually the single most expensive turn of the run.
...
| cost                         | Cost column                                                                                                           | under $0.20 per run. Judge it against the model's price class: Sonnet 4.6 lands at $0.05 to $0.10, so $0.20 for it already means extra turns; an Opus-class model costs about twice that for the same work. A run over $1 is a fragmentation problem, not a price problem |
| prompt cache                 | Cache column: share of all input tokens served from the cache, and the number of turns after the first that missed it | 80% or more, 0 misses. The conversation only grows, so from turn two on nearly the whole prompt should be a cache read                                                                                                                                                    |
