  ...
  module('queue', function (hooks) {
    ...
    module(
      ...
      function (nestedHooks) {
        ...
        test('daily-credit-grant: concurrent enqueues from two instances with same threshold coalesce', async function (assert) {
          let [first, second] = await Promise.all([
            publisher.publish({
          ...
          assert.strictEqual(
            first.id,
            second.id,
            'two instances enqueueing daily-credit-grant with same threshold converge',
          );

          let rows = (await adapter.execute(
            `SELECT id
             FROM jobs
             WHERE job_type = 'daily-credit-grant' AND status = 'unfulfilled'`,
          )) as { id: number }[];
          assert.strictEqual(
            rows.length,
            1,
            'only one pending daily-credit-grant row exists',
          );
        });

        test('daily-credit-grant: enqueues with different thresholds stay as separate jobs', async function (assert) {
          let first = await publisher.publish({
            jobType: 'daily-credit-grant',
            concurrencyGroup: 'daily-credit-grant',
            timeout: 10 * 60,
          ...
          let second = await publisher2.publish({
            ...
            timeout: 10 * 60,
            priority: 4,
            args: { lowCreditThreshold: 250 },
          });

          assert.notStrictEqual(
            first.id,
            second.id,
            'different thresholds describe distinct intent; rows are not coalesced',
          );
          ...
          assert.strictEqual(
            rows.length,
            2,
            'both pending daily-credit-grant rows are retained when thresholds differ',
          );
        });
      },
    );
