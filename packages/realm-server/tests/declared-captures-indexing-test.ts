...
function makeFileSystem() {
  return {
    'product.gts': `
      import { contains, field, linksTo, CardDef, Component, type CaptureSpec } from "@cardstack/base/card-api";
      ...
      // Renders linked data no display format touches — the capture's
      ...
      // Declares a pdf capture: a paged document of the isolated render,
      ...
      // (CAPTURE_MAX_PDF_ENTRIES = 3). The roster is captured name-sorted,
      ...
      class BrokenDocument extends Component<typeof BrokenReport> {
        markFailed = modifier((el) => {
          el.removeAttribute('data-capture-pending');
          el.setAttribute('data-capture-failed', 'fixture: document cannot render');
        });
        <template>
          <div data-capture-pending='true' {{this.markFailed}}>never ready</div>
        </template>
      }
      ...
      // render context's declaration-derived meta.captures: the durable
        ...
        static isolated = class Isolated extends Component<typeof this> {
          <template>
            ...
            {{#if @model.captureURLs.card}}
              <img src={{@model.captureURLs.card}} alt='self preview' />
            {{/if}}
          </template>
        }
        static captures: Record<string, CaptureSpec> = {
      ...
      class DoomedShot extends Component<typeof Doomed> {
        markFailed = modifier((el) => {
          el.removeAttribute('data-capture-pending');
          el.setAttribute('data-capture-failed', 'fixture: content cannot decode');
        });
        <template>
          <div data-capture-pending='true' {{this.markFailed}}>never ready</div>
        </template>
...
module(basename(import.meta.filename), function (hooks) {
  ...
  test('the prerender-html pass captures declared captures, persists them, and records the manifest', async function (assert) {
    ...
    let fileManifest = fileRow?.captures ?? null;
    assert.strictEqual(
      fileManifest,
      null,
    ...
    let errors = (row!.diagnostics as any)?.captureErrors;
    assert.strictEqual(
      errors,
      undefined,
    ...
    let timings = (row!.diagnostics as any)?.captureTimingsMs;
    assert.deepEqual(
  ...
  test('card+json joins the manifest into meta.captures and the ?name= URL serves the capture', async function (assert) {
    await writeAndSettle('widget.json', productDoc('Widget'));
    let row = await prerenderedHtmlRowFor(
      testDbAdapter,
      `${testRealm}widget.json`,
    );
    let manifest = row!.captures as CaptureManifest;
    ...
    assert.deepEqual(
      ...
      {
        card: {
          url: `${testRealm}_capture/widget?name=card`,
        ...
        hero: {
          url: `${testRealm}_capture/widget?name=hero`,
    ...
    let served = await realm.handle(
      new Request(`${testRealm}_capture/widget?name=card`),
    );
  ...
  test('a declared pdf is captured under print media, persisted, and served at ?name=', async function (assert) {
    ...
    let statementTiming = (row!.diagnostics as any)?.captureTimingsMs
      ?.statement;
    assert.strictEqual(
    ...
    let json = await response!.json();
    assert.deepEqual(
      json.data.meta.captures.statement,
      {
        url: `${testRealm}_capture/report?name=statement`,
        hash: statement.objectKey,
        contentType: 'application/pdf',
        pageCount: statement.pageCount,
        byteSize: statement.byteSize,
      },
      'meta.captures projects the pdf entry with its paged facts, no raster geometry',
    );

    let served = await realm.handle(
      new Request(`${testRealm}_capture/report?name=statement`),
    );
    assert.strictEqual(served!.status, 200);
    assert.strictEqual(
  ...
  test('declared pdf slots beyond the per-card pdf cap overflow deterministically with a recorded error', async function (assert) {
    ...
    assert.ok(overflow, 'the overflow slot records a capture error');
    assert.ok(
  ...
  test('a declared pdf render component paginates its own document flow, not the isolated template', async function (assert) {
    ...
    let served = await realm.handle(
      new Request(`${testRealm}_capture/invoice?name=invoice`),
    );
    assert.strictEqual(served!.status, 200);
    assert.strictEqual(
      served!.headers.get('content-type'),
  ...
  test('a failing pdf render component fails its slot under the broken-links model without failing the row', async function (assert) {
    ...
    assert.ok(docError, 'the failed pdf slot records a capture error');
    assert.ok(
      docError!.message.includes(
        'signaled data-capture-failed: fixture: document cannot render',
      ),
  ...
  test('a card can embed its own declared capture on its first prerender pass', async function (assert) {
    ...
    assert.true(
      (row!.isolated_html ?? '').includes(
        `${testRealm}_capture/self-promo-1?name=card`,
      ),
  ...
  test('a useAsThumbnail capture feeds the default fitted tile with no template edits', async function (assert) {
    ...
    assert.true(
      fittedHtml.includes(`${testRealm}_capture/gallery-1?name=tile`),
  ...
  test('a card with no declarations writes a null manifest and no ledger rows', async function (assert) {
    ...
    assert.ok(row, 'the instance row exists');
    let manifest = row!.captures ?? null;
  ...
  test('a data-capture-failed signal fails its slot immediately; sibling slots still capture', async function (assert) {
    ...
    assert.ok(doomedError, 'the failed slot records a capture error');
    assert.ok(
      doomedError!.message.includes(
        'signaled data-capture-failed: fixture: content cannot decode',
      ),
  ...
  test('a re-render captures at the new generation and supersedes the prior ledger row', async function (assert) {
    ...
    let manifest = secondRow!.captures as CaptureManifest;
    assert.deepEqual(Object.keys(manifest).sort(), ['card', 'hero']);
  ...
  test('a capture-only component’s linked-data loads land in the row’s deps and invalidate the capture', async function (assert) {
    await writeAndSettle(
      'maker.json',
      JSON.stringify({
        data: {
    ...
    let heroBefore = (row!.captures as CaptureManifest).hero;
    ...
    let controlHero = (controlRow!.captures as CaptureManifest).hero;
    ...
    // Editing the linked data must fan out to this row — the capture of
    ...
    assert.ok(
      ...
      'editing the linked card re-rendered the capture’s row',
    );
    let heroAfter = (after!.captures as CaptureManifest).hero;
    assert.notStrictEqual(
      heroAfter.objectKey,
      heroBefore.objectKey,
    ...
    // card stops invalidating this capture.
