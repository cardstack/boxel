...
// The two things a caller asks an outcome, read through one accessor each so an
// assertion states the question rather than re-deriving which shape came back.
function rejectionIn(
  outcome: { rejected: string } | { affected: number },
): string | undefined {
  return 'rejected' in outcome ? outcome.rejected : undefined;
}

function affectedBy(
  outcome: { rejected: string } | { affected: number },
): number {
  return 'affected' in outcome ? outcome.affected : 0;
}
  ...
  module('operation lowering is executable', function () {
    test('every program lowering emits can be planned by the executor', async function (assert) {
      ...
      for (let { name, writes } of EXECUTABLE_DECLARATIONS) {
        ...
        assert.notOk(rejectionIn(outcome), `${name} plans: ${source}`);
        if (writes) {
    ...
    test('an assert over a link collection fires on a duplicate and not otherwise', async function (assert) {
      ...
      assert.true(
        Boolean(rejectionIn(onDuplicate)?.includes('assertion-failed')),
