...
function buildModuleResponse(
  ...
  const definitionId = internalKeyFor({ module: rri(moduleURL), name }, undefined);
  return {
    id: moduleURL,
    ...
    createdAt: Date.now(),
    deps,
    definitions: error ? {} : { [definitionId]: buildDefinition(moduleURL, name) },
    error,
  };
}
...
function totalCalls(...sets: Array<GatedPrerendererControls>): (url: string) => number {
...
module(basename(__filename), function () {
  module('ModuleCacheCoordinator unit', function (hooks) {
    ...
    test('waitForKey: NOTIFY for an unrelated key does not resolve', async function (assert) {
      ...
      try {
        await new Promise((r) => setTimeout(r, 100));
        let resolved = false;
        const wait = coordinator
          .waitForKey('target-key', 400)
          .then(() => {
            resolved = true;
          });
        await new Promise((r) => setTimeout(r, 50));
  ...
  module('CachingDefinitionLookup coordinated path (integration)', function (hooks) {
    let dbAdapter: PgAdapter;
    ...
    test('two instances + concurrent lookup of same module → exactly one prerender call (peer wins, other re-reads)', async function (assert) {
      ...
      try {
        ...
        assert.ok(
          defA && 'definition' in defA && defA.definition.displayName === 'Foo',
          'A entry has the Foo definition',
        );
        assert.ok(
          defB && 'definition' in defB && defB.definition.displayName === 'Foo',
          'B entry has the Foo definition',
        );
