...
module(basename(__filename), function () {
  module('Prerender server', function (hooks) {
    ...
    test('dispose-affinity tears down warmed pages for the affinity', async function (assert) {
      let url = `${realmURL.href}3`;
      let permissions: Record<string, ('read' | 'write' | 'realm-owner')[]> = {
        [realmURL.href]: ['read', 'write', 'realm-owner'],
      };
      let auth = testCreatePrerenderAuth(testUserId, permissions);
      // Warm the affinity so the prerender pool has at least one
      // tracked page for the realm.
      await request
        .post('/prerender-visit')
        .set('Accept', 'application/vnd.api+json')
        .set('Content-Type', 'application/json')
      ...
      assert.ok(
        prerenderer.getVacancySnapshot()[affinityKey],
        'precondition: vacancy snapshot includes the warmed affinity',
      );

      let res = await request
        .post('/dispose-affinity')
        .set('Accept', 'application/vnd.api+json')
        .set('Content-Type', 'application/json')
        .send({
          data: {
      ...
      assert.strictEqual(res.status, 204, 'returns 204 on success');
      assert.notOk(
        prerenderer.getVacancySnapshot()[affinityKey],
        'vacancy snapshot no longer includes the disposed affinity',
      );
    });

    test('dispose-affinity for an unknown affinity is a no-op success', async function (assert) {
      let res = await request
        .post('/dispose-affinity')
        .set('Accept', 'application/vnd.api+json')
        .set('Content-Type', 'application/json')
        .send({
          data: {
            attributes: {
              affinityType: 'realm',
              affinityValue: 'http://127.0.0.1:9999/never-warmed/',
            },
          },
        });
      assert.strictEqual(
        res.status,
        204,
        'unknown affinity returns 204 (no-op) so callers can fire-and-forget',
      );
    });

    test('dispose-affinity rejects missing or invalid attributes', async function (assert) {
      let bad = await request
        .post('/dispose-affinity')
        .set('Accept', 'application/vnd.api+json')
        .set('Content-Type', 'application/json')
        .send({ data: { attributes: { affinityType: 'realm' } } });
      assert.strictEqual(bad.status, 400, 'missing affinityValue is rejected');

      let badType = await request
        .post('/dispose-affinity')
        .set('Accept', 'application/vnd.api+json')
        .set('Content-Type', 'application/json')
        .send({
          data: {
            attributes: { affinityType: 'project', affinityValue: 'x' },
          },
        });
      assert.strictEqual(
        badType.status,
