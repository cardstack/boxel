...
module(basename(__filename), function () {
  module('DefinitionLookup', function (hooks) {
    ...
    test('getModuleCacheEntry forwards priority to prerenderModule', async function (assert) {
      await dbAdapter.execute('DELETE FROM modules');

      await definitionLookup.getModuleCacheEntry(`${realmURL}person.gts`, {
        priority: 10,
      });
      assert.strictEqual(
        prerenderModuleCalls,
    ...
    test('getModuleCacheEntry caches non-card modules as no-card markers', async function (assert) {
      await dbAdapter.execute('DELETE FROM modules');
      forceEmptyDefinitions = true;

      let first = await definitionLookup.getModuleCacheEntry(
        `${realmURL}person.gts`,
      );
      assert.strictEqual(
        prerenderModuleCalls,
      ...
      let second = await definitionLookup.getModuleCacheEntry(
        `${realmURL}person.gts`,
      );
      assert.strictEqual(
        prerenderModuleCalls,
