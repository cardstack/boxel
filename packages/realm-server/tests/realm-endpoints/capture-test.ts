...
// The `_capture/` route's HTTP surface: realm-read auth, the
...
module(`realm-endpoints/${basename(import.meta.filename)}`, function () {
  module('GET _capture on a private realm', function (hooks) {
    let testRealm: Realm;
    ...
    test('HEAD is not admitted to the capture route', async function (assert) {
  ...
  module('GET _capture on a world-readable realm', function (hooks) {
    let request: SuperTest<Test>;
  ...
  // The GET dispatch claims the whole `_capture/` subtree, so a realm
  ...
  module('writes under _capture/ are refused', function (hooks) {
    let testRealm: Realm;
    let request: SuperTest<Test>;
    ...
    test('PUT, PATCH, and POST are refused as reserved', async function (assert) {
      for (let method of ['put', 'patch', 'post'] as const) {
        let response = await request[method]('/_capture/logo.png')
