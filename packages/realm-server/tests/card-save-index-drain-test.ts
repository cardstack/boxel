    ...
    (hooks) => {
      ...
      setupPermissionedRealm(hooks, {
        ...
        fileSystem: {
          ...
          'person-1.json': document({ firstName: 'Before', lastName: 'Save' }),
        ...
        QUnit.test(
          ...
          async (assert) => {
            ...
            try {
              ...
              assert.strictEqual(first.status, 200, 'first save acknowledged');
            ...
            const search = await fetch(`${realmURL}_search`, {
              ...
              body: JSON.stringify(
                searchEntryWireQueryFromQuery(
                  {
                    filter: {
                      on: { module: rri(`${realmURL}person`), name: 'Person' },
                      eq: { firstName: 'Two' },
                    },
                  },
                  { fields: ['item'] },
