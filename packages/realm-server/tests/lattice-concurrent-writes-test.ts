...
for (const enabled of [false, true]) {
  QUnit.module(`Lattice | concurrent writes | enabled=${enabled}`, (hooks) => {
          ...
          Array.from({ length: 6 }, (_, i) => [
            `Flapper/${i}.json`,
            {
              data: {
                ...
                meta: { adoptsFrom: { module: '../cards', name: 'Flapper' } },
              },
            },
          ]),
        ),
      ...
      async (assert) => {
        ...
        console.log(
          ...
          JSON.stringify({
            ...
            maxWaitingLocks,
            heartbeat,
            answered: outcomes.filter((r) => r.status === 'fulfilled').length,
          }),
        );
