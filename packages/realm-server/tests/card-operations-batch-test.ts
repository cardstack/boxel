...
module(basename(import.meta.filename), function () {
  ...
  module('card operations batch', function () {
    ...
    test('a patch that changes nothing stages the bytes already on disk', async function (assert) {
      ...
      assert.ok(
        results[0],
        'the entry still reports the version the file holds',
      );
    });
    ...
    test('a local id names one card', async function (assert) {
      ...
      assert.strictEqual(
        failed?.status,
        400,
        'a duplicate local id is refused',
      );
      assert.strictEqual(commits.length, 0, 'nothing is committed');
    });
    ...
    test('a link to a side-load in another realm says which realm holds it', async function (assert) {
      ...
      let namesTheRealm = Boolean(failure?.detail?.includes('another realm'));
      assert.true(
        namesTheRealm,
    ...
    test('a card the realm ignores has no removal to make', async function (assert) {
      ...
      let { core, commits } = stub({
        stored: {
          'hidden/Person/secret.json': cardFile(
            { firstName: 'Secret' },
            PERSON,
          ),
        },
    ...
    test('a card removed in the batch cannot be minted again at its path', async function (assert) {
      ...
      let namesTheEntries = Boolean(
        failure?.detail?.includes('which an earlier entry removes'),
      );
      assert.true(
        namesTheEntries,
    ...
    test('a template that keys into the actor names a member that does not exist', async function (assert) {
    ...
    test('the caller cannot be written into a relationship, however it is spelled', async function (assert) {
