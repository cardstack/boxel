  ...
  module('virtual-network', function () {
    test('will respond with real (not virtual) url when handler makes a redirect', async function (assert) {
      ...
      virtualNetwork.addURLMapping(
        new URL('http://test-base-realm/base/'),
      ...
      virtualNetwork.mount(async (_request: Request) => {
        ...
        return new Response(null, {
          status: 302,
          headers: {
            Location: 'http://test-base-realm/base/__boxel/assets/', // This virtual url should be converted to a real url so that the client can follow the redirect
          },
