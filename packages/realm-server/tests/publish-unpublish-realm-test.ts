...
module(basename(__filename), function () {
  module('publish and unpublish realm tests', function (hooks) {
    ...
    module('with a publishable source realm', function (hooks) {
      ...
      test('QUERY /_info on a root-mounted published realm bypasses server router and reaches the realm handler', async function (assert) {
        // Publish to the root of a host so /_info path conflicts with the
        // server-level /_info route
        let publishedRealmURL = 'http://testuser.localhost:4445/';
        ...
        assert.strictEqual(publishResponse.status, 201, 'publish succeeded');

        let publishedRealmHost = new URL(publishedRealmURL).host;

        // Without the host-mode router bypass fix, this request would match
        // the server-level router.all('/_info', ...) and fail because
        // multiRealmAuthorization expects a { realms: [...] } body.
        // With the fix, it skips the router and reaches the realm's own
        // _info handler via serveFromRealm.
        let infoResponse = await request
          .post('/_info')
          .set('X-HTTP-Method-Override', 'QUERY')
          .set('Accept', 'application/vnd.api+json')
        ...
        assert.strictEqual(
          infoResponse.status,
          200,
          'published realm /_info returns 200',
        );

        // Realm-level _info returns { data: { ... } } (single object)
        // Server-level _info returns { data: [...] } (array)
        assert.notOk(
          Array.isArray(infoResponse.body.data),
          'response is single-realm format, not multi-realm array',
        );
        assert.strictEqual(
          infoResponse.body.data.type,
          'realm-info',
          'response has realm-info type',
        );
        assert.ok(infoResponse.body.data.attributes, 'response has attributes');
      });
