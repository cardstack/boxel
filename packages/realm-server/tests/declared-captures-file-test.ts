...
// Declared captures on a FileDef family, captured on the URL's *file* row.
...
const FILEDEF_MISMATCH_SOURCE = `
      ...
      import { Component, type CaptureSpec } from "@cardstack/base/card-api";

      // Capture-only: referenced only from the declaration, rendered only by
      // the capture render route.
...
module(basename(import.meta.filename), function (hooks) {
  ...
  test("the prerender-html pass captures a file family's declared captures onto the file row", async function (assert) {
    ...
    let manifest = fileRow!.captures as CaptureManifest | null;
    ...
    let errors = (fileRow!.diagnostics as any)?.captureErrors;
    assert.strictEqual(
      errors,
      undefined,
      'no captureErrors diagnostics on a clean capture',
    );
  });

  test("the ?name= URL serves a file row's capture and the file-meta GET joins meta.captures", async function (assert) {
    ...
    let manifest = fileRow!.captures as CaptureManifest;

    let imageResponse = await realm.handle(
      new Request(`${testRealm}_capture/sample.mismatch?name=poster`),
    );
    assert.strictEqual(
      imageResponse!.status,
      200,
      "the named capture URL serves the file row's capture",
    );
    ...
    assert.deepEqual(
      ...
      {
        poster: {
          url: `${testRealm}_capture/sample.mismatch?name=poster`,
      ...
      'the file-meta GET joins the manifest into meta.captures',
    );
  });
  ...
  test("the file's own prerendered HTML embeds the durable URL via the declaration-derived render context", async function (assert) {
    ...
    assert.ok(
      embedded.includes(`_capture/sample.mismatch?name=poster`),
  ...
  test('a vector image captures only the thumb — the rendition slots live on RasterImageDef', async function (assert) {
    ...
    // the render context's declaration-derived meta.captures, so the
    // file's prerendered fitted HTML embeds the durable thumb URL.
    let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
    assert.ok(
      fitted.includes(`_capture/picture.svg?name=thumb`),
  ...
  test('the PDF family captures a first-page poster onto the file row', async function (assert) {
    ...
    let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
    assert.ok(
      fitted.includes(`_capture/doc.pdf?name=poster`),
  ...
  for (let { family, fixture, file } of [
    ...
    test(`the Office family captures a first-page poster onto the file row (${family})`, async function (assert) {
      ...
      let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
      assert.ok(
        fitted.includes(`_capture/${file}?name=poster`),
  ...
  test('an office file whose extraction yields no first unit captures the typed-placeholder poster', async function (assert) {
    ...
    let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
    assert.ok(
      fitted.includes(`_capture/empty.docx?name=poster`),
      `the fitted rendering carries the poster URL (got: ${fitted.slice(
        0,
        500,
      )})`,
  ...
  test('a corrupt PDF captures no poster and the fitted cell keeps the placeholder', async function (assert) {
    ...
    // its readiness signal for `data-capture-failed`, so the slot's
    ...
    let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
    assert.ok(
      fitted.includes(`_capture/broken.pdf?name=poster`),
  ...
  test("an alias-addressed request serves from the matched row's own ledger spelling", async function (assert) {
    ...
    let ledgerSourceURL = captureLedgerSourceURL(
      `${testRealm}note.json`,
      'file',
    );
    assert.strictEqual(
    ...
    await testDbAdapter.execute(
      `update prerendered_html set captures = $1 where url = $2 and type = 'file'`,
    ...
    let aliasResponse = await realm.handle(
      new Request(`${testRealm}_capture/note?name=poster`),
    );
    assert.strictEqual(
    ...
    let extensionResponse = await realm.handle(
      new Request(`${testRealm}_capture/note.json?name=poster`),
    );
    assert.strictEqual(
  ...
  test('the video family captures a poster frame onto the file row', async function (assert) {
    ...
    let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
    assert.ok(
      fitted.includes(`_capture/clip.webm?name=poster`),
      `the fitted rendering carries the poster URL (got: ${fitted.slice(0, 500)})`,
    );
  });
  ...
  test('an undecodable video captures no poster', async function (assert) {
    ...
    // `data-capture-failed`, so the slot fails immediately. No manifest
  ...
  test('the 3D family captures a rendered still onto the file row', async function (assert) {
    ...
    let fitted = JSON.stringify(fileRow!.fitted_html ?? {});
    assert.ok(
      fitted.includes(`_capture/part.glb?name=poster`),
  ...
  test('an unchanged file carries its capture forward; a content change recaptures', async function (assert) {
    ...
    let firstManifest = firstRow!.captures as CaptureManifest;
    ...
    let secondManifest = secondRow!.captures as CaptureManifest;
    assert.ok(
    ...
    let thirdManifest = thirdRow!.captures as CaptureManifest;
