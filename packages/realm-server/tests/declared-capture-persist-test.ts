...
import {
  declaredCaptureTimingsMs,
  persistDeclaredCaptures,
...
function captureResult(
  overrides: Partial<DeclaredCaptureResult> = {},
): DeclaredCaptureResult {
  return {
    name: 'card',
...
module(basename(import.meta.filename), function (hooks) {
  ...
  function persist(args: {
    result: Parameters<typeof persistDeclaredCaptures>[0]['result'];
    priorManifest?: CaptureManifest | null;
    priorCaptureErrors?: Parameters<
      typeof persistDeclaredCaptures
    >[0]['priorCaptureErrors'];
    priorCaptureFailureRenders?: number | null;
    contentHash?: string;
    sourceGeneration?: number;
  }) {
    return persistDeclaredCaptures({
      result: args.result,
      priorManifest: args.priorManifest ?? null,
      priorCaptureErrors: args.priorCaptureErrors ?? null,
  ...
  test('a prior failure recorded without a run count reads as a run of one', async function (assert) {
    let { errors, captureFailureRenders } = await persist({
      ...
      priorCaptureErrors: [{ name: 'hero', message: 'render never painted' }],
    });
  ...
  module('declaredCaptureTimingsMs', function () {
    ...
    test('is undefined when nothing recorded a time', function (assert) {
      assert.strictEqual(declaredCaptureTimingsMs(undefined), undefined);
      assert.strictEqual(declaredCaptureTimingsMs({ entries: [] }), undefined);
    });
  });

  // The engine-side carry-forward decision (`captureDeclared`
