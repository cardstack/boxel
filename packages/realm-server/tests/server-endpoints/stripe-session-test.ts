  ...
  module('Realm Server Endpoints (not specific to one realm)', function (hooks) {
    module('stripe session handler', function (hooks) {
      ...
      test('creates checkout session for AI tokens when user has no Stripe customer', async function (assert) {
        ...
        assert.strictEqual(sessionArgs.mode, 'payment', 'session mode is payment');
        assert.strictEqual(
      ...
      test('creates checkout session for subscription when user has no active subscription', async function (assert) {
        ...
        assert.ok(listSubscriptionsStub.calledOnce, 'subscriptions.list was called');
        assert.deepEqual(
        ...
        assert.ok(retrieveProductStub.calledOnce, 'products.retrieve was called');
        assert.strictEqual(
        ...
        // Verify Stripe customer was NOT created (since user already has one)
        assert.ok(createCustomerStub.notCalled, 'customer.create was not called');
      });
      ...
      test('creates billing portal session when user already has active subscription', async function (assert) {
        ...
        assert.ok(retrieveProductStub.notCalled, 'products.retrieve was not called');
        assert.ok(
