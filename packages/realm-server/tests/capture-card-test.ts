...
module(basename(import.meta.filename), function () {
  module('/_capture-card endpoint', function () {
    ...
    test('rejects target combined with clip', async function (assert) {
      // A `target` is an element-handle capture; it honors no clip.
      await expectCaptureSpecRejected(
        assert,
        {
    ...
    test('rejects fitted without an envelope', async function (assert) {
      let { response, published } = await postCapture('fitted', undefined);
      assert.strictEqual(
        response.status,
        400,
    ...
    test('rejects atom as an unsupported capture format', async function (assert) {
      ...
      let { response, published } = await postCapture('atom', {
    ...
    test('accepts fitted with an envelope and forwards it', async function (assert) {
      let { response, published } = await postCapture('fitted', {
    ...
    test('rejects an envelope on isolated format', async function (assert) {
      let { response, published } = await postCapture('isolated', {
        envelope: { width: 250, height: 275 },
      });
      assert.strictEqual(
        response.status,
  ...
  module('/_capture-card persistence', function (hooks) {
    ...
    function makePersistQueue(
      ...
      ready: CapturePrerenderResponse = READY,
    ): {
      queue: QueuePublisher;
      published: Array<QueuePublishArgs<unknown>>;
    } {
    ...
    test('a capture of an indexed card enqueues with the DSL-matching persist identity and returns a served URL', async function (assert) {
      ...
      assert.deepEqual(attrs.captures, [
        {
          name: null,
          url: `${REALM_URL}_capture/Person/fadhlan`,
          width: 800,
          height: 600,
    ...
    test('a non-default format shows up in the served URL', async function (assert) {
      ...
      assert.strictEqual(
        ...
        `${REALM_URL}_capture/Person/fadhlan?format=embedded`,
      );
    });
    ...
    test('a custom captureSpec persists under its own capture identity and returns its served URL', async function (assert) {
      ...
      assert.strictEqual(
        response.body.data.attributes.captures[0].url,
        `${REALM_URL}_capture/Person/fadhlan?viewport=1280x800`,
    ...
    test('a custom captureSpec answers from its own ledger entry with zero render work', async function (assert) {
      ...
      assert.strictEqual(
        capture.url,
        `${REALM_URL}_capture/Person/fadhlan?viewport=1280x800&dsf=2`,
      );
      assert.strictEqual(
    ...
    test('a wait that outruns the budget answers 503 + Retry-After', async function (assert) {
      ...
      let response = await post(persistApp(queue, { captureSyncWaitMs: 50 }), {
        realmURL: REALM_URL,
        cardId: CARD_ID,
        format: 'isolated',
      });
    ...
    test('a pdf capture persists under its own identity and returns its ?type=pdf URL', async function (assert) {
      ...
      assert.strictEqual(
        response.body.data.attributes.captures[0].url,
        `${REALM_URL}_capture/Person/fadhlan?type=pdf`,
    ...
    test('a pdf capture reports its page count where a raster one reports dimensions', async function (assert) {
      ...
      assert.deepEqual(
        ...
        [
          {
            name: null,
            url: `${REALM_URL}_capture/Person/fadhlan?type=pdf`,
            width: null,
            height: null,
            deviceScaleFactor: 1,
            pageCount: 3,
    ...
    test('a target capture is capture-only: no persist identity, no served URL', async function (assert) {
      ...
      // `_capture/` URL would then serve the crop.
      await seedInstanceRow();
      let { queue, published } = makePersistQueue('ready');
    ...
    test('an errored instance captures without persisting', async function (assert) {
      ...
      // An errored instance can never serve on the GET `_capture/` route
  ...
  module('capture queue twin estimate', function (hooks) {
    let dbAdapter: PgAdapter;
    ...
    test('hasTwin requires the runAs the caller would render under', async function (assert) {
      ...
      let sameIdentity = await estimateCaptureQueueWait(
        dbAdapter,
        concurrencyGroup,
      ...
      // renders anyway.
      let differentRunAs = await estimateCaptureQueueWait(
        dbAdapter,
        concurrencyGroup,
