 ...
 * `_capture/` DSL resolves, so a capture published here serves on that
 * route immediately, even on realms whose `allowArbitraryCaptures` gate
 ...
 * — the prerender server's capture route and the GET `_capture/` URL
 ...
 * GET `_capture/` URL (`?viewport=…&dsf=…`) exactly like a format-only
 ...
 * `CAPTURE_MAX_BATCH_ENTRIES`). Each entry's overrides win over the singular
 ...
 * `CAPTURE_MAX_BATCH_ENTRIES` for the cost contract). The response's `captures`
...
export default function handleCaptureCard({
  ...
  matrixClient,
  mediaCacheAdapter,
  captureSyncWaitMs = CAPTURE_SYNC_WAIT_BUDGET_MS,
  ...
  return async function (ctxt: Koa.Context, _next: Koa.Next) {
    ...
    if (!isOnDemandCaptureFormat(format)) {
      return sendResponseForBadRequest(
        ctxt,
        `format must be one of: ${ON_DEMAND_CAPTURE_FORMATS.map((f) => `"${f}"`).join(', ')}`,
      );
    }
    ...
    let spec: CaptureIdentity | undefined =
    ...
    try {
      ...
      let result: CapturePrerenderResponse | typeof timedOut;
      try {
