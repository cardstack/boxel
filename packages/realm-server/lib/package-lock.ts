...
//
//     "imports": {
//       "experiments/greeter":  "/_packages/experiments/greeter@2.2.0/index.js",
//       "experiments/greeter/": "/_packages/experiments/greeter@2.2.0/"
//     }
//
...
//
// WHY THE PINS ARE ORIGIN-RELATIVE. `/_packages/…`, not an absolute URL. What
// a seal owes its consumer is the VERSION; the host is not the package's
// business. A pack pinned to `https://realm-a.example/_packages/…` is welded
// to one server forever and cannot be mirrored, which is precisely what a
// content-addressed store exists to allow. Origin-relative says "the package
 ...
 */
export function packageBaseURL(name: string, version: string): string {
...
export async function lockDependencies(options: {
  ...
  let { storeDir, dependencies } = options;
  ...
  for (let [key, spec] of Object.entries(dependencies ?? {})) {
    ...
    let base = packageBaseURL(key, version);
