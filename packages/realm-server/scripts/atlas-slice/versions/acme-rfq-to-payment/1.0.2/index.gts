...
//
// ─── PASS 3 (1.0.2): two things 1.0.1 got wrong ─────────────────────────────
//
// FORMAT. `<@fields.openCase />` renders a linked card FITTED, not embedded —
// so ledgerworks' new embedded row never appeared and the case showed as a
// bare title chip anyway. The whole point of taking 1.1.0 was lost to a
// default. Asking for the format explicitly is the fix, and it is the caller's
// job: fitted is for a card in a grid, embedded is for a card in a sentence,
// and only the parent knows which of those it is building.
//
// THE FOOTER WAS LYING. It listed the resolved stack as hardcoded text, so the
// moment 1.0.1 re-sealed onto billing-kit@1.1.0 the card cheerfully kept
// claiming 1.0.0. A hand-maintained copy of a fact the seal already holds is a
// second source of truth, and it went stale on the very next release — which
// is about as fast as that mistake can possibly be caught.
//
      ...
      <section class='run'>
        ...
        <div class='pair'>
          <article class='side'>
            ...
            <div class='clip'>
              {{! EMBEDDED, explicitly. The default for a linked card is
                  FITTED, which draws a title chip and no controls — so the
                  kit's row would never appear no matter which Version was
                  sealed. }}
        ...
        <footer class='stack'>
          {{! Still hand-written, and still the wrong shape — it wants to be
              ...
              because the honest version of this card is one that cannot go
              stale, and that needs a way to ask the pack manifest at render
              time. Noted as owed. }}
          <p class='stack-title'>Resolved stack</p>
          <ol>
            <li>acme/rfq-to-payment@1.0.2 <em>(this card)</em></li>
            <li>ledgerworks/billing-kit@1.1.0</li>
