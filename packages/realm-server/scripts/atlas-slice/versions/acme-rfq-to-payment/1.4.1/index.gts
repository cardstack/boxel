...
// acme/rfq-to-payment 1.3.0 — house style on the payment run.
//
// The resolved-stack table is the point of this card: it reads its own pack
// manifest and prints what it actually resolved to. So it is the table that
// gets the pass — rules as inset shadows, headers in the mono eyebrow voice,
// and the viewport tag tightened to match.
//
// Everything else is unchanged, including the part that matters most: this run
// still renders two majors of openkit/controls side by side, one clipped by its
// container and one escaping to <body>, neither degraded. That demonstration
// depends on the pins, and the pins are not what this pass touches.
    ...
    <template>
      <article class='run'>
        ...
        <section class='compare'>
          ...
          <article class='panel'>
            <header class='panel-head'>
              <span class='pill is-new'>controls@1.1.0</span>
      ...
      <style scoped>
        ...
        /* Law 1 — depth is one property: the table's rules are inset
           shadows, matching the viewport's hairline a few rules above. */
        .stack th {
          padding: 0 var(--ac-sp-sm) var(--ac-sp-xs) 0;
          box-shadow: inset 0 -1px 0 var(--line-strong, var(--ac-line));
          font-family: var(--ac-mono);
          color: var(--ac-ink-2);
          font-size: 10.5px;
          font-weight: 500;
        ...
        .stack td {
          ...
          box-shadow: inset 0 -1px 0 var(--ac-line);
