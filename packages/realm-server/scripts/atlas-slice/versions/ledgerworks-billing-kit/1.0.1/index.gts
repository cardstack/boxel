...
//
// ─── 1.0.1: A BACKPORT ONTO THE MAINTENANCE LINE ────────────────────────────
//
// Published after 1.3.0, for the customers still qualified against the 1.0
// line, and carrying one fix and nothing else. Every dependency is declared `~`
// rather than `^` — including `northwind/records`, which resolves to that
// package's own 1.0.1 backport rather than to its 1.2.0. A maintenance release
// that quietly drags its upstreams forward has not maintained anything; it has
// shipped the new version under an old number.
//
// THE FIX. A case rendered as "Untitled Collection Case" wherever it was
// named, and a case is not a document with a title of its own — it is
// identified by the invoice it is chasing. So the title is computed ACROSS THE
// LINK, which is the honest answer and also the reason this needed thought
// rather than a one-liner: the link may not be loaded, and a title getter is
// called in every list, so it has to degrade to something sensible rather than
// throw or block.
//
// Still no fitted format on this line, and still a clipped 0.2.0 Select. Those
// are what 1.0 was. This only stops it lying about its own name.

// RE-EXPORTED, so a consumer takes the SAME `Invoice` class this kit's fields
// link to. A consumer that imports `northwind/records` itself resolves through
// its own map, which may name a different Version — and two Versions of a card
// type are two classes, so `linksTo` then fails an `instanceof` check with the
// famously unhelpful "tried set Invoice as field 'invoice' but it is not an
// instance of Invoice". A kit that links to a type owes its consumers a way to
// get that exact type.
...
export class CollectionCase extends CardDef {
  ...
  // A case is not a document with a title of its own — it is identified by the
  // invoice it is chasing, so the title is computed ACROSS THE LINK.
  //
  // WHICH NEEDS CARE, and is why this is more than a one-liner. A title getter
  // runs in every list and every header, and the link may not be loaded yet;
  // optional chaining is what keeps that a plain fallback rather than a throw
  // in a render. And it degrades in two steps rather than one, because
  // "Collection case" is still useful to a reader while "Untitled Collection
  // Case" never was.
