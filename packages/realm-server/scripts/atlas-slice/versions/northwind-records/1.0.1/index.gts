...
//
// Published AFTER 1.2.0, and deliberately not on it. Consumers still qualified
// against the 1.0 line — `apps/legacy-collections/` in the atlas realm is one —
// asked for one fix and no features, which is the ordinary shape of a supported
// old release and the reason `~1.0.0` exists as a range at all. Its
// dependencies are declared `~` for the same reason: a maintenance patch that
// silently drags its upstreams forward is not a maintenance patch.
//
// THE FIX. Every invoice rendered as "Untitled Invoice" — in the tab title, in
// the stack header, and worst of all inside any card that LINKS to one, where a
// bare unnamed chip is the entire visible content. The number was in the record
// the whole time; nothing was asking for it. `CardDef.cardTitle` computes from
// `cardInfo.name` and falls back to `Untitled ${displayName}`, so a card that
// has a natural name has to say so.
//
// WHY THIS IS A PATCH AND NOT A FEATURE. It adds no capability and changes no
// shape: it corrects a card that was misreporting its own identity. The 1.0
// line still has no fitted or embedded format, so a linked invoice here is
// still a chip and still visibly poorer than 1.2.0's — which is the point of
// having both apps in one realm. The difference is now DATED rather than
// BROKEN, and those two read very differently to somebody being shown this.
...
export class Invoice extends CardDef {
  ...
  // A CARD THAT KNOWS ITS OWN NAME. Overriding the base computed rather than
  // asking authors to type the number a second time into `cardInfo.name`: the
  // invoice number IS the invoice's name, and a copy of it that can drift from
  // the field beside it is worse than no copy. The base fallback is kept for
