...
if [ -z "${SKIP_BOXEL_HOMEPAGE:-}" ]; then
  WAIT_URLS="$WAIT_URLS|${BOXEL_HOMEPAGE_REALM}${READY_PATH}"
fi
if [ -z "${SKIP_EXPERIMENTS:-}" ]; then
  WAIT_URLS="$WAIT_URLS|${EXPERIMENTS_REALM}${READY_PATH}"
fi
...
WAIT_URLS="$WAIT_URLS|$SYNAPSE_URL|$SMTP_4_DEV_URL|$ICONS_URL"
