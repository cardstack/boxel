...
WAIT_ON_TIMEOUT=900000 \
  ...
  start-server-and-test \
    'run-p -ln --aggregate-output start:pg start:prerender-dev start:prerender-manager-dev start:matrix start:smtp start:worker-development start:development' \
    ...
    'run-p -ln --aggregate-output start:worker-test start:test-realms' \
