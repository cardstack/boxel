...
WAIT_ON_TIMEOUT=900000 \
  ...
  start-server-and-test \
    ...
    'http-get://localhost:4201/base/_readiness-check?acceptHeader=application%2Fvnd.api%2Fjson|http://localhost:8008|http://localhost:5001' \
    'run-p start:worker-test start:test-realms' \
    'http-get://localhost:4202/node-test/_readiness-check?acceptHeader=application%2Fvnd.api%2Fjson' \
    'wait'
