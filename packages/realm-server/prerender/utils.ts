...
export interface ScreenshotCapture {
  ...
  // the image/font paint wait, and the CDP screenshot(s). One record per
  // render — nav/settle/imagePaint happen once for the whole batch, and the
  // capture stage is the whole capture loop. Together these account for nearly
  // all of the render wall; the remainder is the terminal error probe and
  // dimension reads.
...
export async function captureScreenshot(
  ...
  try {
    ...
    // The CDP screenshot(s) as one stage: for a singular capture this is the
    // lone page.screenshot inside captureOneEntry; a batch adds each entry's
    // viewport switch + screenshot. One stepTimings record rides on the whole
    // render, so the capture stage is the whole loop.
    stepStart = Date.now();
