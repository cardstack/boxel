...
describe('prerender-manager-test.ts', function () {
  describe('Prerender manager', function () {
    ...
    it('health includes active servers with affinities and last used times', async function () {
      ...
      expect(healthResponse.headers['content-type']).toBe(
        'application/vnd.api+json',
      );
      ...
      expect(assignedServerData.attributes.affinities[0].affinityType).toBe(
        'realm',
      );
      expect(assignedServerData.attributes.affinities[0].affinityValue).toBe(
        realm,
      );
      expect(assignedServerData.attributes.affinities[0].key).toBe(
    ...
    it('proxies card prerender requests', async function () {
      ...
      expect(proxyResponse.status).toBe(201);
      expect(proxyResponse.headers['x-boxel-prerender-target']).toBe(
        serverUrlA,
      );
    ...
    it('heartbeat: url required; heartbeat updates warmed affinities and status', async function () {
      ...
      expect(registrationResponse.headers['x-prerender-server-id']).toBe(
        serverUrlA,
      );
    ...
    it('proxy: sticky routing with multiplex=1', async function () {
      ...
      expect(secondProxyResponse.headers['x-boxel-prerender-target']).toBe(
        firstTarget,
      );
    });
    ...
    it('unregister removes server from routing', async function () {
      ...
      let proxyResponse = await request
        ...
        .send(realm2RequestBody);
      expect(proxyResponse.status).toBe(201);
      expect(proxyResponse.headers['x-boxel-prerender-target']).not.toBe(
        serverUrlA,
      );
    });
