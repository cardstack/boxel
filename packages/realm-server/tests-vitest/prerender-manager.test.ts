...
describe("prerender-manager-test.ts", function () {
    describe('Prerender manager', function () {
        ...
        it('health includes active servers with affinities and last used times', async function () {
            ...
            expect(Array.isArray(assignedServerData.attributes.affinities)).toBeTruthy();
            expect(assignedServerData.attributes.affinities.length).toBe(1);
            expect(assignedServerData.attributes.affinities[0].affinityType).toBe('realm');
            expect(assignedServerData.attributes.affinities[0].affinityValue).toBe(realm);
            expect(assignedServerData.attributes.affinities[0].key).toBe(realmAffinityKey(realm));
            expect(assignedServerData.attributes.affinities[0].lastUsed).toBeTruthy();
            expect(assignedServerData.attributes.status).toBe('active');
            expect(Array.isArray(assignedServerData.attributes.warmedAffinities)).toBeTruthy();
            ...
            expect(otherServerData.attributes.affinities.length).toBe(0);
        });
        it('proxies card prerender requests', async function () {
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
                data: {
        ...
        it('proxies module prerender requests', async function () {
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('proxies run-command requests', async function () {
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            // Register a single server
            await request.post('/prerender-servers').send({
        ...
        it('heartbeat: url required; heartbeat updates warmed affinities and status', async function () {
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('proxy: sticky routing with multiplex=1', async function () {
        ...
        it('proxy: rotation with multiplex>1, capacity and pressure', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            // register both
        ...
        it('affinity disposal removes server from affinity mapping', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
            ...
            let missingUrlDisposalResponse = await request.delete(`/prerender-servers/affinities/${encodeURIComponent(affinityKey)}`);
            ...
            let disposalResponse = await request
                .delete(`/prerender-servers/affinities/${encodeURIComponent(affinityKey)}`)
        ...
        it('affinity disposal selects least recently used idle server when multiplex=1', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
        ...
        it('unregister removes server from routing', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
        ...
        it('unreachable server is removed by health sweep', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
        ...
        it('stale heartbeat removes server from routing', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
        ...
        it('manager retries another server when one is draining', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('manager prefers warmed affinity when available', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('does not treat warmed user affinity as warmed realm affinity for the same value', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('run-command prefers user-warmed server', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('pressure mode skips unusable LRU server and falls back to healthy', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app, registry, chooseServerForAffinity } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
                data: {
                    type: 'prerender-server',
            ...
            expect(registry.servers.get(serverUrlA!)?.activeAffinities.has(lruAffinityKey)).toBe(false);
        ...
        it('cleanup keeps activeAffinities in sync so capacity is restored after draining', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            ...
            expect(registry.servers.get(serverUrlA!)?.activeAffinities.size).toBe(1);
            ...
            expect(registry.servers.get(serverUrlA!)?.activeAffinities.size).toBe(0);
            ...
            expect(registry.servers
                .get(serverUrlA!)
                ?.activeAffinities.has(realmAffinityKey(realm3)) as boolean).toBe(true);
        });
        it('pressure mode assignment updates activeAffinities for capacity accounting', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app, registry, chooseServerForAffinity } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
            ...
            expect(registry.servers
                .get(serverUrlA!)
                ?.activeAffinities.has(realmAffinityKey(newRealm)) as boolean).toBe(true);
        });
        it('returns draining response if all targets are draining', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('returns draining immediately when manager is draining (no proxy)', async function () {
            let draining = true;
        ...
        it('returns draining when manager starts draining during an in-flight proxy', async function () {
            let draining = false;
            let { app } = buildPrerenderManagerApp({ isDraining: () => draining });
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('recovers when a prerender server disappears without draining', async function () {
            let { app, registry } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
                data: {
        ...
        it('retries another server when the first returns 500', async function () {
            let { app, registry } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
                data: {
        ...
        it('maintenance reset clears realm assignments', async function () {
            let { app, registry } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
                data: {
            ...
            expect(registry.servers
                .get(serverUrlA!)
                ?.activeAffinities.has(affinityKey) as boolean).toBe(true);
            let resetRes = await request.post('/prerender-maintenance/reset');
            expect(resetRes.status).toBe(204);
            expect(registry.servers
                .get(serverUrlA!)
                ?.activeAffinities.has(affinityKey) as boolean).toBe(false);
            expect(registry.affinities.has(affinityKey)).toBe(false);
        });
        it('pressure mode evicts LRU realm when all servers at capacity', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app, registry } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
            ...
            expect(registry.affinities.has(realmAffinityKey(realm1))).toBe(false);
            expect(registry.servers
                .get(serverUrlA!)
                ?.activeAffinities.has(realmAffinityKey(realm2)) as boolean).toBe(true);
        });
        it('heartbeat clears stale active affinities when warmedAffinities are empty', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app, registry, chooseServerForAffinity } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
            ...
            expect(registry.servers.get(serverUrlA!)?.activeAffinities.size).toBe(0);
            expect(registry.affinities.has(staleAffinityKey)).toBe(false);
            let newRealm = 'https://realm.example/newafterclear';
            let target = chooseServerForAffinity('realm', newRealm);
            expect(target).toBe(serverUrlA);
        });
        it('returns 503 when no prerender servers are registered', async function () {
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            let res = await request
                .post('/prerender-card')
        ...
        it('waits for discovery when registry empty before returning 503', async function () {
        ...
        it('returns draining immediately when manager is draining', async function () {
            let draining = true;
            let { app } = buildPrerenderManagerApp({
                isDraining: () => draining,
            });
...
function makeCommandBody(realm: string, command: string, runAs = '@user:localhost') {
    let auth = makeAuth(realm);
    return {
        data: {
