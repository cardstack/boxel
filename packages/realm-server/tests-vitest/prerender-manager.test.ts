...
describe("prerender-manager-test.ts", function () {
    describe('Prerender manager', function () {
        ...
        it('health includes active servers with realms and last used times', async function () {
        ...
        it('heartbeat: url required; heartbeat updates warmed realms and status', async function () {
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('realm disposal removes server from realm mapping', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
        ...
        it('realm disposal selects least recently used idle server when multiplex=1', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
        ...
        it('manager prefers warmed realm when available', async function () {
            process.env.PRERENDER_MULTIPLEX = '2';
            let { app } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
        ...
        it('cleanup keeps activeRealms in sync so capacity is restored after draining', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
        ...
        it('pressure mode assignment updates activeRealms for capacity accounting', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app, registry, chooseServerForRealm } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
        ...
        it('heartbeat clears stale active realms when warmedRealms are empty', async function () {
            process.env.PRERENDER_MULTIPLEX = '1';
            let { app, registry, chooseServerForRealm } = buildPrerenderManagerApp();
            let request: SuperTest<Test> = supertest(app.callback());
            await request.post('/prerender-servers').send({
...
function makeMockPrerender(): {
    ...
    server: Server;
    stop: () => Promise<void>;
    setResponder: (responder: (ctxt: Koa.Context, body: any, type: 'card' | 'module') => Promise<void> | void) => void;
} {
    let app = new Koa();
    ...
    let responder: (ctxt: Koa.Context, body: any, type: 'card' | 'module') => Promise<void> | void = defaultResponder;
    ...
    function defaultResponder(ctxt: Koa.Context, body: any, type: 'card' | 'module') {
        ctxt.status = 201;
        ctxt.set('Content-Type', 'application/vnd.api+json');
        if (type === 'card') {
            ctxt.body = JSON.stringify({
