...
describe("prerender-proxy-test.ts", function () {
    describe('prerender proxy', function () {
        ...
        it('proxies prerender requests to the configured prerenderer', async function () {
            ...
            router.post('/_prerender-card', jwtMiddleware(realmSecretSeed), handlePrerenderProxy({
                kind: 'card',
                prerenderer,
                dbAdapter,
                createPrerenderAuth,
            ...
            let token = createJWT({ user: '@someone:localhost', sessionRoom: '!room:localhost' }, realmSecretSeed);
        ...
        it('returns an error when no upstream is configured', async function () {
            ...
            let res = await supertest(app.callback())
                ...
                .expect(500);
            expect(res.text.includes('Prerender proxy is not configured')).toBeTruthy();
        });
        ...
        it('proxies to prerender server card and module endpoints', async function () {
            ...
            router.post('/_prerender-module', jwtMiddleware(realmSecretSeed), handlePrerenderProxy({
                kind: 'module',
                prerenderer,
                dbAdapter,
                createPrerenderAuth,
            ...
            expect(renderCalls.map(({ kind, args }) => {
