...
describe("queue-test.ts", function () {
    describe('queue', function () {
        ...
        test('it can run a job', async function () {
            await runSharedTest(queueTests, assert, { runner, publisher });
        });
        test(`a job can throw an exception`, async function () {
            await runSharedTest(queueTests, assert, { runner, publisher });
        });
        test('jobs are processed serially within a particular queue', async function () {
            await runSharedTest(queueTests, assert, { runner, publisher });
        });
        test('worker stops waiting for job after its been running longer than max time-out', async function () {
            let events: string[] = [];
            let runs = 0;
            let logJob = async () => {
                let me = runs;
        ...
        describe('multiple queue clients', function () {
            ...
            test('jobs in different concurrency groups can run in parallel', async function () {
                let events: string[] = [];
            ...
            test('jobs are processed serially within a particular queue across different queue clients', async function () {
                let events: string[] = [];
                let logJob = async (jobNum: number) => {
                    events.push(`job${jobNum} start`);
                    await new Promise((r) => setTimeout(r, 500));
            ...
            test('job can timeout; timed out job is picked up by another worker', async function () {
                let events: string[] = [];
                let runs = 0;
                let logJob = async () => {
                    let me = runs;
    ...
    describe('queue - high priority worker', function () {
        ...
        test('worker can be set to only process jobs greater or equal to a particular priority', async function () {
            let events: string[] = [];
    ...
    describe('queue - high priority worker and all priority worker', function () {
        ...
        test('concurrency group enforces concurrency against running jobs', async function () {
