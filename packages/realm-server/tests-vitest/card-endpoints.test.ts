...
import type { Realm, Relationship, ResourceID, } from '@cardstack/runtime-common';
import { baseRealm, isSingleCardDocument, type LooseSingleCardDocument, type SingleCardDocument, } from '@cardstack/runtime-common';
import { parse } from 'qs';
import type { Query } from '@cardstack/runtime-common/query';
import { setupPermissionedRealmCached, setupPermissionedRealmsCached, setupMatrixRoom, closeServer, testRealmInfo, cleanWhiteSpace, createJWT, testRealmServerMatrixUserId, cardInfo, type RealmRequest, withRealmPath, } from './helpers';
...
function makeMinimalPng(): Uint8Array {
    ...
    let totalLength = signature.length + ihdrChunk.length + idatChunk.length + iendChunk.length;
        ...
        describe('card GET request', function () {
            ...
            describe('public readable realm', function () {
                ...
                it('serves a card error request without last known good state', async function () {
                    ...
                    expect(errorBody.meta.stack.includes('at Realm.getSourceOrRedirect')).toBeTruthy();
                    ...
                    expect(errorBody.message).toBe(`missing file ${testRealmHref}does-not-exist.json`);
                    ...
                    if (errorBody.meta.scopedCssUrls.length > 0) {
                        expect(errorBody.meta.scopedCssUrls.every((scopedCssUrl: string) => scopedCssUrl.endsWith('.glimmer-scoped.css'))).toBeTruthy();
                    }
                    else {
                ...
                it('includes FileDef resources for file links in included payload', async function () {
                    ...
                    await realm.writeMany(new Map<string, string>([
                        [
                            'gallery.gts',
                            `
              ...
              `,
                    ...
                    await realm.writeMany(new Map<string, Uint8Array>([
                        ['hero.png', pngBytes],
                        ['first.png', pngBytes],
                        ['second.png', pngBytes],
                    ]));
                    ...
                    let hero = included.find((resource) => resource.id === `${testRealmHref}hero.png`);
                    let first = included.find((resource) => resource.id === `${testRealmHref}first.png`);
                    let second = included.find((resource) => resource.id === `${testRealmHref}second.png`);
                    ...
                    expect((doc.data.relationships?.['attachments.0'] as Relationship)?.data).toEqual({
                        type: 'file-meta',
                        id: `${testRealmHref}first.png`,
                    });
                    expect((doc.data.relationships?.['attachments.1'] as Relationship)?.data).toEqual({
                        type: 'file-meta',
             ...
             AND type = 'instance'`);
                ...
                it('stale linksTo(FileDef subclass) relationship data.type of card is corrected to file-meta', async function () {
                    let { testRealm: realm, request, dbAdapter } = getRealmSetup();
                    await realm.writeMany(new Map<string, string | Uint8Array>([
                        [
                            'skill-card.gts',
                            `
                  import { CardDef, field, contains, linksTo } from "https://cardstack.com/base/card-api";
                ...
                `,
                    ...
                    let included = doc.included ?? [];
                    let linkedFile = included.find((resource) => resource.id === `${testRealmHref}instructions.md`);
             ...
             AND type = 'instance'`);
              ...
              `,
                    ...
                    expect(favorite.data).toEqual({ type: 'card', id: `${testRealmHref}person-1` });
                    ...
                    expect(matchesRelationship.data).toEqual({ type: 'card', id: `${testRealmHref}person-1` });
                    expect(Array.isArray(doc.included)).toBeTruthy();
                    expect(doc.included.some((resource: any) => resource.id === `${testRealmHref}person-1`)).toBeTruthy();
                });
              ...
              `,
                    ...
                    expect(response.status).toBe(200);
                    let doc = response.body;
                    expect(doc.data.relationships['info.queries.favorite']?.data).toEqual({ type: 'card', id: `${testRealmHref}person-1` });
                    expect(doc.data.relationships['info.queries.favorite']?.links?.self).toBe(`./person-1`);
                    expect(doc.data.relationships['info.queries.matches.0']?.data).toEqual({ type: 'card', id: `${testRealmHref}person-1` });
                    ...
                    expect(deepDoc.data.relationships['details.inner.queries.favorite']?.data).toEqual({ type: 'card', id: `${testRealmHref}person-1` });
                    expect(deepDoc.data.relationships['details.inner.queries.matches.0']?.data).toEqual({ type: 'card', id: `${testRealmHref}person-1` });
                });
            });
            ...
            describe('public writable realm', function () {
                ...
                it('serves a card error request with last known good state', async function () {
                    ...
                    expect(response.get('X-boxel-realm-public-readable')).toBe('true');
                    let errorBody = json.errors[0];
                    let lastKnownGoodHtml = cleanWhiteSpace(errorBody.meta.lastKnownGoodHtml);
                    expect(errorBody.meta.stack.includes('at Realm.getSourceOrRedirect')).toBeTruthy();
                    expect(errorBody.status).toBe(404);
                    expect(errorBody.title).toBe('Link Not Found');
                    expect(errorBody.message).toBe(`missing file ${testRealmHref}does-not-exist.json`);
                    expect(lastKnownGoodHtml.includes('Hassan has a friend')).toBeTruthy();
                    expect(lastKnownGoodHtml.includes('Jade')).toBeTruthy();
                    let scopedCssUrls = errorBody.meta.scopedCssUrls;
                    assertScopedCssUrlsContain(assert, scopedCssUrls, cardDefModuleDependencies);
                });
            });
            describe('permissioned realm', function () {
                ...
                it('200 when server user assumes user that has read permission', async function () {
                    let response = await request
                        ...
                        .set('Authorization', `Bearer ${createJWT(testRealm, testRealmServerMatrixUserId, ['assume-user'])}`);
                    expect(response.status).toBe(200);
                    expect(response.get('X-boxel-realm-public-readable')).toBe(undefined);
                });
            ...
            describe('public writable realm', function () {
                ...
                it('serves the request', async function () {
                    ...
                    let response = await request
                        ...
                        .set('Accept', 'application/vnd.card+json');
                    let incrementalEventContent = await expectIncrementalIndexEvent(testRealmHref, realmEventTimestampStart, {
                        assert,
                        getMessagesSince,
                        realm: testRealmHref,
                    });
                    ...
                    expect(json.data.meta.lastModified).toBeTruthy();
                    let cardFile = join(dir.name, 'realm_server_1', 'test', 'CardDef', `${id}.json`);
                ...
                it('creates card instances when it encounters "lid" in the request', async function () {
                    ...
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'Friend', `${id}.json`);
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
                    ...
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'Friend', `local-id-1.json`);
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
                    ...
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'Friend', `local-id-2.json`);
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
                    ...
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'Friend', `local-id-3.json`);
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
        ...
        describe('card PATCH request', function () {
            ...
            describe('public writable realm', function () {
                ...
                it('no-op patch returns existing lastModified and does not rewrite file', async function () {
                    let cardFile = join(dir.name, 'realm_server_1', 'test', 'person-1.json');
                    ...
                    let response = await request
                        ...
                        .set('Accept', 'application/vnd.card+json');
                    expect(response.status).toBe(200);
                    expect(response.body.data.meta.lastModified).toBe(initialLastModified);
               ...
               WHERE url = $2`, {
                    ...
                    expect(card.data.relationships?.['cardInfo.theme']).toEqual({ links: { self: null } });
                });
               ...
               WHERE url = $2`, {
                ...
                it('creates card instances when it encounters "lid" in the request', async function () {
                    ...
                    delete json.data.meta.resourceCreatedAt;
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'hassan.json');
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
                ...
                it('creates card instances when it encounters "lid" in the request for requests that has "isUsed: true" links', async function () {
                    let response = await request
                        ...
                        data: {
                            ...
                            meta: {
                                adoptsFrom: {
                                    module: 'http://localhost:4202/node-test/friend-with-used-link',
                    ...
                    delete json.data.meta.resourceCreatedAt;
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'hassan-x.json');
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
                    ...
                    {
                        let cardFile = join(dir.name, 'realm_server_1', 'test', 'FriendWithUsedLink', `local-id-1.json`);
                        expect(existsSync(cardFile)).toBeTruthy();
                        let card = readJSONSync(cardFile);
                        expect(card).toEqual({
                            data: {
            ...
            describe('public writable realm with size limit', function () {
                ...
                it('returns 413 when card payload exceeds size limit', async function () {
                    ...
                    expect(response.body.errors[0].message.includes('Card size')).toBeTruthy();
                });
            });
            describe('permissioned realm', function () {
                const hooks = createVitestNestedHooksAdapter() as any;
        ...
        describe('card DELETE request', function () {
            ...
            describe('public writable realm', function () {
                ...
                it('removes card JSON file meta when card is deleted', async function () {
                    ...
                    expect(JSON.stringify(after.body).includes('resourceCreatedAt')).toBe(false);
                });
            });
            describe('permissioned realm', function () {
                const hooks = createVitestNestedHooksAdapter() as any;
    ...
    describe('Query-backed relationships runtime resolver', function () {
        ...
        setupPermissionedRealmsCached(hooks, {
            ...
            onRealmSetup({ realms }) {
                ...
                consumerRequest = withRealmPath(supertest(latestRealms[1].realmHttpServer), new URL(consumerRealmURL));
            },
        });
        hooks.afterEach(() => {
            resetCatalogRealms();
        ...
        it('linksTo query resolves the first aggregated result and includes it', async function () {
            ...
            expect(favoriteRelationship?.data).toEqual({ type: 'card', id: `${consumerRealmURL}local-person` });
            ...
            expect(favoriteSearchURL.href.split('?')[0]).toBe(new URL('_search', consumerRealmURL).href);
            let favoriteQueryParams = parseSearchQuery(favoriteSearchURL);
            expect(favoriteQueryParams.page).toEqual({ size: '1', number: '0' });
            expect(favoriteQueryParams.filter?.type?.module).toBe(`${providerRealmURL}person`);
            expect(favoriteQueryParams.filter?.type?.name).toBe('Person');
            expect(Array.isArray(doc.included)).toBeTruthy();
            expect(doc.included.some((resource: any) => resource.id === `${consumerRealmURL}local-person`)).toBeTruthy();
            expect(favoriteRelationship?.data).toEqual({ type: 'card', id: `${consumerRealmURL}local-person` });
            expect(doc.included.find((resource: any) => resource.id === `${consumerRealmURL}local-person`)).toBeTruthy();
        });
        it('linksToMany query returns remote results and records errors for failing realm', async function () {
            ...
            expect(remoteRelationship?.data).toEqual({ type: 'card', id: `${providerRealmURL}person-remote` });
            expect(matchesRelationship?.meta?.errors).toBeFalsy();
            expect(matchesRelationship?.data).toEqual([{ type: 'card', id: `${providerRealmURL}person-remote` }]);
            ...
            expect(matchesSearchURL.href.split('?')[0]).toBe(new URL('_search', providerRealmURL).href);
            ...
            expect(matchesQueryParams.sort?.[0]?.on?.module).toBe(`${providerRealmURL}person`);
            ...
            expect(failingRelationship.meta.errors.some((error: any) => error.realm === UNREACHABLE_REALM_URL)).toBeTruthy();
            ...
            expect(failingSearchURL.href.split('?')[0]).toBe(new URL('_search', UNREACHABLE_REALM_URL).href);
            let failingQueryParams = parseSearchQuery(failingSearchURL);
            expect(failingQueryParams.page).toEqual({ size: '1', number: '0' });
            expect(failingQueryParams.filter?.type?.module).toBe(`${providerRealmURL}person`);
            expect(failingQueryParams.filter?.type?.name).toBe('Person');
            expect(failingRelationship.data).toEqual([]);
            expect(Array.isArray(doc.included)).toBeTruthy();
            let includedIds = (doc.included ?? []).map((resource: any) => resource.id);
            expect(includedIds.includes(`${consumerRealmURL}local-person`)).toBeTruthy();
            expect(includedIds.includes(`${providerRealmURL}person-remote`)).toBeTruthy();
        });
    });
});
