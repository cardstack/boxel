  ...
  describe('indexing (read only)', function () {
    ...
    it('can store card pre-rendered html in the index', async function () {
      ...
      if (entry?.type === 'instance') {
        ...
        ).toBe(cleanWhiteSpace(`<h1> Mango $</h1>`));
        expect(
          trimCardContainer(
        ...
        ).toBe(cleanWhiteSpace(`<h1> Embedded Card Person: Mango </h1>`));
        ...
        ).toBe(cleanWhiteSpace(`<h1> Fitted Card Person: Mango </h1>`));
      } else {
      ...
      }
      {
        ...
        if (entry?.type === 'doc') {
          ...
          if (item?.type === 'instance') {
            ...
            ).toBe(cleanWhiteSpace(`<h1> Van Gogh $50</h1>`));
            expect(
              trimCardContainer(
                stripScopedCSSAttributes(
            ...
            ).toBe(cleanWhiteSpace(`<h1> Fitted Card Person: Van Gogh </h1>`));
          } else {
            expect(false).toBeTruthy();
          }
        } else {
    ...
    it('can make an error doc for a card that has a link to a URL that is not a card', async function () {
      ...
      if (entry?.type === 'error') {
        expect(entry.error.errorDetail.message).toBe(
  ...
  describe('indexing (mutating)', function () {
    ...
    describe('batch and incremental operations', function () {
      ...
      it('pending incremental followed by full index keeps separate pending jobs by type', async function () {
        ...
        try {
          ...
          expect(rows.map((row) => row.job_type).sort()).toEqual([
            'from-scratch-index',
            'incremental-index',
          ]);
          release.fulfill();
      ...
      it('propagates module errors to dependent instances and recovers after missing modules are added', async function () {
        ...
        let rows =
          (await testDbAdapter.execute(`SELECT error_doc IS NULL AS is_sql_null
         FROM boxel_index
    ...
    describe('additive writes', function () {
      ...
      it('collects deep relationship deps from rendered links including field-def linksToMany', async function () {
        ...
        expect(afterLinksToManyInvalidation).not.toBe(
    ...
    describe('error recovery and deletion', function () {
      ...
      it('can incrementally index instance that depends on deleted card source', async function () {
        ...
        if (actual?.type === 'error') {
          ...
          expect(actual.error.errorDetail.message).toBe(
