  ...
  describe('Realm-specific Endpoints | _request-forward', function () {
    ...
    it('should forward request to OpenRouter and deduct credits', async function () {
      ...
      try {
        ...
        expect(firstCallHeaders?.Authorization?.startsWith('Bearer ')).toBe(
          true,
        );
      } finally {
        mockFetch.restore();
    ...
    it('should handle streaming requests', async function () {
      ...
      try {
        ...
        expect(responseText.includes('data: {"id":"gen-stream-123"')).toBe(
          true,
        );
        expect(
    ...
    it('should handle insufficient credits', async function () {
      ...
      expect(response.body.errors?.[0]?.includes('minimum of 10 credits')).toBe(
        true,
      );
    });
    ...
    it('should forward request to Cloudflare AI Gateway with custom header token authentication', async function () {
      ...
      try {
        ...
        const callHeaders = cloudflareCall!.args[1]?.headers as Record<
          ...
          string
        >;
        expect(callHeaders['cf-aig-authorization']).toBe(
    ...
    it('should forward multipart form data when multipart flag is set', async function () {
      ...
      try {
        ...
        expect(bodyText.includes(`name="file"; filename="hello.txt"`)).toBe(
          true,
        );
