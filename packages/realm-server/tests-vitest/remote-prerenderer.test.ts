...
describe('remote-prerenderer-test.ts', function () {
  ...
  describe('remote prerenderer payload', function () {
    async function expectValidationFailure(attrs: any, message: RegExp) {
      ...
      try {
        ...
        await expect(prerenderer.prerenderModule(attrs as any)).rejects.toThrow(
          message,
        );
    ...
    async function expectRunCommandValidationFailure(
      ...
      try {
        let prerenderer = createRemotePrerenderer('http://127.0.0.1:0');
        await expect(prerenderer.runCommand(attrs as any)).rejects.toThrow(
          message,
        );
        expect(fetchCalled).toBe(false);
      } finally {
