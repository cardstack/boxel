...
import type { RealmPermissions, RealmAdapter, RenderResponse, ModuleRenderResponse, FileExtractResponse, RenderRouteOptions, } from '@cardstack/runtime-common';
import { baseRealm, type Realm as RuntimeRealm, } from '@cardstack/runtime-common';
...
import { setupPermissionedRealmsCached, cleanWhiteSpace, testCreatePrerenderAuth, getPrerendererForTesting, } from './helpers';
import '@cardstack/runtime-common/helpers/code-equality-assertion';
import { baseCardRef, trimExecutableExtension, } from '@cardstack/runtime-common';
import { installDelayedRuntimeRealmSearchPatch, installRealmServerAssertOwnRealmServerBypassPatch, installSearchRequestObserverPatch, } from './helpers/prerender-page-patches';
class TestSemaphore {
    #available: number;
...
function makeStubPagePool(maxPages: number, renderSemaphore?: {
    acquire(): Promise<() => void>;
}, createContextDelay?: (contextNumber: number) => Promise<void>, options?: {
...
describe("prerendering-test.ts", function () {
    describe('prerender - mutating tests', function () {
        ...
        let auth = () => {
            let sessions = JSON.parse(testCreatePrerenderAuth(testUserId, permissions)) as Record<string, string>;
        ...
        it('reuses pooled page and picks up updated instance', async function () {
            ...
            await realmAdapter.write('1.json', JSON.stringify({
                data: {
                    attributes: {
                        name: 'Juniper',
                    },
        ...
        it('module prerender reuses pooled page after updates', async function () {
            ...
            expect(first.pool.reused).toBe(false);
            await realmAdapter.write('person.gts', `
          import { CardDef, field, contains, StringField, Component } from 'https://cardstack.com/base/card-api';
          export class Person extends CardDef {
        ...
        it('file extract surfaces broken FileDef module error without remote prerender timeout', async function () {
            await realmAdapter.write('filedef-mismatch.gts', `
        ...
        `);
            ...
            expect(result.response.error?.error.message?.includes('Received HTTP 404 from server')).toBeTruthy();
            let messageIncludesTimeoutMarker = Boolean(result.response.error?.error.message?.includes('Prerender request to'));
    ...
    function defineNonMutatingRunnerTests() {
        describe('runner behavior', function () {
            ...
            it('card prerender hoists module transpile errors', async function () {
                ...
                expect(result.response.error).toBeTruthy();
                expect(result.response.error?.error.status).toBe(406);
                expect(result.response.error?.error.message).toBe(`Parse Error at broken.gts:1:23: 1:24 (${realmURL}broken)`);
                expect(result.response.error?.error.stack?.includes('at transpileJS')).toBeTruthy();
                let additionalErrors = result.response.error?.error.additionalErrors;
                if (additionalErrors !== null) {
                    expect(Array.isArray(additionalErrors)).toBeTruthy();
                    expect(additionalErrors?.every((entry) => entry?.title === 'Console error' ||
                        entry?.title === 'Console assert')).toBeTruthy();
                }
                let deps = result.response.error?.error.deps ?? [];
                expect(deps.some((dep) => dep.includes(`${realmURL}broken`))).toBeTruthy();
            });
            it('card prerender surfaces actionable error for bad icon import', async function () {
                ...
                expect(result.response.error?.error.message?.includes('static icon of BadIconImport is undefined')).toBeTruthy();
            });
            it('card prerender surfaces empty render container', async function () {
                ...
                expect(result.response.error).toBeTruthy();
                expect(result.response.error?.error.title).toBe('Invalid render response');
                expect(result.response.error?.error.message?.includes('[data-prerender] has no child element to capture')).toBeTruthy();
                let errorDeps = result.response.error?.error.deps;
                expect(errorDeps).not.toBe(null);
                let deps = errorDeps ?? [];
                expect(Array.isArray(deps)).toBe(true);
                expect([`${realmURL}no-icon`, `${realmURL}no-icon.json`].some((dep) => deps.includes(dep))).toBe(true);
            });
            it('card prerender surfaces runtime render errors without timing out', async function () {
                ...
                expect(result.response.error).toBeTruthy();
                expect(result.response.error?.error.status).toBe(500);
                expect(result.response.error?.error.message?.includes('boom')).toBeTruthy();
                expect(result.pool.timedOut).toBe(false);
            ...
            it('card prerender includes console errors when render fails', async function () {
                ...
                let additionalErrors = result.response.error?.error.additionalErrors;
                expect(Array.isArray(additionalErrors)).toBeTruthy();
                expect(additionalErrors?.some((error) => typeof error?.message === 'string' &&
                    error.message.includes('console boom'))).toBeTruthy();
            });
            ...
            it('card prerender surfaces unhandled promise rejection without timing out', async function () {
                ...
                expect(result.response.error).toBeTruthy();
                expect(result.response.error?.error.status).toBe(500);
                expect(result.response.error?.error.message?.includes('reject boom')).toBeTruthy();
                expect(result.pool.timedOut).toBe(false);
                expect(result.pool.evicted).toBe(true);
            });
            it('card prerender surfaces RSVP rejection without timing out', async function () {
                ...
                expect(result.response.error).toBeTruthy();
                expect(result.response.error?.error.status).toBe(500);
                expect(result.response.error?.error.message?.includes('rsvp boom')).toBeTruthy();
                expect(result.pool.timedOut).toBe(false);
                expect(result.pool.evicted).toBe(true);
            });
            it('card prerender surfaces errors thrown before the render model hook', async function () {
                ...
                try {
                            ...
                            page.evaluate = async (...args: any[]) => {
                                if (!injected) {
                                    ...
                                    await originalEvaluate(() => {
                                        let entries = (window as any).requirejs?.entries ??
                                            (window as any).require?.entries ??
                                            (window as any)._eak_seen;
                                        let renderModuleName = entries &&
                                            Object.keys(entries).find((name) => name.endsWith('/routes/render'));
                                        if (!renderModuleName) {
                                            throw new Error('render route module not found for injection');
                                        }
                                        let renderRouteModule = (window as any).require(renderModuleName);
                                        let RenderRouteClass = renderRouteModule?.default;
                                        if (!RenderRouteClass?.prototype) {
                                            throw new Error('render route class not found for injection');
                                        }
                                        let originalBeforeModel = RenderRouteClass.prototype.beforeModel;
                                        RenderRouteClass.prototype.beforeModel = async function (...bmArgs: any[]) {
                    ...
                    expect(result.response.error).toBeTruthy();
                    expect(result.response.error?.error.message?.includes('boom before model')).toBeTruthy();
                    expect(result.pool.evicted).toBe(true);
                    ...
                    let deps = errorDeps ?? [];
                    expect(Array.isArray(deps)).toBe(true);
                    expect([`${realmURL}1.json`, `${realmURL}1`].some((dep) => deps.includes(dep))).toBe(true);
                }
                finally {
            ...
            it('card prerender waits for query fallback search and nested relationship loads', async function () {
                ...
                let realmServerPatch = installRealmServerAssertOwnRealmServerBypassPatch();
                ...
                try {
                    ...
                    expect(/data-test-staff-name[^>]*>\s*Bob\s*</.test(isolatedHTML)).toBeTruthy();
                    ...
                    let staff = result.response.searchDoc?.staff as Array<Record<string, any>> | undefined;
                    ...
                    let hasEveWithManager = bobReports?.some((report) => report?.name === 'Eve' && report?.manager?.name === 'Bob');
            ...
            it('file prerender returns extracted metadata', async function () {
                ...
                expect(result.response.deps.includes(`${baseRealm.url}file-api`)).toBeTruthy();
    ...
    function defineRuntimeDepsResetTests() {
        describe('runtime deps reset', function () {
            ...
            it('resets runtime deps between consecutive prerenders', async function () {
                ...
                expect(firstDeps.includes(`${realmURL}dep-reset-friend-a.json`)).toBe(true);
                let second = await prerenderer.prerenderCard({
                    affinityType: 'realm',
                    affinityValue: realmURL,
                    realm: realmURL,
                ...
                expect(secondDeps.includes(`${realmURL}dep-reset-friend-b.json`)).toBe(true);
                expect(secondDeps.includes(`${realmURL}dep-reset-friend-a.json`)).toBe(false);
            });
        });
    }
        ...
        describe('live prerendered search fallback', function () {
            ...
            async function overrideIndexedIsolatedHTML(url: string, html: string) {
                let alternate = url.endsWith('.json')
                    ? url.replace(/\.json$/, '')
                    : `${url}.json`;
                await dbAdapter.execute(`UPDATE boxel_index SET isolated_html = $1 WHERE url = $2 OR url = $3`, { bind: [html, url, alternate] });
            }
            setupPermissionedRealmsCached(hooks, {
                mode: 'before',
                realms: [
            ...
            it('card prerendered search uses live rendered CardDef HTML and keeps unique CSS', async function () {
                ...
                try {
                    let indexedRows = await dbAdapter.execute(`SELECT url FROM boxel_index WHERE url LIKE $1 ORDER BY url`, { bind: [`${realmURL}%live-search%`] });
                    expect(indexedRows.length > 0).toBeTruthy();
                    await overrideIndexedIsolatedHTML(`${realmURL}live-search-result-1`, `<div data-test-stale-card-html>${sentinel}</div>`);
                    let result = await prerenderer.prerenderCard({
                        affinityType: 'realm',
                        affinityValue: realmURL,
                        realm: realmURL,
                    ...
                    expect(isolatedHTML.includes('live-search-css-sentinel')).toBeTruthy();
                    expect(/live-search-css-sentinel[^>]*data-scopedcss-[a-f0-9]{10}-[a-f0-9]{10}/.test(isolatedHTML)).toBeTruthy();
                }
                finally {
            ...
            it('card prerendered search uses live rendered FileDef HTML', async function () {
                ...
                let realmServerPatch = installRealmServerAssertOwnRealmServerBypassPatch();
                try {
                    await overrideIndexedIsolatedHTML(`${realmURL}live-file.live`, `<article data-test-stale-file-html>${sentinel}</article>`);
                    let result = await prerenderer.prerenderCard({
                        affinityType: 'realm',
                        affinityValue: realmURL,
                        realm: realmURL,
                    ...
                    expect(isolatedHTML.includes(sentinel)).toBeFalsy();
                    expect(isolatedHTML.includes('data-test-live-file-search-card')).toBeTruthy();
                }
                finally {
                    await realmServerPatch.restore();
                }
    ...
    describe('prerender - permissioned auth failures', function () {
        ...
        setupPermissionedRealmsCached(hooks, {
            ...
            onRealmSetup({ realms }) {
                indexingReady = Promise.all(realms.map(({ realm }) => realm.indexing())).then(() => undefined);
                permissions = {
                    [consumerRealmURL]: ['read', 'write', 'realm-owner'],
                };
            },
      ...
      `,
        ...
        it('card prerender in a public realm authorizes query fallback search for source-loaded linked cards', async function () {
            ...
            try {
                ...
                expect(delayedSearchPatch.getRequestCount() > 0).toBe(true);
                let isolatedHTML = cleanWhiteSpace(result.response.isolatedHTML ?? '');
                expect(/data-test-directory-person-name[^>]*>\s*Beta\s*</.test(isolatedHTML)).toBeTruthy();
            }
            finally {
                await realmServerPatch.restore();
                delayedSearchPatch.restore();
    ...
    describe('prerender - permissioned auth failures (private query fallback)', function () {
        ...
        it('card prerender in a private realm sends auth header on query fallback federated search', async function () {
            ...
            try {
                ...
                let querySearchRequests = searchRequests.filter((request) => request.method === 'QUERY');
                expect(querySearchRequests.length > 0).toBe(true);
                expect(querySearchRequests.every((request) => request.hasAuthorization)).toBe(true);
            }
            finally {
                delayedSearchPatch.restore();
                searchRequestObserverPatch.restore();
            ...
            `,
                            ...
                            'non-isolated-theme.json': {
                                data: {
                                    ...
                                    attributes: {
                                        markUsage: {
                                            socialMediaProfileIcon: 'https://example.com/non-isolated-social-icon.png',
                                        },
                                    },
                                    meta: {
                                        adoptsFrom: {
  ...
  <pre data-prerender-error>
  {
      ...
      "additionalErrors": null
            ...
            describe('basics', function () {
                ...
                it('embedded HTML', function () {
                    expect(/Maple\s+says\s+Meow/.test(cleanWhiteSpace(result.embeddedHTML![`${realmURL2}cat/Cat`]))).toBeTruthy();
                });
                ...
                it('isolated HTML', function () {
                    expect(/data-test-field="cardInfo-summary"/.test(result.isolatedHTML!)).toBeTruthy();
                });
                ...
                it('head HTML', function () {
                    ...
                    expect(cleanedHead.includes('<title data-test-card-head-title>Untitled Cat</title>')).toBeTruthy();
                    expect(cleanedHead.includes('property="og:title" content="Untitled Cat"')).toBeTruthy();
                    expect(cleanedHead.includes(`property="og:url" content="${realmURL2}1"`)).toBeTruthy();
                    expect(cleanedHead.includes('name="twitter:card" content="summary"')).toBeTruthy();
                });
                ...
                it('deps', function () {
                    ...
                    expect(result.deps?.find((d) => d.match(/^https:\/\/cardstack.com\/base\/card-api\.gts\..*glimmer-scoped\.css$/))).toBeTruthy();
                });
                it('types', function () {
                ...
                it('isUsed field includes a field in search doc that is not rendered in template', async function () {
                    ...
                    expect(/data-test-field="owner"/.test(response.isolatedHTML!)).toBe(false);
                ...
                it('isUsed linksToMany field includes links in search doc that are not rendered in template', async function () {
                    ...
                    expect(/data-test-field="owners"/.test(response.isolatedHTML!)).toBe(false);
                ...
                it('isUsed compound field includes nested linksTo relationship in search doc', async function () {
                    ...
                    expect(/data-test-field="profile"/.test(response.isolatedHTML!)).toBe(false);
                    expect(response.searchDoc?.profile?.primaryOwner?.name).toBe('Hassan');
                });
                it('isUsed compound field includes nested linksToMany relationships in search doc', async function () {
                    ...
                    expect(/Mango Profile/.test(response.isolatedHTML!)).toBeTruthy();
                    expect(/data-test-field="profile"/.test(response.isolatedHTML!)).toBe(false);
                    expect(response.searchDoc?.profile?.caretakers?.[0]?.name).toBe('Hassan');
                    expect(response.searchDoc?.profile?.caretakers?.[1]?.name).toBe('Mango');
                });
                it('non-isolated formats render linked fields and those links appear in search doc', async function () {
                    ...
                    expect(/<link rel="icon" href="https:\/\/example\.com\/non-isolated-social-icon\.png"/.test(headHTML)).toBeTruthy();
                    expect(/<link rel="apple-touch-icon" href="https:\/\/example\.com\/non-isolated-social-icon\.png"/.test(headHTML)).toBeTruthy();
                    let embedded = response.embeddedHTML?.[`${realmURL2}non-isolated-links-card/NonIsolatedLinks`] ?? '';
                    let cleanedEmbedded = cleanWhiteSpace(embedded);
                    expect(/data-test-embedded-owner[^>]*>\s*Hassan\s*</.test(cleanedEmbedded)).toBeTruthy();
                    expect(/data-test-embedded-owner-name[^>]*>\s*Hassan\s*</.test(cleanedEmbedded)).toBeTruthy();
                    expect(/data-test-embedded-owner-name[^>]*>\s*Mango\s*</.test(cleanedEmbedded)).toBeTruthy();
                    expect(/data-test-embedded-profile-lead-name[^>]*>\s*Hassan\s*</.test(cleanedEmbedded)).toBeTruthy();
                    expect(/data-test-embedded-profile-member-name[^>]*>\s*Hassan\s*</.test(cleanedEmbedded)).toBeTruthy();
                    expect(/data-test-embedded-profile-member-name[^>]*>\s*Mango\s*</.test(cleanedEmbedded)).toBeTruthy();
                    expect(response.searchDoc?.owner?.name).toBe('Hassan');
                    expect(response.searchDoc?.owners?.[0]?.name).toBe('Hassan');
                    expect(response.searchDoc?.owners?.[1]?.name).toBe('Mango');
                    expect(response.searchDoc?.profile?.lead?.name).toBe('Hassan');
                    expect(response.searchDoc?.profile?.members?.[0]?.name).toBe('Hassan');
            ...
            describe('errors', function () {
                ...
                it('error during render', async function () {
                    ...
                    expect(error?.error.message).toBe('intentional failure during render');
                ...
                it('error includes blocked timer summary when timers fire', async function () {
                    ...
                    expect(stack.includes('Timers blocked during prerender')).toBeTruthy();
                ...
                it('missing link surfaces 404 without eviction', async function () {
                    ...
                    let { response } = result;
                    expect(response.error).toBeTruthy();
                    expect(response.error?.error.message).toBe(`missing file ${realmURL1}missing-owner.json`);
                ...
                it('fetch failed surfaces error without eviction', async function () {
                    ...
                    let { response } = result;
                    expect(response.error).toBeTruthy();
                    expect(response.error?.error.message).toBe(`unable to fetch http://localhost:9000/this-is-a-link-to-nowhere: fetch failed`);
                ...
                it('unusable triggers eviction and short-circuit', async function () {
                    ...
                    expect(unusable.response.error?.error.message).toBe('forced unusable for test');
            ...
            describe('affinity pooling', function () {
                ...
                it('refreshes prerender session when auth changes for the same realm', async function () {
                    ...
                    expect(second.response.serialized?.data.attributes?.name).toBe('Maple');
                });
                ...
                it('each tab uses a separate browser context', async function () {
                    ...
                    expect(first.page.browserContext()).not.toBe(second.page.browserContext());
                    await first.page.evaluate((key: string, value: string) => localStorage.setItem(key, value), 'boxel-test-local-storage-key', 'realm-a-value');
                    let firstValue = await first.page.evaluate((key: string) => localStorage.getItem(key), 'boxel-test-local-storage-key');
                    let secondValue = await second.page.evaluate((key: string) => localStorage.getItem(key), 'boxel-test-local-storage-key');
    ...
    describe('prerender - module retries', function () {
        it('module prerender retries with clear cache on retry signature', async function () {
            ...
            try {
                let attemptCount = 0;
                RenderRunner.prototype.prerenderModuleAttempt = async function (args: Parameters<RenderRunner['prerenderModuleAttempt']>[0]) {
                    ...
                    let response: ModuleRenderResponse = attemptCount === 1
                        ? {
                            ...baseResponse,
                            status: 'error',
                            error: {
    ...
    describe('prerender - file retries', function () {
        it('file prerender retries with clear cache on retry signature', async function () {
            ...
            try {
                let attemptCount = 0;
                RenderRunner.prototype.prerenderFileExtractAttempt = async function (args: Parameters<RenderRunner['prerenderFileExtractAttempt']>[0]) {
                    let { affinityType, affinityValue, url, renderOptions } = args;
                    attempts.push(renderOptions);
                    attemptCount++;
                    let response: FileExtractResponse = attemptCount === 1
                        ? {
                            id: url,
                            nonce: `nonce-${attemptCount}`,
                            status: 'error',
    ...
    describe('prerender - card retries', function () {
        it('card prerender retries with clear cache on retry signature', async function () {
            ...
            try {
                let attemptCount = 0;
                RenderRunner.prototype.prerenderCardAttempt = async function (args: Parameters<RenderRunner['prerenderCardAttempt']>[0]) {
                    let { affinityType, affinityValue, url: attemptUrl, renderOptions, } = args;
                    attempts.push(renderOptions);
                    attemptCount++;
                    ...
                    let response: RenderResponse = attemptCount === 1
                        ? {
                            ...baseResponse,
                            error: {
                                type: 'instance-error',
