...
import { setupPermissionedRealmCached, runTestRealmServer, setupDB, setupMatrixRoom, createVirtualNetwork, matrixURL, closeServer, type RealmRequest, withRealmPath, } from './helpers';
...
describe("types-endpoint-test.ts", function () {
    describe('Realm-specific Endpoints | GET _types', function () {
        ...
        it('can fetch card type summary', async function () {
            let response = await request
                .get('/_types')
                .set('Accept', 'application/json');
            let iconHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="lucide lucide-captions" viewBox="0 0 24 24"><rect width="18" height="14" x="3" y="5" rx="2" ry="2"></rect><path d="M7 15h4m4 0h2M7 11h2m4 0h4"></path></svg>';
            let chessIconHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="lucide lucide-rectangle-ellipsis" viewBox="0 0 24 24"><rect width="20" height="12" x="2" y="6" rx="2"></rect><path d="M12 12h.01M17 12h.01M7 12h.01"></path></svg>';
            let sortCardTypeSummaries = (summaries: any[]) => [...summaries].sort((a, b) => {
            ...
            expect(sortCardTypeSummaries(response.body.data)).toEqual(sortCardTypeSummaries(expectedData));
        });
    });
});
