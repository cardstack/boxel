...
describe("server-endpoints/user-and-catalog-test.ts", function () {
    describe('Realm Server Endpoints (not specific to one realm)', function () {
        ...
        it('can not create a user without a jwt', async function () {
            let response = await context.request2.post('/_user').send({});
            expect(response.status).toBe(401);
        });
        it('can fetch catalog realms', async function () {
            let response = await context.request2
                .get('/_catalog-realms')
                .set('Accept', 'application/json');
            expect(response.status).toBe(200);
        ...
        it(`returns 200 with empty data if failed to fetch catalog realm's info`, async function () {
