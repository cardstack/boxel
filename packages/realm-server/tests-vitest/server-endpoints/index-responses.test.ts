...
import { DEFAULT_PERMISSIONS, systemInitiatedPriority, type DBAdapter, type Realm, } from '@cardstack/runtime-common';
import type { PgAdapter } from '@cardstack/postgres';
import { testRealmURL } from './helpers';
import { closeServer, createVirtualNetwork, matrixURL, realmSecretSeed, runTestRealmServer, setupDB, setupPermissionedRealmCached, waitUntil, } from '../helpers';
...
describe("server-endpoints/index-responses-test.ts", function () {
    describe('Realm Server Endpoints (not specific to one realm)', function () {
        ...
        it('startup indexing uses system initiated queue priority', async function () {
            let [job] = (await dbAdapter.execute(`SELECT priority FROM jobs WHERE job_type = 'from-scratch-index' AND args->>'realmURL' = '${testRealmURL.href}' ORDER BY created_at DESC LIMIT 1`)) as {
                priority: number;
            }[];
        ...
        it('does not inject head or isolated HTML when realm is not public', async function () {
            await dbAdapter.execute(`DELETE FROM realm_user_permissions WHERE realm_url = '${testRealmURL.href}' AND username = '*'`);
            let response = await request
        ...
        it('ignores deleted index entries for head, isolated, and scoped CSS injection', async function () {
            ...
            await waitUntil(async () => {
                let realmURLNoProtocol = testRealmURL.href.replace(/^https?:\/\//, '');
                for (let slug of deleteSlugs) {
        ...
        it('HTML response includes exactly one favicon and one apple-touch-icon', async function () {
            ...
            let appleTouchIconCount = (response.text.match(/rel="apple-touch-icon"/g) || []).length;
        ...
        it('missing apple-touch-icon is filled with default when only favicon is present in head HTML', async function () {
           ...
           WHERE url = '${cardURL}'
            ...
            let headMatch = response.text.match(/data-boxel-head-start[^>]*>([\s\S]*?)data-boxel-head-end/);
            let headContent = headMatch?.[1] ?? '';
            expect(headContent.includes('<link rel="icon" href="https://example.com/custom-icon.png"')).toBeTruthy();
            expect(headContent.includes('rel="apple-touch-icon"')).toBeTruthy();
            expect(headContent.includes('boxel-webclip.png')).toBeTruthy();
            let faviconCount = (response.text.match(/rel="icon"/g) || []).length;
            let appleTouchIconCount = (response.text.match(/rel="apple-touch-icon"/g) || []).length;
        ...
        it('missing favicon is filled with default when only apple-touch-icon is present in head HTML', async function () {
           ...
           WHERE url = '${cardURL}'
            ...
            let headMatch = response.text.match(/data-boxel-head-start[^>]*>([\s\S]*?)data-boxel-head-end/);
            let headContent = headMatch?.[1] ?? '';
            expect(headContent.includes('<link rel="apple-touch-icon" href="https://example.com/custom-touch.png"')).toBeTruthy();
            expect(headContent.includes('rel="icon"')).toBeTruthy();
            expect(headContent.includes('boxel-favicon.png')).toBeTruthy();
            let faviconCount = (response.text.match(/rel="icon"/g) || []).length;
            let appleTouchIconCount = (response.text.match(/rel="apple-touch-icon"/g) || []).length;
        ...
        it('default head template includes favicon and apple-touch-icon from cardInfo.theme', async function () {
            ...
            await waitUntil(async () => {
                let rows = (await dbAdapter.execute(`SELECT url, head_html FROM boxel_index
        ...
        it('preserves scoped CSS in HTML response after card enters error state', async function () {
            ...
            let errorRows = (await dbAdapter.execute(`SELECT has_error, last_known_good_deps FROM boxel_index
           WHERE url = '${testRealmURL.href}scoped-css-test.json'
             AND type = 'instance'`)) as {
                has_error: boolean;
    ...
    describe('Published realm: theme icon links after _publish-realm', function () {
        ...
        setupDB(hooks, {
            beforeEach: async (_dbAdapter, _publisher, _runner) => {
                ...
                if (createResponse.status !== 201) {
                    throw new Error(`/_create-realm failed with status ${createResponse.status}: ` +
                        (createResponse.text ||
                            (createResponse.body
          ...
          `);
                ...
                let themeResponse = await request
                    ...
                    data: {
                        ...
                        attributes: {
                            markUsage: {
                                socialMediaProfileIcon: 'https://example.com/published-theme-icon.png',
                            },
                        },
                        meta: {
                            adoptsFrom: {
                ...
                if (themeResponse.status !== 204) {
                    throw new Error(`Failed to write brand-guide-theme: ${themeResponse.status} ${themeResponse.text}`);
                }
                ...
                if (cardResponse.status !== 204) {
                    throw new Error(`Failed to write themed-card: ${cardResponse.status} ${cardResponse.text}`);
                }
                ...
                if (publishResponse.status !== 201) {
                    throw new Error(`Failed to publish realm: ${publishResponse.status} ${publishResponse.text}`);
                }
            },
            afterEach: async () => {
        ...
        it('themed card in published realm includes theme icon links in head HTML', async function () {
            ...
            let headMatch = response.text.match(/data-boxel-head-start[^>]*>([\s\S]*?)data-boxel-head-end/);
            let headContent = headMatch?.[1] ?? '';
            expect(headContent.includes('<link rel="icon" href="https://example.com/published-theme-icon.png"')).toBeTruthy();
            expect(headContent.includes('<link rel="apple-touch-icon" href="https://example.com/published-theme-icon.png"')).toBeTruthy();
            // Verify the pristine_doc preserves the theme relationship
            let rows = (await dbAdapter.execute(`SELECT pristine_doc::text FROM boxel_index
           ...
           LIMIT 1`)) as {
            ...
            let themeRel = pristineDoc?.relationships?.['cardInfo.theme']?.links?.self;
