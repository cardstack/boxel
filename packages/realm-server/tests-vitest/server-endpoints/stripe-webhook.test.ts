...
import { createJWT, fetchSubscriptionsByUserId, insertPlan, insertUser, realmSecretSeed, realmServerTestMatrix, setupPermissionedRealmCached, } from '../helpers';
import { createRealmServerSession } from './helpers';
import { APP_BOXEL_REALM_SERVER_EVENT_MSGTYPE } from '@cardstack/runtime-common/matrix-constants';
import type { MatrixEvent, RealmServerEventContent, } from 'https://cardstack.com/base/matrix-event';
import '@cardstack/runtime-common/helpers/code-equality-assertion';
describe("server-endpoints/stripe-webhook-test.ts", function () {
    describe('Realm Server Endpoints (not specific to one realm)', function () {
        describe('stripe webhook handler', function () {
            const hooks = {
                before: beforeAll,
                after: afterAll,
                beforeEach,
            ...
            let waitForBillingNotification = async function () {
                ...
                if (messageEventContentIsRealmServerEvent(firstMessageContent)) {
                    expect((firstMessageContent as RealmServerEventContent).body).toBe(JSON.stringify({ eventType: 'billing-notification' }));
                    return;
                }
                else {
                    await new Promise<void>(resolve => setTimeout(resolve, 1));
                    return await waitForBillingNotification();
                }
            };
            ...
            hooks.beforeEach(async function () {
                ...
                let { sessionRoom } = await createRealmServerSession(matrixClient, request);
            ...
            test('subscribes user back to free plan when the current subscription is expired', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
                let freePlan = await insertPlan(dbAdapter, 'Free plan', 0, 100, 'prod_free');
                ...
                let stripeInvoicePaymentSucceededPayload = JSON.stringify(stripeInvoicePaymentSucceededEvent);
                let stripeInvoicePaymentSucceededSignature = Stripe.webhooks.generateTestHeaderString({
                ...
                expect(subscriptions[0].planId).toBe(creatorPlan.id);
                ...
                let stripeSubscriptionDeletedPayload = JSON.stringify(stripeSubscriptionDeletedEvent);
                let stripeSubscriptionDeletedSignature = Stripe.webhooks.generateTestHeaderString({
                ...
                expect(subscriptions[1].planId).toBe(freePlan.id);
                await waitForBillingNotification();
            });
            test('ensures the current subscription expires when free plan subscription fails', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
                ...
                let response = await request
                    ...
                    .set('Authorization', `Bearer ${createJWT(testRealm, '@test_realm:localhost', [
                    'read',
                    'write',
                ])}`);
            ...
            test('sends billing notification on invoice payment succeeded event', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
                await insertPlan(dbAdapter, 'Free plan', 0, 100, 'prod_free');
                if (!secret) {
                    throw new Error('STRIPE_WEBHOOK_SECRET is not set');
            ...
            test('sends billing notification on checkout session completed event', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
                await insertPlan(dbAdapter, 'Free plan', 0, 100, 'prod_free');
                if (!secret) {
                    throw new Error('STRIPE_WEBHOOK_SECRET is not set');
...
function messageEventContentIsRealmServerEvent(content: MatrixEvent['content']): content is RealmServerEventContent {
    return ('msgtype' in content &&
