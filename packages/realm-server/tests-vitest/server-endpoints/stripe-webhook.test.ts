...
describe("server-endpoints/stripe-webhook-test.ts", function () {
    describe('Realm Server Endpoints (not specific to one realm)', function () {
        describe('stripe webhook handler', function () {
            ...
            it('subscribes user back to free plan when the current subscription is expired', async function () {
            ...
            it('ensures the current subscription expires when free plan subscription fails', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
            ...
            it('sends billing notification on invoice payment succeeded event', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
                await insertPlan(dbAdapter, 'Free plan', 0, 100, 'prod_free');
                if (!secret) {
                    throw new Error('STRIPE_WEBHOOK_SECRET is not set');
            ...
            it('sends billing notification on checkout session completed event', async function () {
                const secret = process.env.STRIPE_WEBHOOK_SECRET;
                await insertPlan(dbAdapter, 'Free plan', 0, 100, 'prod_free');
                if (!secret) {
                    throw new Error('STRIPE_WEBHOOK_SECRET is not set');
