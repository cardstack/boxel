...
type WorkerPrerenderServerState = {
  server?: Server;
...
function logRealmSetup(message: string) {
  ...
  console.log(`[realm-setup ${new Date().toISOString()}] ${message}`);
}

async function timeRealmSetup<T>(
  label: string,
  task: () => Promise<T>,
): Promise<T> {
  let start = Date.now();
  logRealmSetup(`start ${label}`);
  try {
    let result = await task();
    logRealmSetup(`done ${label} in ${Date.now() - start}ms`);
    return result;
  } catch (error) {
    logRealmSetup(`failed ${label} after ${Date.now() - start}ms`);
    throw error;
  }
}
...
function currentVitestWorkerId(): string {
  return (
...
async function withResolvedServerURL(serverURL: URL): Promise<URL> {
  if (serverURL.port !== '0') {
    return serverURL;
  }
  let resolved = new URL(serverURL.href);
  resolved.port = String(await findAvailablePort(serverURL.hostname));
  return resolved;
}
    ...
    async () => {
      ...
      if (realmSetupDebug) {
        ...
        if (now - lastLogAt >= 1000) {
          lastLogAt = now;
          logRealmSetup(
            `queue state unfulfilled=${unfulfilledJobs} activeReservations=${activeReservations}`,
          );
        }
      }
...
async function assertPrerenderDependenciesReady(): Promise<void> {
  let checks = [
    {
      name: 'host standby route',
      url: 'http://localhost:4200/standby',
    },
    {
      name: 'base realm',
      url: 'http://localhost:4201/base/card-api',
    },
    {
      name: 'icons server',
      url: 'http://localhost:4206/@cardstack/boxel-icons/v1/icons/palette.js',
    },
  ];
  ...
  for (let check of checks) {
    try {
      ...
      if (!response.ok) {
        failures.push(
          `${check.name} (${check.url}) returned HTTP ${response.status}`,
        );
      }
    } catch (error: any) {
      failures.push(
        `${check.name} (${check.url}) is unavailable: ${error?.message ?? error}`,
      );
    }
  }
  ...
  if (failures.length > 0) {
    throw new Error(
      `Prerender dependencies are not ready:\n${failures.join('\n')}\n` +
        `Start required services (for example: base realm + icons + host) before running endpoint Vitest suites.`,
    );
  }
}
...
async function startTestPrerenderServer(): Promise<string> {
  ...
  let state = prerenderServersByWorker.get(workerId);
  if (!state) {
    state = {};
    prerenderServersByWorker.set(workerId, state);
  }
  if (state.server?.listening && state.url) {
    logRealmSetup(
      `prerender server already listening for worker ${workerId} at ${state.url}`,
    );
    return state.url;
  }
  if (state.start) {
    logRealmSetup(
      `waiting for existing prerender server start for worker ${workerId}`,
    );
    return await state.start;
  }
  await assertPrerenderDependenciesReady();
  ...
  state.server = server;
  trackServer(server);
  state.start = new Promise<string>((resolve, reject) => {
    let onError = (error: Error) => {
      server.off('error', onError);
      let workerState = prerenderServersByWorker.get(workerId);
      if (workerState) {
        workerState.server = undefined;
        workerState.start = undefined;
        workerState.url = undefined;
      }
      if (server.listening) {
    ...
    server.listen(0, testPrerenderHost, () => {
      server.off('error', onError);
      let address = server.address();
      if (!address || typeof address === 'string') {
        onError(new Error('Could not determine test prerender server address'));
        return;
      }
      let url = `http://${testPrerenderHost}:${address.port}`;
      let workerState = prerenderServersByWorker.get(workerId);
      if (workerState) {
        workerState.start = undefined;
        workerState.url = url;
  ...
  logRealmSetup(`prerender server listening for worker ${workerId} at ${url}`);
  return url;
}

export async function stopTestPrerenderServer() {
  for (let state of prerenderServersByWorker.values()) {
    if (state.server && state.server.listening) {
      if (hasStopPrerenderer(state.server)) {
        await state.server.__stopPrerenderer?.();
      }
...
export async function runTestRealmServer({
  ...
  let resolvedServerURL = serverURL ?? realmURL;
  logRealmSetup(
    `runTestRealmServer setup for ${realmURL.href} via ${resolvedServerURL.href}`,
  );
  let prerenderer = providedPrerenderer ?? (await getTestPrerenderer());
  logRealmSetup(`runTestRealmServer got prerenderer for ${realmURL.href}`);
  ...
  await timeRealmSetup(
    `worker.run(${realmURL.href})`,
    async () => await worker.run(),
  );
  let { realm: testRealm, adapter: testRealmAdapter } = await timeRealmSetup(
    `createRealm(${realmURL.href})`,
    async () =>
      await createRealm({
        dir: testRealmDir,
        fileSystem,
  ...
  await timeRealmSetup(
    `testRealm.logInToMatrix(${realmURL.href})`,
    async () => await testRealm.logInToMatrix(),
  );
  ...
  let testRealmServer = new RealmServer({
    ...
    getIndexHTML,
    grafanaSecret,
    serverURL: new URL(resolvedServerURL.origin),
    assetsURL: new URL(`http://example.com/notional-assets-host/`),
    domainsForPublishedRealms,
    definitionLookup,
    prerenderer,
  });
  logRealmSetup(`starting RealmServer.listen(${resolvedServerURL.port})`);
  let testRealmHttpServer = testRealmServer.listen(
    parseInt(resolvedServerURL.port),
  );
  trackServer(testRealmHttpServer);
  await timeRealmSetup(
    `testRealmServer.start(${realmURL.href})`,
    async () => await testRealmServer.start(),
  );
  logRealmSetup(`runTestRealmServer ready for ${realmURL.href}`);
  return {
    testRealmDir,
    testRealm,
    testRealmServer,
...
async function startPermissionedRealmFixture(
  ...
  let resolvedServerURL = serverURL
    ? await withResolvedServerURL(serverURL)
    : resolvedRealmURL;
  let dir = dirSync();
  logRealmSetup(
    `starting permissioned fixture for ${resolvedRealmURL.href} via ${resolvedServerURL.href}`,
  );
  ...
  let testRealmServer = await timeRealmSetup(
    `runTestRealmServer(${resolvedRealmURL.href})`,
    async () =>
      await runTestRealmServer({
        virtualNetwork,
        testRealmDir,
  ...
  logRealmSetup(
    `permissioned fixture ready for ${resolvedRealmURL.href} in ${Date.now() - fixtureStart}ms`,
  );
...
async function buildPermissionedRealmTemplate(
  ...
  logRealmSetup(
    `building permissioned realm template ${cacheKey.slice(0, 12)}`,
  );
  ...
  await timeRealmSetup(
    `drop template db ${templateDatabaseName}`,
    async () => await dropDatabase(templateDatabaseName),
  );
  await timeRealmSetup(
    `drop builder db ${builderDatabaseName}`,
    async () => await dropDatabase(builderDatabaseName),
  );

  try {
    dbAdapter = await timeRealmSetup(
      `createTestPgAdapter(${builderDatabaseName})`,
      async () =>
    ...
    logRealmSetup(
      `built permissioned realm template ${cacheKey.slice(0, 12)} in ${Date.now() - buildStart}ms`,
    );
  } finally {
    if (fixture) {
      try {
...
async function acquirePermissionedRealmTemplate(
  ...
  if (existing) {
    existing.refs++;
    logRealmSetup(
      `permissioned realm template cache hit ${cacheKey.slice(0, 12)} refs=${existing.refs}`,
    );
    await existing.ready;
    return { cacheKey, templateDatabaseName };
  }
  logRealmSetup(
    `permissioned realm template cache miss ${cacheKey.slice(0, 12)}`,
  );
...
export function setupPermissionedRealmCached(
  ...
  hooks.before(async function () {
    logRealmSetup(`setupPermissionedRealmCached before hook begin`);
    ...
    logRealmSetup(
      `setupPermissionedRealmCached before hook ready cacheKey=${cacheKey.slice(0, 12)} db=${templateDatabaseName}`,
    );
  });
