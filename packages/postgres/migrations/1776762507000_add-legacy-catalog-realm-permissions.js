      ...
      pgm.sql(
        `INSERT INTO realm_user_permissions (realm_url, username, read, write, realm_owner)
         VALUES
           ...
           ('http://localhost:4205/legacy-catalog/', '@legacy_catalog_realm:localhost', true, true, true),
           ('http://localhost:4205/legacy-catalog/', '*', true, false, false)
         ON CONFLICT ON CONSTRAINT realm_user_permissions_pkey
         DO UPDATE SET
           realm_url   = EXCLUDED.realm_url,
           username    = EXCLUDED.username,
...
exports.down = (pgm) => {
  switch (process.env.REALM_SENTRY_ENVIRONMENT) {
    ...
    default:
      pgm.sql(
        "DELETE FROM realm_user_permissions WHERE realm_url IN ('http://localhost:4201/legacy-catalog/', 'http://localhost:4205/legacy-catalog/') AND username IN ('@legacy_catalog_realm:localhost', '*')",
      );
  }
};
