  ...
  switch (process.env.REALM_SENTRY_ENVIRONMENT) {
    ...
    default:
      pgm.sql(
        `INSERT INTO realm_user_permissions (realm_url, username, read, write, realm_owner)
         VALUES
           ('https://localhost:4201/pretui/', '@pretui_realm:localhost', true, true, true),
           ('https://localhost:4201/pretui/', '*', true, false, false),
           ('https://localhost:4205/pretui/', '@pretui_realm:localhost', true, true, true),
           ('https://localhost:4205/pretui/', '*', true, false, false)
         ON CONFLICT ON CONSTRAINT realm_user_permissions_pkey
         DO UPDATE SET
           realm_url   = EXCLUDED.realm_url,
           username    = EXCLUDED.username,
...
exports.down = (pgm) => {
  switch (process.env.REALM_SENTRY_ENVIRONMENT) {
    ...
    default:
      pgm.sql(
        "DELETE FROM realm_user_permissions WHERE realm_url IN ('https://localhost:4201/pretui/', 'https://localhost:4205/pretui/') AND username IN ('@pretui_realm:localhost', '*')",
      );
  }
};
