...
exports.down = (pgm) => {
  switch (process.env.REALM_SENTRY_ENVIRONMENT) {
    ...
    case 'production':
      pgm.sql(
        "DELETE FROM realm_user_permissions WHERE realm_url = 'https://app.boxel.ai/boxel-homepage/' AND username IN ('@boxel_homepage_realm:boxel.ai', '@homepage_writer:boxel.ai', '@homepage_editor:boxel.ai', '*')",
      );
      break;
    default:
      pgm.sql(
