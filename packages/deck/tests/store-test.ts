...
module('store: content addressing and pruning', function (hooks) {
  ...
  test('identical content published twice stores ONE tree', async function (assert) {
    ...
    assert.strictEqual(meta!.versions['1.0.0-dev.1'].storage, 'blobs-v1');
  });

  test('concurrent local publishers preserve both Version pointers', async function (assert) {
    let first = pack(libFiles('1.0.0', 'export const v = 1;\n'));
    let second = pack(libFiles('1.0.0', 'export const v = 2;\n'));
    await Promise.all([
      publishToStore(storeDir, 'lib', '1.0.0-dev.1', first),
      publishToStore(storeDir, 'lib', '1.0.0-dev.2', second),
    ]);
    assert.deepEqual(
      Object.keys((await readStoreMeta(storeDir, 'lib'))!.versions).sort(),
  ...
  test('rejects metadata from a non-current store protocol', async function (assert) {
      ...
      JSON.stringify({
        ...
        versions: {
          '1.0.0': {
            ...
            file: 'packs/claimed.pack.zip',
          },
        },
        tags: {},
      }),
    );
    assert.strictEqual(await readStoreMeta(storeDir, 'lib'), undefined);
  });
  ...
  // A list of versions is only ordered history if the versions carry times, and
  ...
  test('a version records when it landed here and when upstream published it', async function (assert) {
    ...
    let record = (await readStoreMeta(storeDir, 'lib'))!.versions[
      '1.0.0-dev.1'
    ];
    assert.strictEqual(
      record.publishedAt,
    ...
    assert.strictEqual(released.treeHash, record.treeHash, 'and still no copy');
  ...
  // Optional clocks stay absent. The alternatives — the file's mtime or the
  // moment it was first read — would make at-rest state answer differently
  ...
  test('a record written without times reads back without times', async function (assert) {
    ...
    await writeFile(
      ...
      JSON.stringify({
        name: 'lib',
        versions: {
          '1.0.0': { treeHash: 'a'.repeat(64), storage: 'blobs-v1' },
        },
        tags: {},
      }),
    );
    ...
    assert.strictEqual(record.storage, 'blobs-v1');
  });
});
