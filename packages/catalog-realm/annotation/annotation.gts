...
export class AnnotationCard extends CardDef {
  ...
  static isolated = class Isolated extends Component<typeof AnnotationCard> {
    // ⁶ Isolated format
    ...
    startDrawing(event: MouseEvent) {
      if (this.selectedTool === 'drawing') {
        ...
        event.stopPropagation(); // ⁴⁶ Prevent event bubbling to text layer
        this.isDrawing = true;
        ...
        if (canvas) {
          ...
          // ⁴⁷ Store coordinates at display scale for consistency
          const x = event.clientX - rect.left;
          const y = event.clientY - rect.top;
          this.currentDrawingPath = [{ x, y }];

          console.log('Drawing started at display coordinates:', { x, y });
        }
      }
    }
    ...
    continueDrawing(event: MouseEvent) {
      if (this.isDrawing && this.selectedTool === 'drawing') {
        event.preventDefault();
        ...
        if (canvas) {
          ...
          // ⁴⁹ Use display coordinates consistently
          const x = event.clientX - rect.left;
          const y = event.clientY - rect.top;
    ...
    stopDrawing(event?: MouseEvent) {
      if (this.isDrawing && this.currentDrawingPath.length > 1) {
        // ⁶⁶ Convert display coordinates to normalized PDF coordinates for storage
        const canvas = document.getElementById(
          'pdf-canvas',
        ) as HTMLCanvasElement;
        if (!canvas) {
          ...
          this.cleanupDrawingPreviews(); // ⁸⁰ Clean up previews
          return;
        }

        const normalizedPoints = this.currentDrawingPath.map((point) => ({
          x: point.x / canvas.offsetWidth, // Normalize to 0-1 range
          y: point.y / canvas.offsetHeight,
        }));
        ...
        const drawing: DrawingAnnotation = {
          ...
          paths: [
            {
              points: normalizedPoints, // ⁶⁷ Store normalized coordinates
              color: this.selectedColor,
              thickness: this.drawingThickness / canvas.offsetWidth, // ⁶⁸ Normalize thickness too
            },
          ],
          pageNumber: this.currentPage, // ⁶⁹ CRITICAL: Store current page number
          createdAt: this.createTimestamp(),
          color: this.selectedColor,
        };

        console.log('Drawing completed with page info:', {
          drawing,
      ...
      // ⁸¹ CRITICAL: Always clean up drawing previews when stopping drawing
    ...
    private drawCurrentPath() {
      ...
      if (!ctx || this.currentDrawingPath.length < 2) return;

      // ⁵³ Don't interfere with PDF rendering - use overlay canvas approach
      ...
      // ⁵⁴ Remove any existing preview canvas
      const existingPreview =
        canvas.parentElement?.querySelector('.drawing-preview');
      ...
      // ⁵⁵ Scale the temporary canvas context to match device pixel ratio
      const devicePixelRatio = window.devicePixelRatio || 1;
      tempCtx.scale(devicePixelRatio, devicePixelRatio);

      tempCtx.strokeStyle = this.selectedColor;
      tempCtx.lineWidth = this.drawingThickness;
      tempCtx.lineCap = 'round';
      tempCtx.lineJoin = 'round';

      tempCtx.beginPath();
      tempCtx.moveTo(
        this.currentDrawingPath[0].x,
        this.currentDrawingPath[0].y,
      );

      for (let i = 1; i < this.currentDrawingPath.length; i++) {
        tempCtx.lineTo(
          this.currentDrawingPath[i].x,
          this.currentDrawingPath[i].y,
        );
      }
    ...
    private renderPage = task(async (pageNum: number) => {
      ...
      try {
        ...
        if (pageNum < 1 || pageNum > this.totalPages) {
          console.warn(
            `Page ${pageNum} is out of bounds (1-${this.totalPages})`,
          );
          return;
        }
    ...
    private redrawStoredAnnotations(pageNum: number) {
      ...
      drawings.forEach((drawing, index) => {
        ...
        drawing.paths.forEach((path, pathIndex) => {
          ...
          drawingCtx.stroke();
          console.log(
            `Drew path ${pathIndex} with ${path.points.length} points`,
          );
        });
      });
    ...
    // ⁷⁹ Clean up any drawing preview canvases that may be left over
    private cleanupDrawingPreviews() {
      const canvas = document.getElementById('pdf-canvas') as HTMLCanvasElement;
      if (!canvas || !canvas.parentElement) return;

      // Remove all drawing preview canvases
      const previewCanvases =
        canvas.parentElement.querySelectorAll('.drawing-preview');
      previewCanvases.forEach((preview) => {
        console.log('Removing leftover drawing preview canvas');
        preview.remove();
      });
    ...
    <template>
                ...
                <div class='color-strip'>
                  {{#each this.highlightColors as |color|}}
                    <button
                      class='color-chip
                        {{if
                          (eq this.selectedColor color.value)
                          "selected"
                          ""
                        }}'
                      style={{htmlSafe
            ...
            <div class='toolbar-selection'>
              <div class='selection-preview'>
                ...
                <span class='selected-text-preview'>
                  {{if
                    (gt this.selectedText.length 35)
                    (concat (slice this.selectedText 0 35) '...')
                    this.selectedText
                  }}
                </span>
              </div>
            ...
            {{else}}
              <div class='no-document'>
                ...
                <div class='upload-content'>
                  ...
                  <p class='upload-subtitle'>Select a PDF file from your
                    computer</p>
                        ...
                        {{#if (eq annotation.type 'highlight')}}
                          <div class='highlight-text'>
                            {{#if (gt annotation.text.length 120)}}
                              "{{concat (slice annotation.text 0 120) '...'}}"
                            {{else}}
                              "{{annotation.text}}"
                            {{/if}}
                          </div>
      ...
      <style scoped>
        ...
        /* ⁶³ Improved drawing mode isolation */
        ...
        /* ⁶⁴ Drawing layer sits above text layer when drawing */
        .drawing-layer {
          position: relative;
          z-index: 20;
        }
        ...
        /* ⁶⁵ Drawing overlay canvas styling */
        .drawing-overlay {
          position: absolute !important;
          top: 0 !important;
          left: 0 !important;
