...
export class CalendarCard extends CardDef {
  ...
  static isolated = class Isolated extends Component<typeof CalendarCard> {
    ...
    @cached
    get eventsOnDate() {
      const events = this.eventsResult?.instances || [];
    ...
    // ¹⁴ Get events for today - cached to prevent infinite renders
    @cached
    get todaysEvents() {
    ...
    get calendarDays() {
      ...
      for (let i = 0; i < 42; i++) {
        ...
        days.push({
          ...
          events: dayEvents, // Include events array for each day
        });
    ...
    deleteEvent(event: any) {
      try {
        if (this.args.model?.events) {
          const index = this.args.model.events.indexOf(event);
          if (index > -1) {
            this.args.model.events.splice(index, 1);
          }
        }
      } catch (e) {
        console.error('CalendarCard: Error deleting event', e);
      }
    }
        ...
        <header class='calendar-header'>
          ...
          <div class='calendar-controls'>
            ...
            <div class='view-selector' data-active={{this.viewMode}}>
              ...
              <Button
                class='view-button
                  {{if (eq this.viewMode "month") "active" ""}}'
              ...
              <div class='day-schedule'>
                ...
                {{#if (eq this.todaysEvents.length 0)}}
                  <div class='empty-day-overlay'>
                    ...
                    <p>This day is free! Click "Add Event" to schedule
                      something.</p>
                    <Button
                      class='add-event-button'
                      {{on 'click' this.addEvent}}
                    >
                      <svg
                        viewBox='0 0 24 24'
                        fill='none'
          ...
          <div class='more-events-modal'>
            ...
            <div class='modal-content'>
              <div class='modal-header'>
                <h3>{{formatCalendarDate
                    this.showMoreEventsFor.date
                    'long'
                  }}</h3>
                <button
                  class='close-button'
