      ...
      <div class='platform-card'>
        <div class='platform-header'>
          <div class='platform-info'>
            <h4 class='platform-name'>{{if
                @model.name
                @model.name
                'Unknown Platform'
              }}</h4>
            <span class='account-id'>{{if
                @model.accountId
                @model.accountId
                'Not connected'
              }}</span>
        ...
        <div class='platform-stats'>
          <span class='games-count'>{{if @model.gamesCount @model.gamesCount 0}}
            games</span>
          {{#if @model.lastSynced}}
            <span class='sync-time'>Last sync:
              {{formatDateTime @model.lastSynced size='short'}}</span>
          {{else}}
...
export class GameEntryField extends FieldDef {
  static displayName = 'Game Entry';
  static icon = GamepadIcon;
        ...
        <div class='game-info'>
          ...
          <div class='game-stats'>
            {{#if @model.hoursPlayed}}
              <span class='hours'>{{@model.hoursPlayed}}h played</span>
            {{/if}}
            {{#if @model.rating}}
              <span class='rating'>★ {{@model.rating}}/10</span>
            {{/if}}
            {{#if @model.achievements}}
              <span class='achievements'>🏆 {{@model.achievements}}</span>
            {{/if}}
          </div>
          ...
          <div class='game-status'>
            ...
            {{#if @model.isWishlisted}}
              <span class='wishlist-badge'>❤️ Wishlisted</span>
            {{/if}}
          </div>
        </div>
      </div>
...
export class TournamentField extends FieldDef {
  static displayName = 'Tournament';
  static icon = TrophyIcon;

  @field name = contains(StringField);
  @field game = contains(StringField);
    ...
    <template>
      <div class='tournament-card'>
        ...
        <div class='tournament-game'>
          {{if @model.game @model.game 'Game TBD'}}
        </div>
        ...
        <div class='tournament-details'>
          {{#if @model.prizePool}}
            <div class='prize-pool'>
              ...
              {{formatCurrency @model.prizePool currency='USD'}}
            </div>
          {{/if}}
          ...
          {{#if @model.startDate}}
            <div class='start-date'>
              ...
              {{formatDateTime @model.startDate size='short'}}
            </div>
          {{/if}}
        </div>
