...
import { RewardCardSystem, Benefit } from './reward-card-system';
...
export class RewardSystemTracker extends CardDef {
  static displayName = 'Reward System Tracker';

  @field cardholderName = contains(StringField);
  @field cardSystem = linksTo(RewardCardSystem);
  ...
  static isolated = class Isolated extends Component<typeof this> {
    ...
    get annualFee() {
      return this.args.model?.cardSystem?.annualFee?.amount ?? 0;
    }
    ...
    get cardTitle() {
      const name = this.args.model?.cardSystem?.cardName ?? 'Card';
      return `${name} Benefit Tracker`;
    }

    get cardThumbnail() {
      return this.args.model?.cardSystem?.thumbnailURL ?? null;
    }

    get currencyCode() {
      return this.args.model?.cardSystem?.annualFee?.currency?.code ?? 'USD';
    }
        ...
        <section class='summary-grid'>
          ...
          <div class='stats-grid'>
            ...
            <div class='stat-card'>
              ...
              <div class='stat-sub'>{{@model.cardSystem.cardName}}</div>
            </div>
          </div>
        </section>
