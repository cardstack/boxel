...
// openkit/controls 1.1.0 — the last border leaves the building.
//
// BEHAVIOUR UNCHANGED from 1.0.0: still portalled to <body>, still anchored by
// floating-ui, still multiple selection with chips, still a travelling
// highlight. This pass finishes an argument 1.0.0 started and left half-done.
//
// 1.0.0's popup already drew its hairline as a shadow, with a comment saying
// why: a surface that can never leave its outline behind. The TRIGGER, three
// hundred lines up, still used `border: 1px solid`. So the control that opens
// the surface was outlined by a different mechanism than the surface itself —
// two ladders, and at 1px nobody notices until they try to animate one.
//
// Now the trigger, the checkbox and the popup are all shadow, the open trigger
// grows the same 3px focus halo the rest of the system uses, and the selected
// checkbox gets the pressed-metal top edge that marks a primary control.
...
export default class Select extends Component<SelectSignature> {
  ...
  <template>
    ...
    <style scoped>
      ...
      /* The last `border` in the component, and now it is gone: the popup
         already draws its hairline as a shadow, and a trigger outlined in a
         different mechanism than the surface it opens reads as a different
         family of control. One ladder, one decision — plus the soft inset that
         says "you can type here" on every recessed field in the system. */
      .trigger {
        display: flex;
        align-items: center;
        ...
        color: var(--ok-ink);
        box-shadow:
          0 0 0 1px var(--ok-line),
          var(--pretui-shadow-inset, inset 0 1px 2px rgb(0 0 0 / 0.06));
        font: inherit;
        text-align: left;
        cursor: pointer;
        transition: box-shadow var(--ok-motion);
      }
      .trigger[aria-expanded='true'] {
        box-shadow:
          0 0 0 1px var(--ok-accent),
          0 0 0 3px color-mix(in srgb, var(--ok-ring) 35%, transparent),
          var(--pretui-shadow-inset, inset 0 1px 2px rgb(0 0 0 / 0.06));
      }
      ...
      .option.is-selected .box {
        ...
        box-shadow:
          0 0 0 1px color-mix(in srgb, var(--ok-accent) 70%, var(--ok-line)),
