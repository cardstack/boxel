...
// iso/money-codes 1.3.0 — the code becomes a pill.
//
// THE CODE LIST IS UNCHANGED. Same records, same `minorUnits`, same `isActive`,
// same successor chain. What changes is one line of reading: a currency code is
// a machine value — something a reader may retype, or paste into a query — and
// in this system machine values wear a mono, accent-tinted, ringed pill. The
// gloss beside it stays prose, so the boundary between "retype this" and "read
// this" is visible without being explained.
//
// Withdrawn codes keep their strike and LOSE the accent: still a machine value,
// no longer one to reach for. That is the whole reason the treatment is derived
// from a state rather than hardcoded per code.
    ...
    <template>
      ...
      <style scoped>
        .wrap {
          ...
          --iso-line: var(--border, #e3e5ec);
          --iso-surface: var(--card, #ffffff);
          --iso-accent: var(--primary, #3d6bff);
          --iso-accent-ink: var(--pretui-primary-ink, var(--primary, #3d6bff));
        ...
        /* The IDENTIFIER. Mono, tabular, never wraps, never yields — it is the
           ...
           Law 3 — a machine value looks like a machine value: the code wears
           the accent-tinted pill every id, version and file name wears, so a
           reader can tell at a glance which half of this field is retypable
           and which half is prose. */
        .code {
          flex: none;
          ...
          background: color-mix(in srgb, var(--iso-accent) 8%, var(--iso-surface));
          color: color-mix(in srgb, var(--iso-ink) 22%, var(--iso-accent-ink));
          box-shadow: 0 0 0 1px
            color-mix(in srgb, var(--iso-accent-ink) 28%, var(--iso-line));
        }
        ...
        /* Withdrawn keeps the strike AND loses the accent: a retired code is
           still a machine value, but it is no longer one to reach for. */
        .is-withdrawn .code {
          background: var(--inset, #f2f3f7);
          color: var(--iso-ink-2);
          box-shadow: 0 0 0 1px var(--iso-line);
