...
test.describe('Room messages', () => {
  ...
  test('currently viewed file is auto-attached', async ({ page }) => {
    ...
    expect(attachedFiles[0].url).toMatch(
      /^http:\/\/localhost:8009\/_matrix\/client\/v1\/media\/download\/localhost\/[A-Za-z0-9]+$/,
    ); // Example http://localhost:8009/_matrix/client/v1/media/download/localhost/phPajGXxttKlRxWYYjYLTkFP (generated by matrix storage)
  });
