...
test.describe('Room messages', () => {
  ...
  test('currently viewed file is auto-attached', async ({ page }) => {
    ...
    expect(attachedFiles[0].url).toMatch(
      /^http:\/\/localhost:8008\/_matrix\/media\/v3\/download\/localhost\/[A-Za-z0-9]+$/,
    ); // Example http://localhost:8008/_matrix/media/v3/download/localhost/eHDVqYSIxXpMKilfqCANpGej (generated by matrix storage)
  });
  ...
  test('ensure attached files are sent with the message', async ({ page }) => {
    ...
    await page.locator(`[data-test-attached-file="${appURL}/person.gts"] [data-test-remove-file-btn]`).click();
    await expect(
      page.locator(`[data-test-attached-file="${appURL}/person.gts"]`),
    ).toHaveCount(0);
    ...
    let attachedFiles = JSON.parse(lastMessage.content.data).attachedFiles;
    expect(attachedFiles.length).toStrictEqual(1);
    expect(attachedFiles[0].name).toStrictEqual('pet.gts');
    expect(attachedFiles[0].contentType).toStrictEqual(
      'text/plain;charset=UTF-8',
    );
    expect(attachedFiles[0].sourceUrl).toStrictEqual(`${appURL}/pet.gts`);
    expect(attachedFiles[0].url).toMatch(
      /^http:\/\/localhost:8008\/_matrix\/media\/v3\/download\/localhost\/[A-Za-z0-9]+$/,
    ); // Example http://localhost:8008/_matrix/media/v3/download/localhost/eHDVqYSIxXpMKilfqCANpGej (generated by matrix storage)
  });
