...
import { readCard, writeCard } from '../src/workspace-fs';
...
test('update_project writes and reads back a project card', async ({
  ...
  try {
    ...
    let readResult = (await readCard(
      workspace.dir,
      'Projects/tool-test-project.json',
    )) as CardReadResult;
...
test('add_comment appends a comment to an existing issue without changing other fields', async ({
  ...
  try {
    ...
    // Pre-seed an issue directly into the workspace (descriptions are set
    // at creation time, not via update_issue which strips them).
    ...
    let createResult = (await writeCard(
      workspace.dir,
...
test('create_knowledge writes and reads back a knowledge article', async ({
  ...
  try {
    ...
    let readResult = (await readCard(
      workspace.dir,
      'Knowledge Articles/tool-test-article.json',
    )) as CardReadResult;
