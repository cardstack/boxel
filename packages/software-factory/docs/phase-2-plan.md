...
**All `linkedExamples` are instantiated, not just the first.** For each Spec, the step reads every entry in the `linkedExamples` relationship (using the Boxel dotted-key format `linkedExamples.0`, `linkedExamples.1`, etc.), resolves relative `adoptsFrom.module` paths to absolute URLs using `codeRefWithAbsoluteURL` logic, and instantiates all examples in parallel via `Promise.allSettled`. If a Spec has no linked examples, the host command builds a minimal document with just `adoptsFrom` and empty attributes — this still validates that the card class can be loaded and an empty instance deserialized.
...
The agent has a `fetch_transpiled_module` factory tool that GETs the module URL with `Accept: */*` (compared to `read_file` which uses `application/vnd.card+source` for the raw `.gts`). The underlying function is `readTranspiledModule()` in `src/realm-operations.ts` — it strips a trailing `.gts` from the path (the realm resolves modules without the extension) and returns the compiled JS as text. The `software-factory-operations` skill teaches the agent to recognize line/column references in eval/instantiate failures and reach for this tool when debugging. The agent still edits the `.gts` source — the transpiled output is regenerated on every write.
