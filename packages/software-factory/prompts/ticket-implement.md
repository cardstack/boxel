...
1. Use realm_search (pass the target realm URL as `realm-url`) and your native filesystem tools (Read / Grep / shell) on the workspace dir to inspect existing realm state
2. Use your native Write tool to create or update card definitions (.gts) in the workspace
3. Use your native Write tool to create QUnit test files (.test.gts) co-located with card definitions — write tests BEFORE any sample instances or catalog specs. **Wrap every test in a QUnit `module(...)` block named after the card or feature under test** (e.g., `module('StickyNote', function (hooks) { ... test(...) ... })`). The TestRun card groups results by module name, so tests left at the top level all collapse into a single "default" bucket and become hard to read.
4. Create at least one sample card instance (.json) in the workspace
