...
let indexContents = componentsToGenerate
  .map((c) => `import ${c.name} from './${c.outFile}';`)
  .join('\n');
indexContents += '\n\n';
let componentNameArray = componentsToGenerate.map((c) => c.name);
indexContents += `export const ALL_ICON_COMPONENTS = [ ${componentNameArray.join(
  ',\n         ',
)}\n];\n`;
fs.writeFileSync(path.resolve(destDir, 'index.gts'), indexContents);
