  ...
  client.on(
    ...
    async function (event, room, toStartOfTimeline) {
      try {
        ...
        // Perform async operation - iterate 10 times, logging time and sleeping
        ...
        for (let i = 0; i < 10; i++) {
          ...
          let result = await fetch(
            `${MATRIX_URL}/_matrix/client/v3/rooms/${encodeURIComponent(room.roomId)}/send/m.room.message/${txnId}`,
            {
              method: 'PUT',
              headers: {
