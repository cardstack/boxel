...
export function getLinksToManyComponent({
  ...
  // query-backed graph it is the combinatorial breadth that the ancestry cycle
  // guard can't cut (it stops spine re-entries, not the same instance rendered
  // along many sibling paths). Emit each result as a bounded atom instead: no
  // embedded render, so no recursion into the result's own links/query fields.
  // Gated on `__boxelJobId` (not just the render flag) so it applies only to
  // indexing renders whose HTML is stored-then-rehydrated; the SPA and
  // non-indexing prerenders deep-render as before.
