...
function getConstrainedImageSize(maxHeight: number) {
  return class ConstrainedSize extends Component<typeof Base64ImageCard> {
    get height() {
...
class Fitted extends Component<typeof Base64ImageCard> {
  get imageStyle() {
    let css: string[] = [];
    if (this.args.model.size === 'contain') {
      css.push('object-fit: contain;');
    } else if (this.args.model.size === 'cover') {
      css.push('object-fit: cover;');
    } else if (this.args.model.size === 'actual') {
      css.push('object-fit: none;');
    }
    return htmlSafe(css.join(' '));
  }
  <template>
