...
// ============================================================================
// Searchable-driven search-doc generation (CS-11722) — NON-AUTHORITATIVE.
//
// Parallel to `searchDoc` above, which stays in charge of production indexing
// until the cutover (CS-11724). This path derives link depth from the explicit
// `field.searchable` annotation (CS-11721) instead of from what the render
...
// `BaseDef[queryableValue]`. The declared target type is enumerated (not the
// runtime subtype), which drops the unqueryable polymorphic-subtype bloat.
...
// broken / unloadable); the expanded declared-type target when a route names
