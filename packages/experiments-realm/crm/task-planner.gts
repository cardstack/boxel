...
import { CRMTaskStatusField, CRMTask } from './crm-task';
...
export class CRMTaskPlannerIsolated extends Component<typeof CRMTaskPlanner> {
  ...
  get cardInstances() {
    ...
    return this.cards.instances as CRMTask[];
  }
  ...
  @action showTaskCard(card: CRMTask): boolean {
  ...
  hasColumnKey = (card: CRMTask, key: string) => {
  ...
  taskCollection = getKanbanResource(
    this,
    () => this.cardInstances,
    () => CRMTaskStatusField.values.map((status) => status.label) ?? [],
    () => this.hasColumnKey,
  );
  ...
  get getTaskQuery(): Query {
    ...
    // Use the model (CRM App) passed from parent
    if (!this.args.model?.id) {
      console.log('No CRM App');
      everyArr.push({ eq: { 'crmApp.id': null } });
    } else {
      everyArr.push({ eq: { 'crmApp.id': this.args.model.id } });
    }
