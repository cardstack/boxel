...
import { EMPLOYEE_STATUS_COLORS } from '../trt-employee';
...
class OrgTreeNode extends GlimmerComponent<OrgTreeNodeSignature> {
  ...
  get childrenStyle() {
    // Animate expand/collapse via max-height instead of an instant {{#if}}
    // snap — grid-template-rows would be cleaner but max-height on a fixed
    // cap reads fine for org-chart depths (a handful of direct reports).
    return htmlSafe(
      this.expanded
        ? 'max-height: 62.5rem; opacity: 1;'
  ...
  <template>
    <li class='org-node'>
      <div class='org-row'>
        ...
        {{#if this.hasChildren}}
          <span class='report-count'>{{this.reportCount}}</span>
        {{/if}}
      </div>
      {{#if this.hasChildren}}
        <ul class='org-children' style={{this.childrenStyle}}>
