...
import { Paginator } from './fulfilment-paginator';
import { FieldRenderer } from './fulfilment-field-renderer';
import { getFieldsResource } from './fulfilment-get-fields-resource';
            ...
            {{#if field}}
              ...
              {{else}}
                {{#if (eq field.fieldType 'contains')}}
                  {{#if field.component}}
                    {{! DIVERGENCE from the upstream copy, deliberate: upstream
                        renders `@format='edit'` here even when `showClean` is
                        true, so a browse table comes up full of live inputs and
                        an accidental keystroke edits a card. `showClean` is
                        documented as "read-only cells", and `atom` is the format
                        built for a compact single value in a cell. }}
                    <field.component @format='atom' @displayContainer={{false}} />
                  {{else}}
                    {{field.value}}
                  {{/if}}
                {{else if (eq field.fieldType 'linksTo')}}
                  {{#if field.value}}
                    ...
                    >
                      {{! the linked card's OWN atom, rather than `.title` —
                          cards whose display name is the computed `cardTitle`
                          leave `.title` empty, which is what rendered every
                          linked cell as the literal "[linked card]".
                          displayContainer is false because a card atom
                          otherwise brings its own CardContainer boundaries,
                          which inside this Pill reads as a box around a pill. }}
                      <field.component @format='atom' @displayContainer={{false}} />
                    </Pill>
                  {{/if}}
                ...
                {{else if (eq field.fieldType 'linksToMany')}}
                  {{#each field.value as |linkedCard|}}
                    ...
                    >
                      {{! same reason as the linksTo branch above: `.title` is
                          empty on cards that compute `cardTitle` }}
                      {{if linkedCard.title linkedCard.title linkedCard.cardTitle}}
                    </Pill>
                  {{/each}}
                {{else}}
                  {{field.value}}
