...
//   model.screenshotURLs.hero    ->  durable PNG URL (undefined until captured)
//   {realm}_screenshot/{path}?name=hero`;
...
{realm}_screenshot/{path}                      # PNG, isolated (default)
{realm}_screenshot/{path}?format=embedded      # embedded format
{realm}_screenshot/{path}?viewport=1280x800&dsf=2   # retina, sized
...
class Isolated extends Component<typeof ScreenshotsPdfWalkthrough> {
  get targetCard() {
  ...
  private get base(): string | undefined {
    ...
    return `${realm}_screenshot/${id.slice(realm.length)}`;
  }
...
export class ScreenshotsPdfWalkthrough extends CardDef {
  static displayName = 'Screenshots & PDF Walkthrough';
  static icon = Camera;
