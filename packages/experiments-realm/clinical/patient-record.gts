...
export class PatientRecord extends CardDef {
  ...
  // The append-only ledger the batches write a line to. Naming `LogFile` here
  // is what gives the batch handle its `record` member: a batch's members come
  // from the type the field declares, and the platform types every stored
  // `.log` as `LogFile`, so the name resolves on both sides.
