...
A Boxel card (`PasswordGameForm`) modeled after neal.fun's _The Password
Game_ — except instead of a single password field, you must fill out a
...
You _start_ with rule 1 visible. Each time the visible rules all pass,
...
- The UI walks them in order. It shows:
  - all rules up to and including the first failing one, _plus_ all
...
2. A per-rule **failing fixture** (the canonical solution with _one_
...
The same `RULES` array can be pasted into the `.gts`. The canonical
...
- **Boolean-valued.** Every rule is a single pure boolean expression.
  ...
  `.gts` computed fields (which are _not_ in scope here).
