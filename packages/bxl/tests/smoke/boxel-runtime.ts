...
strictEqual(
  first.annotationCards[0]?.entries[0]?.targetPath,
  '.matchingProgram',
  'annotation drafts should preserve target paths',
);

const second = session.applyPatch('.amount', 1200);
strictEqual(
...
const third = session.applyPatch('.payment', 'Credit card');
strictEqual(
...
const fifth = session.applyPatch('.amount', 100);
strictEqual(
...
console.log('Boxel runtime smoke passed');
