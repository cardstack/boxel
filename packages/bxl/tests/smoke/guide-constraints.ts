...
import { evaluateBxl, type ReadableSchema } from '../../src/index.ts';
...
const benjamin = {
  donor: 'Benjamin Chen',
  email: 'benjamin@chen.family',
  amount: 260,
  currency: null,
  campaign: 'Spring Annual Drive — ends Jun 30',
  payment: 'Credit Card',
  anonymous: false,
  match: null,
  note: null,
...
const grace = {
  donor: 'Grace Lin',
  email: 'grace.lin@harborschool.org',
...
const cases: Case[] = [
  {
    ...
    expr: 'Campaign <> null AND Campaign <> ""',
    grace: true,
    benjamin: true,
  },
  {
    name: 'campaign not archived',
    expr: 'NOT ((Campaign // "") | startswith("Archived"))',
    grace: true,
    benjamin: true,
  },
  ...
  {
    name: 'whole-dollar',
    expr: 'Amount = ROUND(Amount, 0)',
    grace: true,
    benjamin: true,
  },
  {
    name: '>=1000 requires note',
    expr: '(Amount < 1000) OR ((Note // "") | length) > 0',
    grace: true,
    benjamin: true,
  },
  ...
  {
    name: 'phone length',
    expr: 'Phone = null OR Phone = "" OR LEN(Phone) >= 7',
    grace: true,
    benjamin: true,
  },

  {
    name: 'anonymous-major check',
    expr: 'Anonymous <> true OR Amount < 5000',
    grace: true,
    benjamin: true,
  },

  {
    name: 'note length cap',
    expr: 'Note = null OR LEN(Note) <= 500',
    grace: true,
    benjamin: true,
  },
];
...
for (const c of cases) {
  for (const [name, input, expected] of [
    ...
    try {
      ...
      console.log(
        `${pass ? 'OK  ' : 'FAIL'} ${name.padEnd(8)} ${c.name.padEnd(35)} → ${got}${pass ? '' : ` (expected ${expected})`}`,
      );
      if (!pass) fail++;
    } catch (e) {
      console.log(
        `FAIL ${name.padEnd(8)} ${c.name.padEnd(35)} threw: ${(e as Error).message}`,
      );
