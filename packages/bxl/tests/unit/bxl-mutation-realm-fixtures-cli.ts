...
import { verifyMutationFixture } from '../../examples/bxl-mutation-fixture-runner.ts';

if (realmMutationExamples.length < 6) {
  throw new Error(
    `Expected at least 6 realm-shaped mutation fixtures, received ${realmMutationExamples.length}`,
  );
}
...
const missingEvidence = realmMutationExamples.filter((fixture) => {
  ...
  return (
    !('sourceEvidence' in schema) || typeof schema.sourceEvidence !== 'string'
  );
});
if (missingEvidence.length > 0) {
  throw new Error(
    `Realm mutation fixtures lack source evidence: ${missingEvidence.map((fixture) => fixture.id).join(', ')}`,
  );
}
...
const nonportableEvidence = realmMutationExamples.filter((fixture) => {
  ...
  return (
    evidence.startsWith('/') ||
    evidence.startsWith('~') ||
    evidence.includes('/Users/')
  );
});
if (nonportableEvidence.length > 0) {
  throw new Error(
    `Realm mutation source evidence must be workspace-relative: ${nonportableEvidence.map((fixture) => fixture.id).join(', ')}`,
  );
}
...
const accepted = realmMutationExamples.filter(
  (fixture) => fixture.outcome === 'accepted',
).length;
const rejected = realmMutationExamples.length - accepted;
if (accepted < 4 || rejected < 2) {
  throw new Error(
    `Realm mutation seam must retain accepted writes and rejected boundaries; received ${accepted}/${rejected}`,
  );
}
const rejectionCodes = new Set(
  realmMutationExamples.flatMap((fixture) =>
