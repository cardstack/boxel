...
would mean diverging from Excel. `YEARFRAC`'s basis 0 does apply a February
rule, which is not a contradiction — the two are separately observed functions.
...
30/360 count reads a February start a third way again. Three implementations of
one nominal convention, so a change to any of them settles nothing about the
others.
...
and `ACCRINT` parting from it on basis 4 for that reason. `fsprojects/ExcelFinancialFunctions`
counts with its clamped `dateDiff360Eu` throughout, which is the shape here.
