  ...
  return new Proxy(
    {},
    {
      ...
      getOwnPropertyDescriptor(_facade, prop) {
        const desc = Reflect.getOwnPropertyDescriptor(target, prop);
        const keys = resolveFieldKeys();
        // Path reads can name a declared link even when FieldDef enumeration
        // omits links from the stored attributes shape.
        const isField =
          typeof prop === 'string' &&
          (fieldDef ? !!resolveFieldMap()?.[prop] : !!keys?.includes(prop));
        if (fieldDef && keys && !isField) {
          return undefined;
        }
