...
function record(value: unknown, label: string, statement: number): RecordValue {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new BxlMutationError('parse', 'operation-shape', statement, `${label} must be an object.`);
  }
  return value as RecordValue;
}

function path(value: unknown, label: string, statement: number): BxlMutationPath {
  if (!Array.isArray(value) || !value.every((part) => typeof part === 'string' || (typeof part === 'number' && Number.isInteger(part)))) {
    throw new BxlMutationError('parse', 'operation-path', statement, `${label} must be an array of string/integer path parts.`);
  }
...
function pathSource(parts: BxlMutationPath): string {
  ...
  for (const part of parts) {
    ...
    else if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(part)) source += source === '.' ? part : `.${part}`;
...
function jsonSource(value: unknown, label: string, statement: number): string {
  ...
  if (source === undefined) {
    throw new BxlMutationError('parse', 'operation-value', statement, `${label} must be JSON serializable.`);
  }
  return source;
}
...
function selectorPredicate(where: unknown, statement: number): string {
  if (!Array.isArray(where) || where.length === 0) {
    throw new BxlMutationError('parse', 'operation-selector', statement, 'Selector where must contain at least one equality term.');
  }
  return where.map((term, index) => {
    const condition = record(term, `where[${index}]`, statement);
    if (!('equals' in condition)) {
      throw new BxlMutationError('parse', 'operation-selector', statement, 'Version 1 structured selectors require equals.');
    }
    return `${pathSource(path(condition.path, `where[${index}].path`, statement))}==${jsonSource(condition.equals, 'equals', statement)}`;
  }).join(' and ');
}

function targetSource(targetValue: unknown, statement: number, bulk = false): string {
  const target = record(targetValue, 'target', statement);
  if ('path' in target) return pathSource(path(target.path, 'target.path', statement));
  ...
  const relative = path(target.relativePath ?? [], 'target.relativePath', statement);
  if (bulk) {
...
function expressionOrValue(operation: RecordValue, statement: number): string {
  ...
  if ('value' in operation) return jsonSource(operation.value, 'operation.value', statement);
  throw new BxlMutationError('parse', 'operation-value', statement, 'Operation requires value or expression.');
}
...
function positionCall(
  ...
  if (typeof position.index === 'number') return `insert_at(${collection};${position.index};${value})`;
  if (position.before) return `insert_item_before(${value};${targetSource(position.before, statement)})`;
  if (position.after) return `insert_item_after(${value};${targetSource(position.after, statement)})`;
  throw new BxlMutationError('parse', 'operation-position', statement, 'Unsupported insertion position.');
}

function moveCall(operation: RecordValue, statement: number): string {
  const item = targetSource(operation.target, statement);
  const collection = pathSource(path(record(operation.into, 'into', statement).path, 'into.path', statement));
  const position = record(operation.position, 'position', statement);
  if (position.at === 'start') return `move_item_to_start(${item};${collection})`;
  if (position.at === 'end') return `move_item_to_end(${item};${collection})`;
  if (position.before) return `move_item_before(${item};${targetSource(position.before, statement)})`;
  if (position.after) return `move_item_after(${item};${targetSource(position.after, statement)})`;
  throw new BxlMutationError('parse', 'operation-position', statement, 'Unsupported move position.');
}

function relationSelector(target: unknown, cardId: string, statement: number): string {
...
function relationshipType(
  ...
  for (const part of parts) {
    ...
    if (field.fieldType === 'linksTo' || field.fieldType === 'linksToMany') return field.fieldType;
...
function operationSource(
  ...
  switch (operation.op) {
    case 'assert':
      if (typeof value.expression !== 'string' || typeof value.message !== 'string') {
        throw new BxlMutationError('parse', 'operation-value', statement, 'assert requires string expression and message fields.');
      }
    ...
    case 'update':
      if (typeof value.expression !== 'string') throw new BxlMutationError('parse', 'operation-value', statement, 'update requires expression.');
      return `${targetSource(value.target, statement)}|=${value.expression}`;
    case 'update-all':
      if (typeof value.expression !== 'string') throw new BxlMutationError('parse', 'operation-value', statement, 'update-all requires expression.');
    ...
    case 'insert': {
      const collection = pathSource(path(record(value.into, 'into', statement).path, 'into.path', statement));
      return positionCall(value, expressionOrValue(value, statement), collection, statement);
    }
    case 'move':
    ...
    case 'relate': {
      if (typeof value.cardId !== 'string') throw new BxlMutationError('parse', 'operation-card-id', statement, 'relate requires cardId.');
      ...
      if (relationshipType(value.target, options, statement) === 'linksToMany') {
        return `append(${target};${card})`;
      }
      return `${target}=${card}`;
    }
    case 'unrelate':
      if (typeof value.cardId !== 'string') throw new BxlMutationError('parse', 'operation-card-id', statement, 'unrelate requires cardId.');
      return `del(${relationSelector(value.target, value.cardId, statement)})`;
    case 'move-relation': {
      if (typeof value.cardId !== 'string') throw new BxlMutationError('parse', 'operation-card-id', statement, 'move-relation requires cardId.');
      ...
      if (position.at === 'start') return `move_item_to_start(${item};${field})`;
      if (position.at === 'end') return `move_item_to_end(${item};${field})`;
      throw new BxlMutationError('parse', 'operation-position', statement, 'Unsupported relationship move position.');
    }
  }
}
...
export function solidifyBxlMutationOperations(
  ...
  if (operations.length === 0) {
    throw new BxlMutationError('parse', 'program-empty', 1, 'Structured mutation program must contain operations.');
  }
  const ids = new Set<string>();
  return operations.map((operation, index) => {
    const statement = index + 1;
    if (!operation.id) throw new BxlMutationError('parse', 'operation-id-required', statement, 'Every operation requires a stable id.');
    if (ids.has(operation.id)) throw new BxlMutationError('parse', 'duplicate-operation-id', statement, `Duplicate operation id ${JSON.stringify(operation.id)}.`);
...
export function prepareBxlMutationOperations(
  ...
  const prepared = prepareBxlMutation(source, { ...options, syntax: 'solidified' });
  return Object.freeze({
    ...prepared,
    language: 'bxl-mutation-ops/1' as const,
    source: JSON.stringify(operations),
    plan(snapshot: BxlMutationJson, planOptions: Parameters<PreparedBxlMutation['plan']>[1]): BxlMutationPlan {
      return {
