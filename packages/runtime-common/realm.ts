...
export class Realm {
  ...
  private async checkPermission(
    ...
    let isPublicReadable = await realmPermissionChecker.can('*', 'read');
    let isPublicWritable = await realmPermissionChecker.can('*', 'write');
    if (
      (neededPermission === 'read' && isPublicReadable) ||
      (neededPermission === 'write' && isPublicWritable)
    ) {
      return;
    }
