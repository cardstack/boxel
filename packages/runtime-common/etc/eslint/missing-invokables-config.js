/* eslint-disable no-undef */
'use strict';

module.exports = {
  invokables: {
    add: ['add', '@cardstack/boxel-ui/helpers'],
    and: ['and', '@cardstack/boxel-ui/helpers'],
    array: ['array', '@ember/helper'],
    bool: ['bool', '@cardstack/boxel-ui/helpers'],
    concat: ['concat', '@ember/helper'],
    divide: ['divide', '@cardstack/boxel-ui/helpers'],
    enumOptions: ['enumOptions', '@cardstack/base/enum'],
    enumValues: ['enumValues', '@cardstack/base/enum'],
    eq: ['eq', '@cardstack/boxel-ui/helpers'],
    fn: ['fn', '@ember/helper'],
    formatDateTime: ['formatDateTime', '@cardstack/boxel-ui/helpers'],
    get: ['get', '@ember/helper'],
    gt: ['gt', '@cardstack/boxel-ui/helpers'],
    gte: ['gte', '@cardstack/boxel-ui/helpers'],
    hash: ['hash', '@ember/helper'],
    lt: ['lt', '@cardstack/boxel-ui/helpers'],
    lte: ['lte', '@cardstack/boxel-ui/helpers'],
    multiply: ['multiply', '@cardstack/boxel-ui/helpers'],
    not: ['not', '@cardstack/boxel-ui/helpers'],
    on: ['on', '@ember/modifier'],
    or: ['or', '@cardstack/boxel-ui/helpers'],
    substring: ['substring', '@cardstack/boxel-ui/helpers'],
    subtract: ['subtract', '@cardstack/boxel-ui/helpers'],
  },
};
