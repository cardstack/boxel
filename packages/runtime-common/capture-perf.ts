// Per-capture stage telemetry for the capture pipeline.
//
// One JSON-object log line per event on the `boxel:capture-perf` channel —
...
export type CaptureRequestSurface = 'get-dsl' | 'get-named' | 'post';
...
export type CaptureRequestOutcome =
  ...
  | 'timeout'
  // 403: the realm's `allowArbitraryCaptures` gate is closed.
  | 'gated'
...
export interface CaptureRequestPerfEvent extends CapturePerfBase {
  eventType: 'request';
  outcome: CaptureRequestOutcome;
  ...
  // GET only: reading the realm's `allowArbitraryCaptures` config.
  gateMs?: number;
...
export interface CaptureRunPerfEvent extends CapturePerfBase {
  eventType: 'capture';
  ...
  persistMs?: number;
  persistOutcome: CapturePersistOutcome;
}

export type CapturePerfEvent = CaptureRequestPerfEvent | CaptureRunPerfEvent;

export const CAPTURE_PERF_CHANNEL = 'boxel:capture-perf';
...
// stdout.
let capturePerfSink: ((event: CapturePerfEvent) => void) | undefined;

export function setCapturePerfSink(
  sink: ((event: CapturePerfEvent) => void) | undefined,
): void {
  capturePerfSink = sink;
}
...
let capturePerfLog: ReturnType<typeof logger> | undefined;

export function emitCapturePerf(event: CapturePerfEvent): void {
  if (capturePerfSink) {
    capturePerfSink(event);
    return;
  }
  (capturePerfLog ??= logger(CAPTURE_PERF_CHANNEL)).info(
    JSON.stringify({ channel: CAPTURE_PERF_CHANNEL, ...event }),
  );
}
