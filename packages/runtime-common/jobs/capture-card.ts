...
// so the marginal per-entry cost (viewport resize + capture) is small, and
// the batch ceiling (`CAPTURE_MAX_BATCH_ENTRIES`) is sized to finish well within
...
// ones; the `sameCaptureIdentity` compare below is belt-and-braces against a
...
export function chooseCaptureCardCoalesceDecision(
  ...
  let twin = [...candidates, ...inFlightCandidates].find((candidate) => {
    ...
    return (
      ...
      sameCaptureIdentity(
        candidateArgs.captureSpec,
        incomingArgs.captureSpec,
      ) &&
...
// `parseCaptureRequestSpec` (defaults elided), so a default-valued field
// never appears on one side only. Total over `CaptureRequestSpec`, not
...
// and `rest` is asserted empty, so a field added to `CaptureRequestSpec`
...
function sameCaptureIdentity(
  a: CaptureRequestSpec | null | undefined,
  b: CaptureRequestSpec | null | undefined,
): boolean {
  if (!a || !b) {
    return !a && !b;
  }
...
function samePersist(
  a: CapturePersistArgs | null | undefined,
  b: CapturePersistArgs | null | undefined,
): boolean {
  if (!a || !b) {
    return !a && !b;
  }
...
// when the realm's serialized capture lane is already deep enough that a
...
// pre-check.
export async function estimateCaptureQueueWait(
  dbAdapter: DBAdapter,
