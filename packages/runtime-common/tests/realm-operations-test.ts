...
import type { RealmClient } from '../realm-client.ts';
...
const tests = Object.freeze({
  'publishRealm POSTs normalized URLs and maps the 202 response': async (
    assert,
  ) => {
  ...
  'publishRealm throws a RealmOperationError carrying the conflict status':
    async (assert) => {
  ...
  'unpublishRealm POSTs and maps the response': async (assert) => {
    let { client, calls } = makeClient(() =>
      jsonResponse(200, {
        data: {
  ...
  'unpublishRealm throws a RealmOperationError with status on failure': async (
    assert,
  ) => {
    let { client } = makeClient(() =>
  ...
  'checkDomainAvailability builds the query and returns the result': async (
    assert,
  ) => {
    let { client, calls } = makeClient(() =>
    ...
    let result = await checkDomainAvailability(client, { subdomain: 'mysite' });
  ...
  'fetchPublishabilityReport maps the report': async (assert) => {
    let { client, calls } = makeClient(() =>
      jsonResponse(200, {
        data: {
  ...
  'fetchPublishabilityReport defaults violations to an empty array': async (
    assert,
  ) => {
    let { client } = makeClient(() =>
      jsonResponse(200, {
  ...
  'waitForReady resolves once readiness returns ok': async (assert) => {
  ...
  'waitForReady throws after the timeout elapses': async (assert) => {
