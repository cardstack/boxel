...
import {
  ...
  type CaptureRunPerfEvent,
  type CapturePersistOutcome,
...
export interface CaptureCardArgs extends JSONTypes.Object {
  ...
  // null).
  captureSpec: CaptureRequestSpec | null;
  ...
  // bounded-wait caller (the GET `_capture/` route's sync wait) safe to
  ...
  persist: CapturePersistArgs | null;
...
const captureCard: Task<CaptureCardArgs, CapturePrerenderResponse> = ({
  ...
  async function (args) {
    ...
    // logs age out.
    let perfEvent = (
      fields: Partial<CaptureRunPerfEvent> & {
        status: CaptureRunPerfEvent['status'];
    ...
    if (!prerenderer.prerenderCapture) {
      let message = `${jobIdentity(jobInfo)} prerenderer does not support capture`;
      log.error(message);
    ...
    let persistOutcome: CapturePersistOutcome = 'skipped';
    ...
    if (persist && response.status === 'ready' && response.base64) {
      ...
      // hash would collapse onto the geometry-only key), so no persist
      // target can legitimately name one; those hash to null and always
      // refuse.
      let renderedSpecHash =
        isCanonicalCaptureFormat(format) &&
    ...
    // `render-runner.runCaptureAttempt`).
    ...
    let event = perfEvent({
      ...
      ...(diagnostics.renderElapsedMs != null
        ...
        : {}),
      ...(diagnostics.captureNavMs != null
        ? { navMs: diagnostics.captureNavMs }
        : {}),
      ...(diagnostics.captureSettleMs != null
        ? { settleMs: diagnostics.captureSettleMs }
        : {}),
      ...(diagnostics.captureImagePaintMs != null
        ? { imagePaintMs: diagnostics.captureImagePaintMs }
        : {}),
      ...(diagnostics.cdpCaptureMs != null
        ? { cdpCaptureMs: diagnostics.cdpCaptureMs }
        : {}),
      ...(decodeMs != null ? { decodeMs } : {}),
      ...(persistMs != null ? { persistMs } : {}),
    });
    emitCapturePerf(event);
    if (
      persist &&
