...
const MANIFEST_FILE_NAME = 'capture-manifest.json';
...
function topLevelError(error: string): CaptureResult {
...
export function registerCaptureCommand(program: Command): void {
  program
    .command('capture')
    // `screenshot` is kept as a typed alias: it is the word most people reach
    // for at a prompt, and a CLI verb costs nothing to carry. The platform
    // vocabulary is `capture` everywhere else.
    .alias('screenshot')
    .description(
      'Capture a card via the realm server: writes image or PDF files (or prints served URLs) plus a per-capture manifest',
    )
    .argument(
      '[card-url]',
    ...
    .option(
      '--spec <file>',
      'JSON batch spec — an entry or array of {card, format?, captureSpec?}; one request per card, writes capture-manifest.json into --out',
    )
    .option(
      '--url-only',
      'Print served capture URLs without downloading image bytes',
    )
    .option(
      '--realm <realm-url>',
    ...
    .action(async (cardUrl: string | undefined, opts: CaptureCliOptions) => {
      try {
        let realmSecretSeed = await resolveRealmSecretSeed(
          opts.realmSecretSeed === true,
        );
        let result = await capture(cardUrl, {
          format: opts.format,
