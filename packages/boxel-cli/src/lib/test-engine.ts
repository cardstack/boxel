...
interface QunitRunnerOptions {
  ...
  debug?: boolean;
  /**
   * Value for the HTTP `Authorization` header to send on requests to the
   * target realm's origin (remote mode) — sent verbatim, so it must be the
   * full header value (typically `Bearer <jwt>`), not a bare token. Takes
   * precedence over `pm.getRealmToken`, letting callers that hold a token
   * directly (e.g. the software factory, which owns its own BoxelCLIClient)
   * authenticate without an active CLI profile for the realm.
   */
...
async function runQunitInBrowser(options: QunitRunnerOptions): Promise<{
  ...
  try {
    ...
    let runEndTimeout = options.timeoutMs ?? 300_000;
    try {
      await page.waitForFunction(
        ...
        null,
        { timeout: runEndTimeout },
      );
    } catch (err) {
      // Label only the run-end wait timeout so callers can distinguish it from
      // other Playwright timeouts (navigation, asset fetch) and translate it
      // into their own diagnostic. The original error is kept as `cause`.
      if (err instanceof Error && err.name === 'TimeoutError') {
        throw new Error(
          `QUnit suite did not reach runEnd within ${runEndTimeout}ms`,
          { cause: err },
        );
      }
      throw err;
...
export interface RunRealmQunitOptions {
  ...
  debug?: boolean;
  /**
   * Value for the HTTP `Authorization` header, sent verbatim (typically
   * `Bearer <jwt>`, not a bare token); lets callers authenticate without a CLI
   * profile.
   */
  authorization?: string;
