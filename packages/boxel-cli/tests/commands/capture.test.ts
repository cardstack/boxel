...
function tempDir(): string {
  return mkdtempSync(join(tmpdir(), 'boxel-capture-test-'));
}

describe('boxel capture: single capture', () => {
  it('discovers the realm, posts a canonical request, and writes the image from base64', async () => {
    ...
    let out = tempDir();
    let result = await capture(`${CARD_URL}.json`, { authenticator, out });
    ...
    expect(entry.url).toBe(`${REALM_URL}_capture/Person/fadhlan`);
  ...
  it('names a paged capture from its content type, not the image default', async () => {
    ...
    let out = tempDir();
    let result = await capture(`${CARD_URL}.json`, {
      authenticator,
      out,
  ...
  it('passes the capture spec through verbatim (including target)', async () => {
    ...
    let result = await capture(CARD_URL, {
      authenticator,
      out: tempDir(),
      captureSpec: spec,
    });
  ...
  it('skips discovery when --realm is given and rejects a card outside it', async () => {
    ...
    let outside = await capture('http://elsewhere.test/Person/1', {
      authenticator,
      out: tempDir(),
      realm: REALM_URL,
    });
  ...
  it('downloads from the served URL when the response carries no base64', async () => {
    let servedUrl = `${REALM_URL}_capture/Person/fadhlan`;
    let { authenticator } = makeFake({
      postResponses: [
        () =>
          readyResponse({
...
describe('boxel capture: busy-server retry', () => {
...
describe('boxel capture: --url-only', () => {
...
describe('boxel capture: --spec batch', () => {
  it('posts once per card, writes named files, and writes a manifest', async () => {
    ...
    let result = await capture(undefined, {
      authenticator,
      out,
      specPath,
    });
    ...
    expect(result.manifestPath).toBe(join(out, 'capture-manifest.json'));
  ...
  it('lists every requested capture name when a card-level request fails', async () => {
    ...
    let result = await capture(undefined, { authenticator, out, specPath });
    expect(result.ok).toBe(false);
  ...
  it('rejects combining a card URL or capture flags with --spec', async () => {
    ...
    let both = await capture(CARD_URL, { specPath });
    expect(both.error).toContain('not both');

    let flags = await capture(undefined, {
      specPath,
  ...
  it('rejects a spec entry without a card URL', async () => {
    ...
    let result = await capture(undefined, { specPath });
  ...
  it('rejects a seed-mode spec whose cards span more than one realm owner', async () => {
    ...
    let crossOrigin = await capture(undefined, {
      specPath,
      realmSecretSeed: 'test-seed',
