...
// Drives `boxel capture` end-to-end through the real realm server, queue,
// and capture-card worker task. The prerenderer is a stub that returns a
...
const captureStubPrerenderer: Prerenderer = {
  prerenderModule: async () => ({ html: '', status: 200 }) as any,
  prerenderVisit: async () => ({}) as any,
  runCommand: async () => ({ status: 'ready' }),
  prerenderCapture: async (args): Promise<CapturePrerenderResponse> => {
...
const CARD_JSON = JSON.stringify({
  data: {
    type: 'card',
    attributes: { title: 'Capture Target' },
    meta: {
...
beforeAll(async () => {
  await startTestRealmServer({
    fileSystem: { 'capture-target.json': CARD_JSON },
    prerenderer: captureStubPrerenderer,
  });
  realmUrl = `${TEST_REALM_SERVER_URL}/test/`;
  cardUrl = `${realmUrl}capture-target`;
...
function tempOutDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'boxel-capture-out-'));
}

describe('capture (integration)', () => {
  it('captures a card and writes the image with a manifest entry', async () => {
    let out = tempOutDir();
    let res = await runBoxel(['capture', cardUrl, '--out', out, '--json'], {
      home,
    });
    let result = res.json<ManifestJson>();

    expect(result.ok, `capture failed: ${JSON.stringify(result)}`).toBe(true);
  ...
  it('runs a --spec batch and writes named files plus capture-manifest.json', async () => {
    let out = tempOutDir();
    ...
    let res = await runBoxel(
      ['capture', '--spec', specPath, '--out', out, '--json'],
      { home },
    );
    let result = res.json<ManifestJson>();
    ...
    let manifest = JSON.parse(
      fs.readFileSync(path.join(out, 'capture-manifest.json'), 'utf8'),
    ) as ManifestJson;
  ...
  it('reports a missing card as an error and exits non-zero', async () => {
    let res = await runBoxel(['capture', `${realmUrl}no-such-card`, '--json'], {
      home,
    });
    let result = res.json<ManifestJson>();
