...
| Ask | Run |
|---|---|
| "what's failing to index in <realm>?" | `boxel realm indexing-status --realm <realm-url>` |
| "give me the full error documents as JSON" | `boxel realm indexing-status --realm <realm-url> --json` |
| "is anything broken in this realm right now?" | `boxel realm indexing-status --realm <realm-url>` — exit 0 with "No indexing errors." is the all-clear |
...
boxel realm indexing-status --realm https://app.boxel.ai/owner/realm/
...
boxel realm indexing-status --realm https://app.boxel.ai/owner/realm/ --json \
...
- The list does not include queue depth, running-job progress, or a "last finished at" timestamp. Those are separate concerns and may be folded into this command later.
