...
The local monorepo source is useful for _browsing names_ (`ls ~/Projects/boxel/packages/boxel-icons/src/icons/ | grep -i check`) to find candidates, but always confirm against the CDN before committing to one. The Lucide picker at https://lucide.dev/icons/ is also a useful browser — Lucide icon names map to boxel-icons names — but again, verify each pick against the CDN.
