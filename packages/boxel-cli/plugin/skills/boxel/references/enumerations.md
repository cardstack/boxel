...
## Rendering enums as labels / icons / styled pills

The default `<@fields.priority />` renders a `BoxelSelect` (editable). When you just want to **display** the chosen value as a labeled chip / icon / colored pill — including from `embedded` and `fitted` templates — you need a value-to-label map. Several built-in helpers exist:

### Read the structured option for the current value

```gts
import enumField, { enumOptions } from 'https://cardstack.com/base/enum';

// In a Component getter:
get currentPriorityLabel() {
  const opts = enumOptions(this.args.model, 'priority');
  const current = opts.find(o => o.value === this.args.model.priority);
  return current?.label ?? this.args.model.priority;
}
```

### The const-map pattern (compact, no helper call per render)

When the labels never change at runtime, a module-level const map is cheaper than calling `enumOptions` each render:
...
const PRIORITY_ICONS: Record<string, ComponentLike> = {
  high: ArrowUpIcon,
  medium: MinusIcon,
  low: ArrowDownIcon,
};
...
// In the template:
<template>
  <span class='priority-pill {{@model.priority}}'>
    {{#let (get PRIORITY_ICONS @model.priority) as |Icon|}}
      {{#if Icon}}<Icon class='priority-icon' />{{/if}}
    {{/let}}
    {{get PRIORITY_LABELS @model.priority}}
  </span>
</template>
```
...
```hbs
{{! ❌ Avoid this — gets unreadable with 4+ options and locks logic in the template }}
{{#if (eq @model.priority 'high')}}<HighIcon />High
{{else if (eq @model.priority 'medium')}}<MediumIcon />Medium
{{else}}<LowIcon />Low
{{/if}}
```

Use the const-map or `enumOptions` pattern instead.
