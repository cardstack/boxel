...
const filtered: Query = {
  filter: {
    every: [{ type: meetRef }, { on: meetRef, eq: { meetLevel: 'Sectional' } }],
  },
};
```

Rule of thumb: `on` appears _inside_ `eq` / `contains` / `range` / `not` / `every` / `any`, never as the only key under `filter`.
...
Also: `realmURL` is a Symbol exported from `runtime-common` — import it directly. Don't write `Symbol.for('realmURL')`; that gives you a _different_ Symbol that doesn't match what the host injected.
...
// 3. "All of this type, filtered by a range"
{
  every: [
    { type: ref },
    { on: ref, range: { dueDate: { gte: '2026-01-01' } } },
  ];
}
...
const query: Query = {
  ...
  sort: [
    {
      by: 'price',
      on: codeRef(here, './product', 'Product'),
      direction: 'asc',
    },
  ],
};
