...
function loadToneJs(): Promise<void> {
  ...
  toneLoaderPromise = new Promise((resolve, reject) => {
    ...
    s.src = TONE_CDN_URL; s.async = true;
...
- **Sequencer / sampler creation order.** Build `Tone.Sequence`, `Tone.Sampler`, etc. *after* `Tone.start()`. They reference the global audio clock; created before resume, they sit silent until you tear them down and rebuild.
