...
**What this gives you:** A parent card whose entire job is layout — it has N `linksTo` children, and each child is rendered in the parent's template at a *chosen* format (one as `isolated`, one as `embedded`, one as `fitted`). The result: a single scrollable board composed of independent, separately-owned cards.
