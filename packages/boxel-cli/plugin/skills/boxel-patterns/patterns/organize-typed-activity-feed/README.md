...
**The insight:** Boxel CardDefs support inheritance: `class AcademicEntry extends FeedEntry` is a real CardDef that _is-a_ `FeedEntry`. The realm indexer tracks both — querying `filter: { type: codeRef(here, './feed', 'FeedEntry') }` returns every subtype; querying `filter: { type: codeRef(here, './feed', 'AcademicEntry') }` returns only that one. So you get one polymorphic stream + N typed-and-filtered views from the same data model, without an enum dispatch.
...
- [`organize-base-class-taxonomy`](../organize-base-class-taxonomy/README.md) — when the base has _no fields_, just the type marker. This pattern's base has fields too.
- [`polymorphic-field-subclass`](../polymorphic-field-subclass/README.md) — when the variant is a _slot inside one card_ (FieldDef inheritance), not a feed-of-cards (CardDef inheritance).
