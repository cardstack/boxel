...
**The insight:** `class extends StringField` works inside a function body. Return it. The closure captures `options` and `view`, the returned class is itself a valid field type. This is *much* cleaner than a shared `OptionField` base that subclasses extend — there's no inheritance to maintain.

**Recipe shape:**

```ts
const StatusField   = createOptionSelectField({ options: ['todo','doing','done'], view: 'boxel-select' });
const PriorityField = createOptionSelectField({ options: ['P0','P1','P2'],         view: 'radio' });
```
...
**Gotchas:**
- The returned class is a *new class every call*. Don't `===`-compare classes across modules.
