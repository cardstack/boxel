...
**When to use:** Whenever you build a card _family_ — 2+ related CardDefs (Meet + Swimmer + Club, Project + Task + Person, Show + Listing + Venue, etc.). Building a single utility card? Skip this pattern. Building anything where the user will accumulate instances over time? Build the home.
...
There is no snapshot/live toggle to reach for — the surface is live by default. The lever you _do_ have is `@mode`: `'hover'` (default) hydrates each result so it can respond to hover; `'none'` renders the prerendered HTML with no per-result interactivity, which is cheaper for dense read-only sections.
