...
**What this gives you:** A `Resource` subclass that owns mutable state (parsed library data, kanban column order, third-party adapter, expensive computation result) and _automatically re-runs_ when its named args change. Render-time `@tracked` properties on the Resource drive template reactivity.
...
class ChessJSResource extends Resource<ChessArgs> {
  ...
  @action prev() {
    /* navigate history; updates @tracked snapshotPgn */
  }
  @action next() {
    /* same */
  }
  ...
  commit(cards: CardDef[], columnKeys: string[]) {
    columnKeys.forEach((key) => {
      ...
      if (current) {
        ...
        const kept = current.cards.filter((c) =>
          matching.some((m) => m.id === c.id),
        );
