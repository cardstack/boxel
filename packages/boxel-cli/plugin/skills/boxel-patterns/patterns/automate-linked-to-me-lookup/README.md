...
- ❌ `model[Symbol.for('realmURL')]` — silent zero-rows trap. `Symbol.for` uses the global Symbol registry, which is a _different_ Symbol from the one the host injected at module scope. Import `realmURL` from `@cardstack/runtime-common` instead — that gives you the canonical Symbol the host uses.
