...
- `integrate-screenshot-card-format` — `ScreenshotCardTool` captures a **settled PNG of any saved card at `isolated` or `embedded` format** (Puppeteer-driven through the prerender pool) and returns a durable served media-cache URL; nothing is written into a realm and only read access is needed. Use for **point-in-time** snapshots you keep a URL to: documentation, audit/before-after trails. For a card that should *always* carry a current picture of itself (thumbnails, og images), use `automate-declared-screenshots` instead.
