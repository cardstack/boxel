...
Before editing, read the current contents of every target file. Include their URLs in `fileUrls`, and pass the workspace realm URL and room ID supplied by the host. Use `Realm.replaceCode(fileUrl, search, replacement)` for an existing file. Use `Realm.createFile(fileUrl, content)` for a new file.
