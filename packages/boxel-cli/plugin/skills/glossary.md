...
- **FileDef format shells** (`packages/base/file-formats/`) — The four shared atom/embedded/fitted/isolated shells every FileDef family composes; a family adds its `@field`s and a `static previewComponent` and inherits identity, facts, budgets, and state. `MarkdownDef`/`TextFileDef` embedded is a fixed-height, scroll-within bounded preview (no height custom-property lever); use `@format='isolated'` for the full-width document stage. → `boxel-ui-guidelines/references/delegated-render-control.md`
