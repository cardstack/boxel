---
description: List every `boxel_index` row in a realm whose latest indexing attempt failed (`has_error = TRUE`), rendered cleanly but holds broken `linksTo` / `linksToMany` targets, indexed but couldn't parse its frontmatter YAML, or indexed a skill whose frontmatter tools failed schema generation. Use to answer "why is this realm broken", "what's failing to index", "which cards have dead references", "why did my skill's commands disappear", or "why isn't my skill's tool callable" without reading the database directly.
---

# Realm indexing errors

`boxel realm indexing-errors` returns four classes of "indexing finding" for a realm:
...
4. **`tool-schema-error`** — `boxel_index` rows where `has_error = FALSE` but one or more of a skill markdown file's frontmatter tools failed index-time schema generation (module wouldn't load, missing export, schema generation threw), persisted as `diagnostics.toolSchemaErrors`. The skill indexes with the tools that did enrich; each failed tool won't be callable via the pull model until fixed — this finding is the only indexed signal. Each entry exposes `attributes.toolSchemaErrors: ToolSchemaError[]` — `{ module, name, message }`.
...
| "why isn't my skill's tool callable / which tools failed schema generation?" | `boxel realm indexing-errors --realm <realm-url> --json \| jq '.data[] \| select(.type == "tool-schema-error")'` |
...
Each line is `[<entryType>] <url>  <short summary>`. For `indexing-error` rows the summary is `errorDoc.title` (falling back to `errorDoc.message`). For `broken-link` rows it is `<N> broken: fieldName→reference, …` (up to three, then `+N more`). For `frontmatter-error` rows it is `frontmatter parse error (line L:C): <message>`. For `tool-schema-error` rows it is `<N> tool schema failure(s): module#name: <message>; …` (up to three, then `+N more`).
    ...
    },
    {
      ...
      "attributes": {
        ...
        "diagnostics": {
          "toolSchemaErrors": [
            {
              "module": "https://realm.example.com/missing-tool",
              "name": "default",
...
- `broken-link`, `frontmatter-error`, and `tool-schema-error` findings have **no** `errorDoc` — `has_error` is `FALSE`. Branch on `data[].type` (or check `attributes.brokenLinks` / `attributes.frontmatterParseError` / `attributes.toolSchemaErrors`) before reading `errorDoc`.
...
- A `tool-schema-error` likewise means the skill _did_ index; only the named tools lack generated schemas. Fixing the tool module (or the codeRef) and re-indexing the skill file clears it.
